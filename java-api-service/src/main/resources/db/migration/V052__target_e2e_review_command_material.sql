...
create table target_e2e_review_command_material (
    ...
    constraint uq_target_e2e_review_material_command unique (activation_id, command_id),
    constraint fk_target_e2e_review_material_admission
        foreign key (admission_id) references target_e2e_command_admission(admission_id),
    constraint fk_target_e2e_review_material_activation
    ...
    constraint fk_target_e2e_review_material_case
    ...
    constraint fk_target_e2e_review_material_epoch
    ...
    constraint ck_target_e2e_review_material_shape check (
        execution_lane = 'TARGET_E2E_CANDIDATE' and room_type = 'REVIEW'
        and material_schema_version = 'target-e2e-review-command-material.v1'
...
create or replace function enforce_target_e2e_review_command_material()
returns trigger language plpgsql as $$
declare admitted target_e2e_command_admission%rowtype; material jsonb;
begin
  select * into admitted from target_e2e_command_admission where admission_id = new.admission_id for share;
  if not found or admitted.activation_id is distinct from new.activation_id
    ...
    raise exception using errcode = '23514', message = 'target E2E Review material must exactly bind its command admission';
  end if;
  ...
  if material #>> '{schema_version}' is distinct from new.material_schema_version
    ...
    raise exception using errcode = '23514', message = 'target E2E Review material is not an exact admission binding';
  end if;
  return new;
end $$;

create trigger trg_target_e2e_review_material_binding before insert on target_e2e_review_command_material
for each row execute function enforce_target_e2e_review_command_material();
create trigger trg_target_e2e_review_material_immutable before update or delete on target_e2e_review_command_material
for each row execute function reject_target_e2e_append_only_mutation();
create trigger trg_target_e2e_review_material_no_truncate before truncate on target_e2e_review_command_material
for each statement execute function reject_target_e2e_append_only_mutation();
revoke all on target_e2e_review_command_material from public;
