create table target_e2e_evidence_terminal_receipt (
    ...
    constraint uq_target_e2e_evidence_terminal_epoch unique (case_id, room_epoch),
    constraint ck_target_e2e_evidence_terminal_hashes check (
...
create trigger trg_target_e2e_evidence_terminal_receipt_immutable
before update or delete on target_e2e_evidence_terminal_receipt
for each row execute function reject_target_e2e_append_only_mutation();
create trigger trg_target_e2e_evidence_terminal_receipt_no_truncate
before truncate on target_e2e_evidence_terminal_receipt
for each statement execute function reject_target_e2e_append_only_mutation();
revoke all on target_e2e_evidence_terminal_receipt from public;
