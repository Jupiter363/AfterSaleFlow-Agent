...
create table target_e2e_activation (
    ...
    production_target_e2e_activation varchar(16) not null,
    registered_at timestamptz not null default current_timestamp,
    constraint uq_target_e2e_activation_nonce unique (nonce),
    constraint uq_target_e2e_activation_identity unique (
        environment_id, environment_generation, activation_id, nonce, manifest_hash
    ),
    constraint uq_target_e2e_activation_epoch_binding unique (
        activation_id, manifest_hash, execution_lane, isolated_domain_db_binding_hash
    ),
    constraint ck_target_e2e_activation_identity check (
        activation_id ~ '^p9act[.]v1[.][0-9a-f]{32}$'
        and contract_version = 'target-e2e-activation.v1'
    ...
    constraint ck_target_e2e_activation_time check (
        expires_at > issued_at
        and expires_at <= issued_at + interval '7200 seconds'
        and lifecycle_changed_at >= registered_at
    ),
    constraint ck_target_e2e_activation_lifecycle check (
        (
    ...
    constraint ck_target_e2e_activation_scope check (
        (
    ...
    constraint ck_target_e2e_activation_rooms check (
    ...
    constraint ck_target_e2e_activation_bindings check (
    ...
    constraint ck_target_e2e_activation_images check (
    ...
    constraint ck_target_e2e_activation_database_separation check (
    ...
    constraint ck_target_e2e_activation_authority check (
        ...
        and production_target_e2e_activation = 'DISABLED'
    )
);

create index idx_target_e2e_activation_environment
    on target_e2e_activation(environment_id, environment_generation, lifecycle_status);

create table target_e2e_environment_generation_watermark (
    ...
    constraint ck_target_e2e_environment_watermark check (
...
create or replace function enforce_target_e2e_environment_generation()
returns trigger
language plpgsql
as $$
declare
...
begin
    ...
    if found then
        ...
        raise exception using errcode = '23505',
            message = 'target E2E activationId or nonce replay conflicts with a durable grant';
    end if;
    ...
    if not watermark_found then
        ...
        raise exception using errcode = '23514',
            message = 'target E2E environment generation must advance its durable high-water mark';
    else
        update target_e2e_environment_generation_watermark
...
create trigger trg_target_e2e_activation_generation
before insert on target_e2e_activation
for each row execute function enforce_target_e2e_environment_generation();

create or replace function guard_target_e2e_environment_watermark()
returns trigger
language plpgsql
as $$
begin
    if new.environment_id is distinct from old.environment_id
       ...
       or new.advanced_at < old.advanced_at then
        raise exception using errcode = '23514',
            message = 'target E2E environment generation high-water mark cannot regress';
    end if;
    return new;
end
$$;
...
create table target_e2e_case_reservation (
    ...
    constraint pk_target_e2e_case_reservation primary key (activation_id, slot_number),
    constraint uq_target_e2e_case_reservation_id unique (reservation_id),
    constraint uq_target_e2e_case_reservation_exact unique (
        activation_id, slot_number, reservation_id
    ),
    constraint uq_target_e2e_case_reservation_route unique (
        activation_id, tenant_surrogate, case_id
    ),
    constraint fk_target_e2e_case_reservation_activation
        foreign key (activation_id) references target_e2e_activation(activation_id),
    constraint ck_target_e2e_case_reservation_identity check (
    ...
    constraint ck_target_e2e_case_reservation_shape check (
        (
...
create or replace function enforce_target_e2e_case_reservation()
returns trigger
language plpgsql
as $$
declare
...
begin
    ...
    if not found then
        raise exception using errcode = '23503',
            message = 'target E2E case reservation requires an activation';
    end if;
    if activation_row.expires_at <= clock_timestamp()
       or (
        ...
        raise exception using errcode = '23514',
            message = 'target E2E case reservation requires a live activation in its allowed state';
    end if;
    if new.environment_id is distinct from activation_row.environment_id
       ...
       or new.case_scope_hash is distinct from activation_row.case_scope_hash then
        raise exception using errcode = '23514',
            message = 'target E2E case reservation binding mismatch';
    end if;
    if activation_row.case_scope_mode = 'EXPLICIT_CASE_IDS' then
        if new.reservation_kind <> 'EXPLICIT_CASE_ID'
           or new.slot_number > activation_row.explicit_case_count then
            raise exception using errcode = '23514',
                message = 'target E2E explicit case reservation exceeds signed scope';
        end if;
    else
        if new.reservation_kind <> 'ISOLATED_SYNTHETIC_NEW_CASE'
           ...
           or new.fixture_bytes_canonical_hash is distinct from
            ...
            raise exception using errcode = '23514',
                message = 'target E2E synthetic case reservation exceeds signed scope';
        end if;
    end if;
    if new.reservation_kind = 'ISOLATED_SYNTHETIC_NEW_CASE' and exists (
        ...
        raise exception using errcode = '23505',
            message = 'target E2E generated case ID conflicts with a durable reservation';
    end if;
    if new.reservation_kind = 'EXPLICIT_CASE_ID' and exists (
        select 1 from target_e2e_generated_case_tombstone tombstone
         where tombstone.generated_case_id = new.case_id
    ) then
        raise exception using errcode = '23505',
            message = 'target E2E explicit case ID conflicts with a global generated-ID tombstone';
    end if;
    return new;
end
$$;

create trigger trg_target_e2e_case_reservation_scope
before insert on target_e2e_case_reservation
for each row execute function enforce_target_e2e_case_reservation();
...
create trigger trg_target_e2e_case_reservation_immutable
before update or delete on target_e2e_case_reservation
...
create table target_e2e_generated_case_tombstone (
    ...
    constraint uq_target_e2e_generated_tombstone_reservation unique (reservation_id),
    constraint fk_target_e2e_generated_tombstone_reservation
        foreign key (activation_id, slot_number, reservation_id)
        references target_e2e_case_reservation(activation_id, slot_number, reservation_id),
    constraint ck_target_e2e_generated_tombstone check (
        generated_case_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$'
        and environment_generation between 1 and 9007199254740991
    )
);

create or replace function tombstone_target_e2e_generated_case()
returns trigger
language plpgsql
as $$
begin
    if new.reservation_kind = 'ISOLATED_SYNTHETIC_NEW_CASE' then
        insert into target_e2e_generated_case_tombstone (
...
create trigger trg_target_e2e_generated_case_tombstone
after insert on target_e2e_case_reservation
for each row execute function tombstone_target_e2e_generated_case();
create index idx_target_e2e_case_reservation_case
    on target_e2e_case_reservation(case_id);
create trigger trg_target_e2e_generated_tombstone_immutable
before update or delete on target_e2e_generated_case_tombstone
for each row execute function reject_target_e2e_append_only_mutation();
create trigger trg_target_e2e_generated_tombstone_no_truncate
before truncate on target_e2e_generated_case_tombstone
for each statement execute function reject_target_e2e_append_only_mutation();

create or replace function assert_target_e2e_explicit_scope_complete()
returns trigger
language plpgsql
as $$
declare
...
begin
    ...
    if persisted_count <> new.explicit_case_count then
        raise exception using errcode = '23514',
            message = 'target E2E explicit case scope must be persisted atomically and exactly';
    end if;
    return new;
end
$$;

create constraint trigger trg_target_e2e_explicit_scope_complete
...
begin
    if new.activation_id is distinct from old.activation_id
       ...
       or new.production_target_e2e_activation is distinct from old.production_target_e2e_activation
       or new.registered_at is distinct from old.registered_at then
        raise exception using errcode = '55000',
            message = 'target_e2e_activation immutable binding cannot be rewritten';
    end if;
    ...
    if new.lifecycle_changed_at < old.lifecycle_changed_at
       or not (
        ...
        raise exception using errcode = '23514',
            message = 'target E2E activation lifecycle transition is invalid';
    end if;
    if new.lifecycle_status = 'DRAINED' and exists (
        ...
        raise exception using errcode = '23514',
            message = 'target E2E activation cannot be DRAINED with unresolved accepted work';
    end if;
    return new;
end
$$;

create trigger trg_target_e2e_activation_guard
before update on target_e2e_activation
for each row execute function guard_target_e2e_activation_mutation();
create trigger trg_target_e2e_activation_no_delete
before delete on target_e2e_activation
for each row execute function reject_target_e2e_append_only_mutation();
create trigger trg_target_e2e_activation_no_truncate
before truncate on target_e2e_activation
for each statement execute function reject_target_e2e_append_only_mutation();

create trigger trg_target_e2e_environment_watermark_no_delete
before delete on target_e2e_environment_generation_watermark
for each row execute function reject_target_e2e_append_only_mutation();
create trigger trg_target_e2e_environment_watermark_no_truncate
before truncate on target_e2e_environment_generation_watermark
for each statement execute function reject_target_e2e_append_only_mutation();
...
create table target_e2e_command_admission (
    ...
    constraint uq_target_e2e_command_admission unique (activation_id, command_id),
    constraint fk_target_e2e_command_activation
        foreign key (
            activation_id, activation_manifest_hash, execution_lane,
            isolated_domain_db_binding_hash
        ) references target_e2e_activation(
    ...
    constraint fk_target_e2e_command_case
        foreign key (activation_id, tenant_surrogate, case_id)
        references target_e2e_case_reservation(activation_id, tenant_surrogate, case_id),
    constraint ck_target_e2e_command_admission check (
...
create or replace function enforce_target_e2e_command_admission()
returns trigger
language plpgsql
as $$
declare
...
begin
    ...
    if not found
       ...
       or clock_timestamp() >= activation_row.expires_at then
        raise exception using errcode = '23514',
            message = 'target E2E command requires pre-expiry ACTIVE admission';
    end if;
    return new;
end
$$;

create trigger trg_target_e2e_command_admission_guard
before insert on target_e2e_command_admission
for each row execute function enforce_target_e2e_command_admission();
create trigger trg_target_e2e_command_admission_immutable
before update or delete on target_e2e_command_admission
for each row execute function reject_target_e2e_append_only_mutation();
create trigger trg_target_e2e_command_admission_no_truncate
before truncate on target_e2e_command_admission
for each statement execute function reject_target_e2e_append_only_mutation();

create table target_e2e_command_completion (
    admission_id varchar(64) primary key,
    activation_id varchar(64) not null,
    command_id varchar(128) not null,
    command_hash varchar(64) not null,
    ...
    constraint fk_target_e2e_command_completion
        foreign key (admission_id) references target_e2e_command_admission(admission_id),
    constraint ck_target_e2e_command_completion_hashes check (
...
create or replace function enforce_target_e2e_command_completion()
returns trigger
language plpgsql
as $$
declare
...
begin
    ...
    if not found
       ...
       or new.completed_at < admission_row.admitted_at then
        raise exception using errcode = '23514',
            message = 'target E2E command completion binding mismatch';
    end if;
    return new;
end
$$;

create trigger trg_target_e2e_command_completion_guard
before insert on target_e2e_command_completion
for each row execute function enforce_target_e2e_command_completion();
create trigger trg_target_e2e_command_completion_immutable
before update or delete on target_e2e_command_completion
for each row execute function reject_target_e2e_append_only_mutation();
create trigger trg_target_e2e_command_completion_no_truncate
before truncate on target_e2e_command_completion
for each statement execute function reject_target_e2e_append_only_mutation();
...
alter table case_intake_epoch_selection_binding
    ...
    add constraint fk_target_e2e_intake_selection_activation
        foreign key (
            activation_id, activation_manifest_hash, execution_lane,
            isolated_domain_db_binding_hash
        ) references target_e2e_activation(
...
create trigger trg_target_e2e_intake_selection_guard
before insert on case_intake_epoch_selection_binding
for each row execute function enforce_target_e2e_intake_selection();

alter table agent_execution_manifest
    add constraint uq_target_e2e_agent_manifest_hash unique (id, manifest_sha256);
...
create table target_e2e_finalization_receipt (
    ...
    constraint uq_target_e2e_finalization_receipt_hash unique (receipt_hash),
    constraint uq_target_e2e_finalization_logical_run unique (activation_id, logical_run_id),
    constraint fk_target_e2e_finalization_activation
        foreign key (
            activation_id, activation_manifest_hash, execution_lane,
            isolated_domain_db_binding_hash
        ) references target_e2e_activation(
    ...
    constraint ck_target_e2e_finalization_hashes check (
        command_hash ~ '^[0-9a-f]{64}$'
        and command_envelope_hash ~ '^[0-9a-f]{64}$'
    ...
    constraint ck_target_e2e_finalization_authority check (
...
create or replace function enforce_target_e2e_finalization_receipt()
returns trigger
language plpgsql
as $$
declare
...
begin
    ...
    if not found
       ...
       or manifest_row.checkpoint_id is distinct from new.checkpoint_id then
        raise exception using errcode = '23514',
            message = 'target E2E finalization receipt manifest binding mismatch';
    end if;
    if not exists (
        select 1 from target_e2e_case_reservation reservation
         where reservation.activation_id = new.activation_id
        ...
        raise exception using errcode = '23514',
            message = 'target E2E finalization receipt case is outside activation scope';
    end if;
    return new;
end
$$;

create trigger trg_target_e2e_finalization_binding
before insert on target_e2e_finalization_receipt
for each row execute function enforce_target_e2e_finalization_receipt();
create trigger trg_target_e2e_finalization_immutable
before update or delete on target_e2e_finalization_receipt
for each row execute function reject_target_e2e_append_only_mutation();
create trigger trg_target_e2e_finalization_no_truncate
before truncate on target_e2e_finalization_receipt
for each statement execute function reject_target_e2e_append_only_mutation();
