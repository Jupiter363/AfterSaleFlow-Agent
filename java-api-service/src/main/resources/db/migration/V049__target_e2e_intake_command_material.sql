...
alter table target_e2e_room_epoch_binding
    add constraint uq_target_e2e_room_epoch_material_route unique (
...
create table target_e2e_intake_command_material (
    ...
    constraint uq_target_e2e_intake_material_command unique (activation_id, command_id),
    constraint fk_target_e2e_intake_material_admission
        foreign key (admission_id) references target_e2e_command_admission(admission_id),
    constraint fk_target_e2e_intake_material_activation
        foreign key (
    ...
    constraint fk_target_e2e_intake_material_case
    ...
    constraint fk_target_e2e_intake_material_epoch
        foreign key (
            activation_id, tenant_surrogate, case_id, room_type, room_epoch,
            room_fencing_token
        ) references target_e2e_room_epoch_binding(
            activation_id, tenant_surrogate, case_id, room_type, room_epoch,
            room_fencing_token
        ),
    constraint ck_target_e2e_intake_material_shape check (
        material_schema_version = 'target-e2e-intake-command-material.v1'
...
create trigger trg_target_e2e_intake_material_binding
before insert on target_e2e_intake_command_material
for each row execute function enforce_target_e2e_intake_command_material();
create trigger trg_target_e2e_intake_material_immutable
before update or delete on target_e2e_intake_command_material
for each row execute function reject_target_e2e_append_only_mutation();
create trigger trg_target_e2e_intake_material_no_truncate
before truncate on target_e2e_intake_command_material
for each statement execute function reject_target_e2e_append_only_mutation();

revoke all on target_e2e_intake_command_material from public;
