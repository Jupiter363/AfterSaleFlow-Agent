 ...
 * 文件职责：声明主张争点证据矩阵在 PostgreSQL 中的查询与写入契约。
...
// 所属模块：【PostgreSQL 事实模型 / 仓储接口层】类型「ClaimIssueEvidenceMatrixRepository」。
// 类型职责：声明主张争点证据矩阵在 PostgreSQL 中的查询与写入契约；本类型显式提供 框架生成的默认访问器。
