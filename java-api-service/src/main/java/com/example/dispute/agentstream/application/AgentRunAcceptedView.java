/*
 * 所属模块：Agent 流式运行。
 * 文件职责：定义Agent运行受理响应跨层传递时使用的不可变数据契约。
...
// 所属模块：【Agent 流式运行 / 应用编排层】类型「AgentRunAcceptedView」。
// 类型职责：定义Agent运行受理响应跨层传递时使用的不可变数据契约；本类型显式提供 框架生成的默认访问器。
// 协作关系：由同模块控制器、应用服务或框架生命周期创建和调用。
// 边界意义：运行必须绑定案件、房间和受众；任何协议越界都要在内容公开前终止
// Java 语法：record 用于不可变数据载体，编译器会生成组件访问器和值语义方法。
public record AgentRunAcceptedView(
        String runId,
        String status,
        String streamUrl,
