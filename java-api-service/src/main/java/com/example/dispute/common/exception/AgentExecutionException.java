/*
 * 所属模块：后端公共边界。
 * 文件职责：表达Agent执行失败语义，使上层能够区分业务拒绝、协议错误和可重试故障。
...
// 所属模块：【后端公共边界 / 核心业务层】类型「AgentExecutionException」。
// 类型职责：表达Agent执行失败语义，使上层能够区分业务拒绝、协议错误和可重试故障；本类型显式提供 「AgentExecutionException」。
// 协作关系：主要由 「CaseFulfillmentDisputeActivitiesImpl.awaitAgentRun」、「EvidenceAgentTurnService.safeRun」、「EvidenceAgentTurnService.validateEvidenceAssessmentCoverage」、「IntakeAgentTurnService.safeRun」 使用。
