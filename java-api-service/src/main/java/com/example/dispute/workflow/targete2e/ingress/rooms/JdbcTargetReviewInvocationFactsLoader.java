...
public final class JdbcTargetReviewInvocationFactsLoader {
  ...
  private static final String SQL = """
      select command.command_id, command.tenant_surrogate, command.case_id, command.room_epoch,
             ...
             task.id as review_task_id, task.packet_id as task_packet_id, task.task_status,
             ...
             packet.action_hash, packet.agent_run_refs_json::text, packet.packet_status, packet.frozen,
             packet.frozen_at, packet.expires_at,
  ...
  public Facts load(RoomGraphCommand command, long fencingToken) {
    ...
    return new Facts(value.reviewTaskId, value.packetId, value.packetVersion, value.taskStatus, fencingToken,
        ...
        ContractJson.sha256Hex(event), event, ContractJson.sha256Hex(event), refs(value.claimsJson, value.evidenceMatrixJson,
  ...
  private JsonNode frozenPacket(Row value) {
    ...
    packet.put("review_task_status", value.taskStatus); packet.put("assigned_reviewer_id", value.reviewerId);
  ...
  private static void collect(JsonNode node, com.fasterxml.jackson.databind.node.ArrayNode target, String... names) {
    ...
    if (node.isObject()) { node.fields().forEachRemaining(field -> { for (String name : names) if (name.equals(field.getKey()) && field.getValue().isTextual()) addIdentifier(target, field.getValue().asText()); collect(field.getValue(), target, names); }); }
  ...
  private static Row row(ResultSet r) throws SQLException { return new Row(r.getString("command_id"), r.getString("tenant_surrogate"), r.getString("case_id"), r.getLong("room_epoch"), r.getString("actor_role"), r.getLong("expected_process_revision"), r.getString("payload_sha256"), r.getString("event_json"), r.getString("review_task_id"), r.getString("task_packet_id"), r.getString("task_status"), r.getString("assigned_reviewer_id"), offset(r,"due_at"), r.getString("packet_id"), r.getString("packet_case_id"), r.getString("packet_plan_id"), r.getInt("packet_version"), r.getString("case_summary_json"), r.getString("claims_json"), r.getString("issues_json"), r.getString("evidence_matrix_json"), r.getString("draft_json"), r.getString("remedy_json"), r.getString("risk_flags_json"), r.getString("ruleset_version"), r.getString("action_hash"), r.getString("agent_run_refs_json"), r.getString("packet_status"), r.getBoolean("frozen"), offset(r,"expires_at"), r.getLong("case_version"), r.getInt("dossier_version"), r.getInt("issue_version"), r.getInt("adjudication_draft_version"), r.getInt("deliberation_report_version"), r.getInt("remedy_plan_version"), r.getString("approval_task_id"), r.getString("approval_packet_id"), r.getInt("review_packet_version"), r.getString("action_snapshot_hash"), r.getString("policy_version"), r.getString("reviewer_id"), r.getString("draft_id"), r.getString("authoritative_policy_version")); }
  ...
  private record Row(String commandId,String tenant,String caseId,long roomEpoch,String actorRole,long processRevision,String payloadHash,String eventJson,String reviewTaskId,String taskPacketId,String taskStatus,String reviewerId,OffsetDateTime dueAt,String packetId,String packetCaseId,String packetPlanId,int packetVersion,String caseSummaryJson,String claimsJson,String issuesJson,String evidenceMatrixJson,String draftJson,String remedyJson,String riskFlagsJson,String rulesetVersion,String actionHash,String agentRunRefsJson,String packetStatus,boolean frozen,OffsetDateTime expiresAt,long caseVersion,int dossierVersion,int issueVersion,int adjudicationDraftVersion,int deliberationReportVersion,int remedyPlanVersion,String approvalTaskId,String approvalPacketId,int approvalPacketVersion,String approvalActionHash,String policyVersion,String reviewerIdFromApproval,String draftId,String authoritativePolicyVersion) {}
}
