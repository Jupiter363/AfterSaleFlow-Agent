...
public final class JdbcTargetReviewOutcomeStartBindingPort implements TargetReviewOutcomeStartBindingPort {
  private static final String SQL = """
      select epoch.id as epoch_id, epoch.tenant_surrogate, epoch.case_id, epoch.room_id,
        ...
        join target_e2e_room_epoch_binding activation
        ...
        join target_e2e_activation activation_state
        ...
        join lateral (
          select policy_value.policy_version
            from approval_policy_decision policy_value
           where policy_value.case_id = task.case_id
             and policy_value.plan_id = task.plan_id
             and policy_value.created_at <= task.created_at
           order by policy_value.created_at desc, policy_value.id desc
           limit 1
        ) policy on true
       ...
       for update of epoch, activation, activation_state, task, packet, plan, draft
      """;
  ...
  private static void requireEpochMatches(ProvisionRoomEpoch provision, Row value) {
    if (!"REVIEW".equals(value.roomType) || !"ACTIVE".equals(value.lifecycleStatus)
  ...
  private static void requireFrozenPacket(Row value) {
    if (!value.packetId.equals(value.taskPacketId) || !value.taskPlanId.equals(value.packetPlanId)
        ...
        || !value.actionHash.matches("[0-9a-f]{64}") || value.policyVersion == null
  ...
  private static Row row(ResultSet rs) throws SQLException {
    return new Row(rs.getString("tenant_surrogate"), rs.getString("case_id"), rs.getString("room_id"),
        ...
        rs.getString("task_plan_id"), rs.getString("task_packet_id"), offset(rs, "review_opened_at"),
        ...
        rs.getBoolean("frozen"), offset(rs, "frozen_at"), offset(rs, "expires_at"), rs.getString("policy_version"));
  }
  ...
  private record Row(String tenant, String caseId, String roomId, String roomType, long epoch,
      ...
      String taskPacketId, OffsetDateTime openedAt, OffsetDateTime dueAt,
      ...
      boolean frozen, OffsetDateTime frozenAt, OffsetDateTime expiresAt, String policyVersion) {}
}
