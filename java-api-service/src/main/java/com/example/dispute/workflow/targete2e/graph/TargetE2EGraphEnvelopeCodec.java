...
/** Strict codec and RFC 8785 hash authority for the frozen target-E2E Graph wrappers. */
public final class TargetE2EGraphEnvelopeCodec {
  ...
  public TargetE2EGraphEnvelopeCodec(ObjectMapper objectMapper) {
    this(objectMapper, new AgentPlatformContractCodec());
  }

  public TargetE2EGraphEnvelopeCodec(
  ...
  public TargetE2EGraphCommandEnvelope wrapCommand(
    ...
    unhashed.put("schema_version", TargetE2EGraphCommandEnvelope.SCHEMA_VERSION);
    unhashed.put("execution_lane", TargetE2EGraphCommandEnvelope.EXECUTION_LANE);
    ...
    return new TargetE2EGraphCommandEnvelope(
        TargetE2EGraphCommandEnvelope.SCHEMA_VERSION,
        TargetE2EGraphCommandEnvelope.EXECUTION_LANE,
        activationId,
        roomFencingToken,
        commandHash,
        envelopeHash,
  ...
  public TargetE2EGraphCommandEnvelope decodeCommand(byte[] body) {
  ...
  public TargetE2EGraphResultEnvelope decodeResult(
      byte[] body,
    ...
    TargetE2EGraphResultEnvelope envelope =
        new TargetE2EGraphResultEnvelope(
            requiredText(node, "schema_version"),
            requiredText(node, "execution_lane"),
            requiredText(node, "activation_id"),
            requiredLong(node, "room_fencing_token"),
  ...
  String declaredProposalHash(byte[] body) {
    ...
    TargetE2EGraphCommandEnvelope.requirePattern(
        value, TargetE2EGraphCommandEnvelope.SHA256, "proposalHash");
    return value;
  }
  ...
  public TargetE2EGraphResultEnvelope wrapResult(
    ...
    ObjectNode unhashed = mapper.createObjectNode();
    unhashed.put("schema_version", TargetE2EGraphResultEnvelope.SCHEMA_VERSION);
    unhashed.put("execution_lane", TargetE2EGraphCommandEnvelope.EXECUTION_LANE);
    ...
    TargetE2EGraphResultEnvelope envelope =
        new TargetE2EGraphResultEnvelope(
            TargetE2EGraphResultEnvelope.SCHEMA_VERSION,
            TargetE2EGraphCommandEnvelope.EXECUTION_LANE,
            ...
            proposalHash,
            envelopeHash,
            TargetE2EGraphResultEnvelope.GRAPH_OUTPUT_AUTHORITY,
            result);
    requireValidResultEnvelope(envelope, commandEnvelope, schemaValidatedProposalSource);
    return envelope;
  }

  private void requireValidCommandEnvelope(TargetE2EGraphCommandEnvelope envelope) {
  ...
  private ValidatedProposal validatedProposal(
    ...
    TargetE2ERoomProposalSource decoded;
    try {
      decoded = mapper.treeToValue(source, TargetE2ERoomProposalSource.class);
    } catch (IOException exception) {
      throw new IllegalArgumentException("room proposal source is invalid", exception);
    }
    TargetE2ERoomProposalSource.Proposal proposal = decoded.proposal();
    boolean matches =
