/*
 * 所属模块：共享小法庭。
 * 文件职责：定义庭审结果Orchestration跨层传递时使用的不可变数据契约。
 * 业务链路：该文件主要提供类型或包级契约；管理固定轮次陈述、庭审时钟、和解版本、Agent 协作消息以及非最终裁决草案。
 * 关键边界：最多三轮且受三小时时钟约束；AI 输出必须进入平台终审而非直接生效
 */
package com.example.dispute.hearing.application;

// 所属模块：【共享小法庭 / 应用编排层】类型「HearingOutcomeOrchestrationResult」。
// 类型职责：定义庭审结果Orchestration跨层传递时使用的不可变数据契约；本类型显式提供 框架生成的默认访问器。
