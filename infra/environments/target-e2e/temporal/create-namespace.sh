...
control_build_component="${TARGET_E2E_CONTROL_BUILD_ID:-}"
agent_build_component="${TARGET_E2E_AGENT_BUILD_ID:-}"

if [ -z "$namespace" ]; then
  echo "TARGET_E2E_TEMPORAL_NAMESPACE must be set" >&2
  exit 64
fi
case "$namespace" in
  after-sale-flow-p9-*) ;;
...
require_nonempty_build_component TARGET_E2E_CONTROL_BUILD_ID "$control_build_component"
require_nonempty_build_component TARGET_E2E_AGENT_BUILD_ID "$agent_build_component"

if ! command -v tctl >/dev/null 2>&1; then
  echo "tctl is required to create the target E2E namespace" >&2
  exit 69
fi
...
ensure_default_legacy_build_id() {
  ...
  # Do not promote or mutate a conflicting Build ID set during target E2E startup.
