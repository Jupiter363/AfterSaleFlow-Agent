...
- Court-ledger modal accessibility follow-up: RED added focus-entry/trapping, explicit-close focus restoration, stacked-modal Escape ordering, and listener teardown coverage. Before implementation, the HearingCourtView target reported 42/44 passing with the two new behavior tests failing as expected, while the hearing-only Chromium run reported 18/20 passing with the same two missing behaviors. The minimum implementation now focuses the ledger close control on open, traps Tab and Shift+Tab inside the ledger, restores the exact ledger launcher after close-button/backdrop/Escape dismissal, preserves an underlying evidence drawer on the first Escape, and removes the shared keydown listener on teardown. GREEN verification reported 44/44 Vitest cases, 20/20 Chromium cases, and a successful production Vite build with only the existing large-chunk warning.
