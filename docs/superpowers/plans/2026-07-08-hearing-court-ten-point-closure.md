...
## 2026-07-14 终局双阶段草案与全角色卡片流式改造

本节继续作为十点方案的统一事实源，不创建独立终局方案文件。

### 第三轮终局顺序

1. 第三轮双方自然语言意见封存，只作为复核方向，不直接采纳为裁决。
2. 主审法官生成“最终拟处理方案 V1”；V1 必须明确、非空，并标注为非最终建议。
3. 前端立即创建法官占位卡，以“主审法官 + 小正：”开头逐段展示 V1。
4. V1 同时作为不可变审核对象交给唯一的“AI 评审员 + 小察”。
5. AI 评审员按事实完整性、证据一致性、规则适用、程序公平、方案可行性、风险与遗漏六项指标审核 V1；先创建评审占位卡，再逐段展示面向当事人的评审摘要。
6. 完整评审报告通过 A2A 返回法官；Java 必须校验 `reviewed_proposal` 与 V1 完全一致，禁止评审员替换审核对象、审批案件或触发执行。
7. 法官读取 V1、正式评审报告和既有庭审卷宗，在 C6 生成“裁决草案 V2”。V2 必须逐项回应高风险、阻断项和 `requires_revision`，只有 V2 能进入人工审核。
8. V2、方案和 ReviewTask 全部成功落库后，系统写入幂等休庭卡：庭审草案已记录，本庭休庭；案件进入人工审核，平台审核员将在一个工作日内处理，完成后可查看最终结果。
9. 休庭后用户与商家输入关闭并停留在等待审核状态；不得在 `DRAFT_READY` 或 `REVIEW_GATE_READY` 时自动跳到结果页。审核员可进入工作台；普通参与方仅在 `CASE_CLOSED` 后进入最终结果页。
10. 任一模型、结构化校验、评审、草案、方案或 ReviewTask 步骤失败时显式报错：不生成 V2、不写休庭消息、不使用模板兜底。

旧五 Critic 合议子流程对新案件停用。Temporal 代码只通过 `getVersion` 为已经写入历史的旧工作流保留确定性重放兼容；新案件不得在统一 AI 评审后再次启动第二套评审。

### 所有模型卡片的统一流式合同

适用角色：争议接待官、证据书记官、主审法官、AI 评审员、审核解释官，以及后续新增的用户可见数字人。重点保证法官、评审员和证据书记官生成的每一张用户可见卡片都遵循同一合同。

```text
Agent Run start
  -> 前端按 operation/node 立即创建角色占位卡
  -> 固定卡头：身份 + 人名：
  -> visible_delta 按 field_path 逐段追加到对应卡片
  -> 光标只跟随当前正在生成的字段
  -> final 到达后刷新 durable REST 消息并无跳变替换占位卡
```

规则：

- 身份和人名由可信的前端角色注册表提供，不能由模型自行生成；模型正文无需重复自我介绍。
- 法官、评审员、证据书记官的不同输出必须分别创建卡片，不能把 V1、评审报告、V2 拼进同一临时气泡。
- 一个 Agent Run 可以按确定的阶段顺序产生多张卡片；每张卡片由 `card_key + sender_role + speaker_name + public_field` 唯一定位。
- 法官公开卡由 `message_text` 逐段输出；第三轮 `final_proposed_resolution` 必须逐字嵌入该正文，同时作为结构化审核字段保留，不另生成一张内容重复的卡。AI 评审员公开字段为 `jury_review_report.public_message`；证据书记官公开字段覆盖 `room_utterance` 及实际展示为卡片的核验结论。
- 结构化看板、正式评审报告、证据矩阵和草案对象仍以 final 为权威；允许其用户可见文本字段流式预览，但不得把半成品写入数据库。
- 流式失败时保留失败卡的角色上下文并弹窗说明，禁止把残缺文本当正式消息；重试使用新的 run 或原幂等 run 的事件重放。
- 前端统一组件负责逐 token/短片段节奏、自动滚动、断线续传、光标和完成态；各房间只提供角色与卡片映射，不自行实现打字器。

### 人工审核时限

“一个工作日”必须与 `ReviewTask.due_at` 一致。初版按本地时区计算下一个工作日并跳过周六、周日；法定节假日表未接入前不得宣称已覆盖法定节假日。
