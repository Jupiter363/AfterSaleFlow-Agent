...
COMPOSE_FILE = ROOT / "infra/compose/production-runtime-uat.yml"
IMAGE_KEYS = (
    "postgres",
    "redis",
    "minio",
...
# The production-runtime release unit has one shared graph binding.  Keep the
...
PRODUCTION_RUNTIME_TOOL_POLICY_VERSION = "tools.none.v1"
PRODUCTION_RUNTIME_ALLOWED_ROOM_TYPES = ["INTAKE", "EVIDENCE", "HEARING", "REVIEW"]
PRODUCTION_RUNTIME_ALLOWED_STAGE_CODES = [
    "INTAKE_MESSAGE",
    "EVIDENCE_SEAL",
...
def parse_timestamp(value: Any, context: str) -> dt.datetime:
    if not isinstance(value, str):
        raise ProductionError(f"{context} must be an ISO-8601 timestamp")
    try:
        parsed = dt.datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as error:
        raise ProductionError(f"{context} must be an ISO-8601 timestamp") from error
    if parsed.tzinfo is None:
        raise ProductionError(f"{context} must include a timezone")
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ProductionError(f"cannot load strict JSON from {path}") from error
    if not isinstance(value, dict):
        raise ProductionError(f"{path} must contain a JSON object")
    return value
...
def atomic_create_json(path: Path, value: Any) -> None:
    ...
    try:
        ...
        raise ProductionError(f"atomic host lock already exists: {path}") from error
    try:
...
def verify_self_hash(document: dict[str, Any], context: str) -> None:
    ...
    if (
        ...
        raise ProductionError(f"{context} self-hash is missing or invalid")
...
def assert_regular_single_link(path: Path, context: str) -> None:
    ...
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise ProductionError(f"{context} must be a regular non-link file")
    if os.name != "nt" and metadata.st_nlink != 1:
        raise ProductionError(f"{context} must have exactly one hard link")
...
def load_image_lock(
    ...
    if set(document) != expected_fields:
        raise ProductionError("image lock fields drifted")
    if document["schema_version"] != "production-runtime-image-lock.v2":
        raise ProductionError("unsupported image lock schema")
    ...
    for key, record in images.items():
        ...
        if record["manifest_digest"] != reference.rsplit("@", 1)[1]:
            raise ProductionError(f"image {key} reference and manifest digest differ")
        if not isinstance(record["config_digest"], str) or not DIGEST.fullmatch(
            record["config_digest"]
        ):
            raise ProductionError(f"image {key} config digest is invalid")
        ...
        if (
            ...
            raise ProductionError(f"image {key} layer inventory is invalid")
        if not isinstance(record["build_id"], str) or not TOKEN.fullmatch(
            record["build_id"]
        ):
            raise ProductionError(f"image {key} build ID is invalid")
        source_revision = record["source_revision"]
        if not isinstance(source_revision, str) or not TOKEN.fullmatch(source_revision):
            raise ProductionError(f"image {key} source revision is invalid")
...
def expected_resource_names(run_id: str) -> dict[str, list[str]]:
    prefix = f"aflow_production_runtime_{run_id}_"
    return {
...
def load_run_lock(path: Path, *, require_active: bool = True) -> dict[str, Any]:
    ...
    for key, expected_path in expected_paths.items():
        if Path(lock.get(key, "")).resolve() != expected_path.resolve():
            raise ProductionError(f"run lock path is not host-derived: {key}")
    expected_lock_path = runtime_root / ".locks" / f"{project_name}.lock.json"
    if path.resolve() != expected_lock_path.resolve():
        raise ProductionError("run lock path is not the host-derived project lock")
    if lock.get("resources") != expected_resource_names(run_id):
        raise ProductionError("run lock resource inventory drifted")
    if require_active and (
        not isinstance(lock.get("ledger_public_key_sha256"), str)
        or not SHA256.fullmatch(lock["ledger_public_key_sha256"])
    ):
        raise ProductionError("active run lock has no trusted ledger key fingerprint")
    return lock
...
def validate_port_lock(lock: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or document.get("schema_version") != "production-runtime-port-lock.v1"
    ):
        raise ProductionError("gateway port lock fields drifted")
    expected = {
        "state": "ACTIVE",
    ...
    if any(document.get(key) != value for key, value in expected.items()):
        raise ProductionError("gateway port lock is not active for this exact run")
    return document


def validate_env_lock(env_file: Path) -> tuple[dict[str, str], dict[str, Any]]:
    env_file = assert_external_runtime_path(env_file)
    assert_regular_single_link(env_file, "production runtime env file")
    env = parse_env_file(env_file)
    lock_path = Path(env.get("PRODUCTION_RUNTIME_LOCK_PATH", ""))
    lock = load_run_lock(lock_path)
    expected = {
        "PRODUCTION_RUNTIME_RUN_ID": lock["run_id"],
        "PRODUCTION_RUNTIME_PROJECT_NAME": lock["project_name"],
        "PRODUCTION_RUNTIME_LOCK_NONCE": lock["lock_nonce"],
        "PRODUCTION_RUNTIME_BUILD_ID": lock["candidate_sha"],
        "PRODUCTION_RUNTIME_SOURCE_COMMIT": lock["candidate_sha"],
        "PRODUCTION_RUNTIME_IMAGE_LOCK_HASH": lock["image_lock_hash"],
        "PRODUCTION_RUNTIME_GATEWAY_PORT": str(lock["gateway_port"]),
    }
    for key, value in expected.items():
        if env.get(key) != value:
            raise ProductionError(f"env file does not match host lock field {key}")
    if env_file.resolve() != Path(lock["env_file"]).resolve():
        raise ProductionError("env file path does not match the host lock")
    if Path(lock["run_directory"]).resolve() != env_file.parent.resolve():
        raise ProductionError("run directory does not match the locked env directory")
    run_directory = Path(lock["run_directory"])
    exact_runtime_paths = {
        "PRODUCTION_RUNTIME_IMAGE_LOCK_PATH": run_directory / "image-lock.snapshot.json",
        "PRODUCTION_RUNTIME_RUN_CONTEXT_PATH": run_directory / "run-context.json",
        "PRODUCTION_RUNTIME_SECRETS_DIR": run_directory / "secrets",
        "PRODUCTION_RUNTIME_PUBLIC_DIR": run_directory / "public",
        "PRODUCTION_RUNTIME_ACTIVATION_DIR": run_directory / "java-activation",
        "PRODUCTION_RUNTIME_EVIDENCE_DIR": run_directory / "evidence",
        "PRODUCTION_RUNTIME_SOCKET_DIR": run_directory / "python-socket",
    }
    for key, expected_path in exact_runtime_paths.items():
        if Path(env.get(key, "")).resolve() != expected_path.resolve():
            raise ProductionError(f"env file redirects locked runtime path {key}")
...
def ledger_context_from_run_context(run_context: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or run_context.get("schema_version") != "production-runtime-run-context.v2"
    ):
        raise ProductionError("run context fields drifted")
    projection = run_context.get("runtime_projection")
    if (
        not isinstance(projection, dict)
        or set(projection) != RUNTIME_PROJECTION_KEYS
        or projection.get("schemaVersion") != "production-runtime-context.v1"
        or projection.get("executionLane") != "PRODUCTION"
        or projection.get("perCommandManifestAllowed") is not False
    ):
        raise ProductionError("run context runtime projection fields drifted")
    for key in (
        "activationId",
        "environmentId",
        "runNonce",
        ...
        if not isinstance(projection[key], str) or not TOKEN.fullmatch(projection[key]):
            raise ProductionError(f"runtime projection token is invalid: {key}")
    if (
        type(projection["environmentGeneration"]) is not int
        or projection["environmentGeneration"] < 1
    ):
        raise ProductionError("runtime projection environment generation is invalid")
    if not isinstance(projection["candidateSha"], str) or not SHA1.fullmatch(
        projection["candidateSha"]
    ):
        raise ProductionError("runtime projection candidate SHA is invalid")
    if (
        ...
        raise ProductionError("runtime projection activation manifest hash is invalid")
    ...
    if projection["allowedRoomTypes"] != ["INTAKE", "EVIDENCE", "HEARING", "REVIEW"]:
        raise ProductionError("runtime projection room inventory drifted")
    project = projection.get("composeProject")
    if not isinstance(project, str) or not project.startswith("aflow-production-runtime-"):
        raise ProductionError("runtime projection Compose project is invalid")
    run_id = project.removeprefix("aflow-production-runtime-")
    if not RUN_ID.fullmatch(run_id) or run_context[
        "resources"
    ] != expected_resource_names(run_id):
        raise ProductionError("runtime projection resource inventory drifted")
    ...
    if (
        ...
        raise ProductionError("runtime projection build bindings are invalid")
    ...
    if (
        ...
        raise ProductionError("runtime projection image digests are invalid")
    database_identities = projection["databaseIdentities"]
    if not isinstance(database_identities, dict) or set(database_identities) != {
        "domain",
        "graph",
    }:
        raise ProductionError("runtime projection database identities drifted")
    ...
    if domain != {
        ...
        raise ProductionError("runtime projection Domain identity is not authority-free")
    if (
    ...
    if (
        ...
        raise ProductionError("runtime projection trusted signing keys are invalid")
    if not isinstance(projection["caseScope"], dict):
        raise ProductionError("runtime projection case scope is missing")
    executor_bindings = run_context["executor_bindings"]
    if not isinstance(executor_bindings, list) or len(executor_bindings) != 1:
        raise ProductionError("run context must contain exactly one composite executor binding")
    ...
    if not isinstance(executor_binding, dict) or set(executor_binding) != TARGET_EXECUTOR_BINDING_KEYS:
        raise ProductionError("production runtime executor binding fields drifted")
    expected_executor_values = {
        "graph_key": PRODUCTION_RUNTIME_GRAPH_KEY,
        "graph_version": PRODUCTION_RUNTIME_GRAPH_VERSION,
        "checkpoint_schema_version": PRODUCTION_RUNTIME_CHECKPOINT_SCHEMA_VERSION,
        "output_schema_version": PRODUCTION_RUNTIME_OUTPUT_SCHEMA_VERSION,
        "tool_policy_version": PRODUCTION_RUNTIME_TOOL_POLICY_VERSION,
        "allowed_room_types": list(PRODUCTION_RUNTIME_ALLOWED_ROOM_TYPES),
        "allowed_stage_codes": list(PRODUCTION_RUNTIME_ALLOWED_STAGE_CODES),
    }
    if any(executor_binding.get(key) != value for key, value in expected_executor_values.items()):
        raise ProductionError("production runtime executor binding is not the all-room composite")
    for key in ("state_schema_hash", "binding_hash"):
        if not isinstance(executor_binding[key], str) or not SHA256.fullmatch(executor_binding[key]):
            raise ProductionError(f"production runtime executor binding hash is invalid: {key}")
    if not isinstance(executor_binding["code_build_id"], str) or not TOKEN.fullmatch(
        executor_binding["code_build_id"]
    ):
        raise ProductionError("production runtime executor code build ID is invalid")
    for key in (
        "activation_manifest_hash",
        "image_lock_hash",
        "jwks_sha256",
        ...
        if not isinstance(run_context[key], str) or not SHA256.fullmatch(
            ...
            raise ProductionError(f"run context hash binding is invalid: {key}")
    ...
    if not isinstance(mtls, dict) or set(mtls) != {
        ...
        raise ProductionError("run context mTLS binding fields drifted")
    if mtls["expected_spiffe_id"] != "spiffe://after-sale-flow/java-api-service" or any(
        ...
        raise ProductionError("run context mTLS binding is invalid")
    return {
...
def validate_run_context_bindings(
    ...
    if any(
        ...
        raise ProductionError("run context does not match the atomic host lock")
    if (
        Path(run_context["image_lock_path"]).resolve()
        != Path(env["PRODUCTION_RUNTIME_IMAGE_LOCK_PATH"]).resolve()
    ):
        raise ProductionError("run context image-lock path is not the locked snapshot")
    if (
        image_lock["self_hash"] != lock["image_lock_hash"]
        or image_lock["candidate_sha"] != lock["candidate_sha"]
    ):
        raise ProductionError("image-lock snapshot does not match the atomic host lock")
    try:
        environment_generation = int(env.get("PRODUCTION_RUNTIME_ENVIRONMENT_GENERATION", ""))
    except ValueError as error:
        raise ProductionError("environment generation is not an integer") from error
    expected_projection_bindings: dict[str, Any] = {
        "candidateSha": lock["candidate_sha"],
        "composeProject": lock["project_name"],
        "activationId": env.get("PRODUCTION_RUNTIME_ACTIVATION_ID"),
        "environmentId": env.get("PRODUCTION_RUNTIME_ENVIRONMENT_ID"),
        "environmentGeneration": environment_generation,
        "temporalNamespace": env.get("PRODUCTION_RUNTIME_TEMPORAL_NAMESPACE"),
        "runNonce": env.get("PRODUCTION_RUNTIME_RUN_NONCE"),
    }
    if any(
        projection.get(key) != value
        for key, value in expected_projection_bindings.items()
    ):
        raise ProductionError("runtime projection does not match the locked environment")
    runtime_context_hash = canonical_sha256(projection)
    if (
        env.get("PRODUCTION_RUNTIME_ACTIVATION_MANIFEST_HASH")
        != run_context["activation_manifest_hash"]
        or env.get("PRODUCTION_RUNTIME_GRAPH_RUNTIME_CONTEXT_HASH") != runtime_context_hash
    ):
        raise ProductionError(
    ...
    if run_context["mtls"]["ca_certificate_sha256"] != env.get(
        "PRODUCTION_RUNTIME_MTLS_CA_CERT_SHA256"
    ) or run_context["mtls"]["client_certificate_sha256"] != env.get(
        "PRODUCTION_RUNTIME_MTLS_CLIENT_CERT_SHA256"
    ):
        raise ProductionError(
            "runtime projection mTLS fingerprints differ from the environment"
        )
    jwks_path = Path(env["PRODUCTION_RUNTIME_PUBLIC_DIR"]) / "jwks" / "graph-jwks.json"
    if file_sha256(jwks_path) != run_context["jwks_sha256"]:
        raise ProductionError("static JWKS resource differs from the signed run context")
    try:
        env_projection = json.loads(env["GRAPH_PRODUCTION_RUNTIME_CONTEXT"])
        env_bindings = json.loads(env["GRAPH_PRODUCTION_RUNTIME_BINDINGS"])
...
def parse_env_file(path: Path) -> dict[str, str]:
    ...
    for number, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        if not raw or raw.startswith("#"):
            continue
        if "=" not in raw:
            raise ProductionError(f"invalid env line {number}")
        key, value = raw.split("=", 1)
        if not re.fullmatch(r"[A-Z][A-Z0-9_]*", key) or key in values:
            raise ProductionError(f"invalid or duplicate env key on line {number}")
...
def run_command(
    ...
    *,
    ...
    if check and completed.returncode:
        ...
        raise ProductionError(f"command failed ({completed.returncode}): {message}")
    return completed
...
def container_id(env_file: Path, service: str) -> str:
    ...
    if len(identifiers) != 1:
        raise ProductionError(f"expected exactly one {service} container")
    return identifiers[0]


def env_quote(value: str) -> str:
    if "\x00" in value or "\n" in value or "\r" in value or "'" in value:
        raise ProductionError("env values must be single-line text")
    return "'" + value + "'"
