...
TARGET_BUILD_LABEL = "production-runtime.after-sale-flow.dev/build-id"
...
APPLICATION_IMAGES = {
    "java": ApplicationImage(
        repository="java",
        context=common.ROOT / "apps/domain-service",
        dockerfile=common.ROOT / "apps/domain-service" / "Dockerfile.production-runtime",
    ),
    "python": ApplicationImage(
        repository="python",
        context=common.ROOT / "apps/agent-runtime",
        dockerfile=common.ROOT / "apps/agent-runtime" / "Dockerfile",
    ),
    "ocr": ApplicationImage(
        repository="ocr",
        context=common.ROOT / "apps/ocr-parser",
        dockerfile=common.ROOT / "apps/ocr-parser" / "Dockerfile",
    ),
    "frontend": ApplicationImage(
        repository="frontend",
        context=common.ROOT / "apps/web",
        dockerfile=common.ROOT / "apps/web" / "Dockerfile",
    ),
}
...
def _strict_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, item in pairs:
        if key in value:
            raise common.ProductionError(f"base image input repeats JSON member: {key}")
        value[key] = item
    return value
...
def load_base_images(path: Path) -> dict[str, str]:
    ...
    try:
        ...
        raise common.ProductionError("base image input is not strict UTF-8 JSON") from error
    ...
    if any(
        ...
        raise common.ProductionError("every base image must be an immutable manifest reference")
...
def _run(arguments: list[str], *, timeout: int = 3600) -> subprocess.CompletedProcess[str]:
    ...
    if completed.returncode:
        ...
        raise common.ProductionError(f"image-lock command failed: {detail}")
    return completed


def _repository_state(candidate: str) -> str:
    if common.SHA1.fullmatch(candidate) is None:
        raise common.ProductionError("candidate must be one exact lowercase 40-character Git SHA")
    ...
    if head != candidate or resolved != candidate:
        raise common.ProductionError("candidate must equal the exact checked-out HEAD commit")
    if dirty:
        raise common.ProductionError("candidate image build requires a clean working tree")
    return head
...
def _source_tree_digest(candidate: str) -> str:
    ...
    if completed.returncode:
        ...
        raise common.ProductionError(f"cannot archive exact candidate: {detail}")
...
def _inspect_image(docker: str, reference: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise common.ProductionError("Docker image inspect did not return JSON") from error
    if not isinstance(documents, list) or len(documents) != 1 or not isinstance(documents[0], dict):
        raise common.ProductionError("Docker image inspect did not return exactly one image")
    return documents[0]
...
def _build_manifest_digest(path: Path) -> str:
    ...
    try:
        ...
        raise common.ProductionError("Buildx metadata is not strict UTF-8 JSON") from error
    ...
    if not isinstance(digest, str) or common.DIGEST.fullmatch(digest) is None:
        raise common.ProductionError("Buildx metadata has no exact manifest digest")
    return digest
...
def _record_from_inspection(
    ...
    *,
    ...
    if not isinstance(repo_digests, list):
        raise common.ProductionError(f"image {repository} has no registry manifest digest")
    matching = sorted(
        value
    ...
    if expected_reference is not None:
        ...
        raise common.ProductionError("built image manifest identity is missing or ambiguous")
    ...
    if (
        ...
        raise common.ProductionError("Docker image config or ordered layer identity is invalid")
    return {
        "reference": reference,
...
def _build_application_image(
    *,
    ...
    if not specification.dockerfile.is_file() or not specification.context.is_dir():
        raise common.ProductionError(f"application image build inputs are missing for {key}")
    ...
    if key == "java":
        build_command.extend(["--build-arg", f"PRODUCTION_RUNTIME_SOURCE_SHA={candidate}"])
    ...
    if not isinstance(labels, dict) or (
        ...
        raise common.ProductionError(f"built application image labels drifted for {key}")
    ...
    if exact != record:
        raise common.ProductionError(f"built image changed when pulled by digest for {key}")
    return record
...
def build_lock(
    *,
    ...
    if set(base_images) != BASE_IMAGE_KEYS:
        raise common.ProductionError("base image inventory is incomplete")
    if REPOSITORY_PREFIX.fullmatch(repository_prefix) is None or "@" in repository_prefix:
        raise common.ProductionError("repository prefix is invalid")
    if INVOCATION_ID.fullmatch(invocation_id) is None or common.TOKEN.fullmatch(builder_id) is None:
        raise common.ProductionError("build invocation or builder identity is invalid")
    docker_path = shutil.which(docker)
    if docker_path is None:
        raise common.ProductionError("Docker CLI is unavailable")
    ...
    attestation = common.seal_self_hash(
        {
            "schema_version": "production-runtime-build-attestation.v1",
    ...
    lock = common.seal_self_hash(
        {
            ...
            "build_provenance": {
                ...
                "source_tree_sha256": source_tree_sha256,
                "built_at": timestamp.isoformat(timespec="seconds"),
                "attestation_type": "production-runtime-build-attestation.v1",
                "attestation_digest": "sha256:" + common.canonical_sha256(attestation),
            },
            "images": images,
        }
    )
    lock_path = root / "production-runtime-image-lock.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (common.ProductionError, OSError, subprocess.SubprocessError) as error:
