...
def _environment(service: dict[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(value, dict):
        raise common.ProductionError("rendered Compose environment must be an object")
...
def _validate_graph_mtls_proxy_networks(proxy: dict[str, Any]) -> None:
    if _networks(proxy) != EXPECTED_GRAPH_MTLS_PROXY_NETWORKS:
        raise common.ProductionError("mTLS proxy has an unexpected peer network")


def _validate_graph_exchange_proxy(proxy: dict[str, Any]) -> None:
    if _networks(proxy) != EXPECTED_GRAPH_EXCHANGE_PROXY_NETWORKS:
        raise common.ProductionError("graph exchange proxy has an unexpected peer network")
    volumes = proxy.get("volumes")
    if not isinstance(volumes, list) or len(volumes) != 1:
        raise common.ProductionError("graph exchange proxy config mount drifted")
    mount = volumes[0]
...
def _resource_labels(resource: dict[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(labels, dict):
        raise common.ProductionError("resource labels must be an object")
...
def validate_rendered_config(
    ...
    run_id = env["PRODUCTION_RUNTIME_RUN_ID"]
    ...
    required_labels = {
        "production-runtime.after-sale-flow.dev/run-id": run_id,
        "production-runtime.after-sale-flow.dev/project": expected_project,
    ...
    for name, service in services.items():
        ...
        if service.get("image") != expected_image:
            raise common.ProductionError(f"{name} image is not the locked OCI manifest")
    ...
    if _networks(services["domain-db"]) & _networks(services["graph-db"]):
        raise common.ProductionError("Domain and Graph databases share a network")
    ...
    if urlsplit(dsn).hostname != "graph-db" or "/production_graph" not in dsn:
        raise common.ProductionError(
    ...
    if "--uds /run/production-runtime/python/agent.sock" not in command or "--host" in command:
        raise common.ProductionError(
    ...
    if _networks(python) & _networks(services["domain-db"]):
        raise common.ProductionError("Python can reach the Domain database network")

    exchange_proxy = services["graph-exchange-proxy"]
    if exchange_proxy.get("image") != images["nginx"]["reference"]:
        raise common.ProductionError("graph exchange proxy image is not the locked Nginx manifest")
    _validate_graph_exchange_proxy(exchange_proxy)
    if (
        python_environment.get("JAVA_API_SERVICE_URL")
        != "http://graph-exchange-proxy:8080"
    ):
        raise common.ProductionError("Python must use the restricted graph exchange proxy")
    ...
    if graph_exchange_members != {"graph-exchange-proxy", "java-api-service"}:
        raise common.ProductionError("graph exchange network membership drifted")
    ...
    if socket_sources != {env["PRODUCTION_RUNTIME_SOCKET_DIR"]}:
        raise common.ProductionError(
    ...
    if mtls_members != {"graph-mtls-proxy", "java-agent-worker", "mtls-proof"}:
        raise common.ProductionError("the mTLS client network membership drifted")

    jwks = services["jwks-server"]
    if jwks.get("image") != images["nginx"]["reference"] or _networks(jwks) != {
        "python-egress"
    }:
        raise common.ProductionError("JWKS must be a public-only static Nginx service")
    ...
    for service in services.values():
        for port in service.get("ports", []):
            ...
            if published in FORBIDDEN_BASELINE_PORTS or published in published_ports:
                raise common.ProductionError(
                    "production runtime port overlaps a baseline or another target service"
                )
            published_ports.add(published)
    if published_ports != {lock["gateway_port"]}:
        raise common.ProductionError(
            "only the locked production runtime gateway port may be published"
        )
    ...
    for value in _walk_strings(config):
        ...
        if hostname not in ALLOWED_URL_HOSTS:
            raise common.ProductionError(f"non-local endpoint is forbidden: {hostname}")
...
def verify_local_images(images: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for key, expected in images.items():
        ...
        if result.returncode:
            raise common.ProductionError(f"locked image is unavailable locally: {key}")
        document = json.loads(result.stdout)[0]
        if document.get("Id") != expected["config_digest"]:
            raise common.ProductionError(f"local image config digest mismatch: {key}")
        repo_digests = set(document.get("RepoDigests") or [])
        if expected["reference"] not in repo_digests:
            raise common.ProductionError(f"local image manifest digest mismatch: {key}")
        layers = document.get("RootFS", {}).get("Layers") or []
        if layers != expected["layer_digests"]:
            raise common.ProductionError(f"local image layer inventory mismatch: {key}")
        measured[key] = {
...
def run_preflight(env_file: Path) -> dict[str, Any]:
    ...
    if not records or records[0]["payload_type"] != "PROVISIONED_RUN_CONTEXT":
        raise common.ProductionError("evidence ledger lacks its provisioned root record")
    ...
    payload = {
        "schema_version": "production-runtime-preflight.v2",
        "status": "PASS",
        "run_id": lock["run_id"],
    ...
    record = ledger.append_record(
        ...
        source_identity="production-runtime-preflight",
        case_id=None,
    ...
    common.write_json(Path(env["PRODUCTION_RUNTIME_EVIDENCE_DIR"]) / "preflight.json", receipt)
    return receipt
