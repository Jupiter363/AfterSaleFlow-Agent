...
def _validate_labels(
    ...
    if any(labels.get(key) != value for key, value in _expected_labels(lock).items()):
        raise common.ProductionError(f"{context} labels do not match the host lock")
...
def _locked_resources(lock: dict[str, Any]) -> tuple[list[str], list[str], list[str]]:
    ...
    labeled_container_ids = {
        ...
        for value in common.run_command(
            [
                ...
                "--all",
                "--filter",
                f"label=production-runtime.after-sale-flow.dev/project={lock['project_name']}",
                "--format",
                "{{.ID}}",
            ]
        ).stdout.splitlines()
    ...
    if labeled_container_ids != set(container_ids):
        raise common.ProductionError(
            "host contains a mislabeled or non-Compose container for this production runtime project"
        )
...
def _release_port_lock(lock: dict[str, Any], released_at: str) -> None:
    ...
    if any(document.get(key) != value for key, value in expected.items()):
        raise common.ProductionError("gateway port lock owner or identity drifted")
    common.write_json(
        path,
...
def teardown(env_file: Path) -> dict[str, Any]:
    ...
    base_receipt = {
        "schema_version": "production-runtime-teardown-receipt.v2",
        "status": "PASS",
        "run_id": lock["run_id"],
    ...
    record = ledger.append_record(
        ...
        source_identity="production-runtime-exact-resource-teardown",
        case_id=None,
    ...
    common.write_json(Path(env["PRODUCTION_RUNTIME_LOCK_PATH"]), released_lock)
    return receipt
...
def cleanup_incomplete_provision(lock_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "production-runtime-incomplete-provision-cleanup.v1",
        "status": "PASS",
        "run_id": lock["run_id"],
        "removed_container_ids": sorted(container_ids),
        "removed_networks": sorted(networks),
