...
def _b64url_decode(value: str, context: str) -> bytes:
    if not isinstance(value, str) or not value or "=" in value:
        raise common.TargetE2EError(f"{context} is not canonical base64url")
    try:
        decoded = base64.b64decode(
            value + "=" * (-len(value) % 4), altchars=b"-_", validate=True
        )
    except (binascii.Error, ValueError) as error:
        raise common.TargetE2EError(f"{context} is not canonical base64url") from error
...
def load_private_key(path: Path) -> ec.EllipticCurvePrivateKey:
    ...
    if not isinstance(key, ec.EllipticCurvePrivateKey) or not isinstance(
        ...
        raise common.TargetE2EError("attestation private key must be P-256")
    return key
...
def load_public_key(path: Path) -> ec.EllipticCurvePublicKey:
    ...
    if not isinstance(key, ec.EllipticCurvePublicKey) or not isinstance(
        ...
        raise common.TargetE2EError("attestation public key must be P-256")
    return key
...
def _verify(key: ec.EllipticCurvePublicKey, signature: str, payload: bytes) -> None:
    try:
        ...
        raise common.TargetE2EError("P-256 attestation signature is invalid") from error
...
def verify_attested_document(
    ...
    *,
    ...
    if not isinstance(attestation, dict) or set(attestation) != ATTESTATION_KEYS:
        raise common.TargetE2EError(f"{context} attestation fields drifted")
    ...
    if (
        ...
        raise common.TargetE2EError(f"{context} attestation identity is invalid")
...
def verify_compact_jws(
    ...
    *,
    ...
    if not isinstance(compact, str) or len(compact) > 1024 * 1024:
        raise common.TargetE2EError("compact JWS is missing or too large")
    parts = compact.split(".")
    if len(parts) != 3:
        raise common.TargetE2EError("compact JWS must have three segments")
    try:
        ...
        raise common.TargetE2EError("compact JWS JSON is invalid") from error
    if not isinstance(protected, dict) or set(protected) != {"alg", "kid", "typ"}:
        raise common.TargetE2EError("JWS protected header fields drifted")
    if protected["alg"] != "ES256" or protected["typ"] != expected_typ:
        raise common.TargetE2EError("JWS algorithm or type is invalid")
    key_id = protected["kid"]
    key = public_keys.get(key_id)
    if key is None:
        raise common.TargetE2EError("JWS signing key is not trusted")
    raw = _b64url_decode(parts[2], "JWS signature")
    if len(raw) != 64:
        raise common.TargetE2EError("ES256 JWS signature must be 64 bytes")
    ...
    try:
        ...
        raise common.TargetE2EError("Java evidence JWS signature is invalid") from error
    if not isinstance(payload, dict):
        raise common.TargetE2EError("JWS payload must be an object")
    if payload.get("iss") != expected_issuer or payload.get("aud") != expected_audience:
        raise common.TargetE2EError("JWS issuer or audience is invalid")
    observed = now or common.utc_now()
    issued = payload.get("iat")
    expires = payload.get("exp")
    if type(issued) is not int or type(expires) is not int:
        raise common.TargetE2EError("JWS iat and exp must be integer epoch seconds")
    if expires <= issued or expires - issued > 300:
        raise common.TargetE2EError("JWS freshness window exceeds five minutes")
    epoch = int(observed.timestamp())
    if issued > epoch + 5 or expires < epoch:
        raise common.TargetE2EError("JWS is future-dated or expired")
    ...
    *,
    ...
    for index, record in enumerate(records, 1):
        if (
            ...
            or record.get("schema_version") != "target-e2e-evidence-ledger-record.v1"
        ):
            raise common.TargetE2EError("evidence ledger record fields drifted")
        ...
        if record["payload_hash"] != common.canonical_sha256(record["payload"]):
            raise common.TargetE2EError("evidence ledger payload hash is invalid")
        ...
        if claimed_hash != expected_hash:
            raise common.TargetE2EError("evidence ledger record self-hash is invalid")
        if not isinstance(attestation, dict) or set(attestation) != ATTESTATION_KEYS:
            raise common.TargetE2EError("evidence ledger attestation fields drifted")
        if (
        ...
        if fresh_until <= recorded or fresh_until - recorded > dt.timedelta(minutes=10):
            raise common.TargetE2EError("ledger record freshness window is invalid")
        if prior_time is not None and recorded < prior_time:
            raise common.TargetE2EError("ledger timestamps are not monotonic")
        prior_time = recorded
    ...
    if require_fresh_last and (
        ...
        raise common.TargetE2EError("last ledger attestation is missing or stale")
    return records
...
def append_record(
    ...
    *,
    ...
    if source_kind not in {"HARNESS_DIRECT", "JAVA_SIGNED"}:
        raise common.TargetE2EError("ledger source kind is not trusted")
    ...
    try:
        ...
        now = common.utc_now()
        unsigned = {
            "schema_version": "target-e2e-evidence-ledger-record.v1",
