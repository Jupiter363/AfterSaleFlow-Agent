...
def load_command(path: Path) -> list[str]:
    ...
    try:
        ...
        raise common.TargetE2EError("Batch 4 command file is not strict UTF-8 JSON") from error
    if (
        ...
        raise common.TargetE2EError("Batch 4 command must be a bounded argv array")
    executable = value[0]
    if Path(executable).is_absolute():
        if not Path(executable).is_file():
            raise common.TargetE2EError("Batch 4 command executable is unavailable")
...
def _run_stage(command: list[str], env_file: Path, case_id: str, stage: str) -> None:
    ...
    if completed.returncode:
        raise common.TargetE2EError(f"Batch 4 {stage} stage failed")
    if len(completed.stdout.encode("utf-8")) > 1024 * 1024 or len(
        completed.stderr.encode("utf-8")
    ) > 1024 * 1024:
        raise common.TargetE2EError(f"Batch 4 {stage} stage output is unbounded")
...
def run_batch4(
    ...
    *,
    ...
    if wait_timeout < 30 or wait_timeout > 1800:
        raise common.TargetE2EError("Batch 4 wait timeout is outside the bounded range")
    ...
    scenario_path = Path(env["TARGET_E2E_EVIDENCE_DIR"]) / "batch-4-scenario.json"
