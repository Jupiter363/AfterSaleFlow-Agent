...
class MtlsExtensionAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        if ca_certificate.fingerprint(hashes.SHA256()).hex() != expected_ca_sha256:
            raise RuntimeError("target E2E mTLS CA fingerprint mismatch")
        constraints = ca_certificate.extensions.get_extension_for_oid(
            ExtensionOID.BASIC_CONSTRAINTS
        ).value
        if not constraints.ca:
            raise RuntimeError("target E2E mTLS trust anchor is not a CA")
        self._app = app
    ...
    def _verified_certificate_der(self, escaped_pem: bytes) -> bytes:
        ...
        if certificate.issuer != self._ca_certificate.subject:
            raise ValueError("mTLS client issuer is not the target E2E CA")
        if (
...
def create_app() -> MtlsExtensionAdapter:
    ca_path = Path(_required_env("TARGET_E2E_MTLS_CA_CERTIFICATE_PATH"))
    if not ca_path.is_absolute() or ca_path.is_symlink() or not ca_path.is_file():
        raise RuntimeError("target E2E mTLS CA path must be an absolute regular file")
    ca_certificate = x509.load_pem_x509_certificate(ca_path.read_bytes())
    ca_sha256 = _required_env("TARGET_E2E_MTLS_CA_CERT_SHA256")
    client_sha256 = _required_env("TARGET_E2E_MTLS_CLIENT_CERT_SHA256")
    if any(
        len(value) != 64
        or any(character not in "0123456789abcdef" for character in value)
        for value in (ca_sha256, client_sha256)
    ):
        raise RuntimeError("target E2E mTLS fingerprints must be lowercase SHA-256")
