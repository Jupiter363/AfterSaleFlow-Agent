...
### P4-R1.5 Authority-Binding Contract Gate
...
[`phase-4-r15-authority-binding-contract.yaml`](./phase-4-r15-authority-binding-contract.yaml) and
closed [manifest schema](./phase-4-r15-authority-binding-contract.schema.json). The formal wire
schemas are
[`intake-human-input-command.v1`](../contracts/agent-platform/intake/v2/intake-human-input-command.schema.json),
[`intake-branch-command.v1`](../contracts/agent-platform/intake/v2/intake-branch-command.schema.json),
and
[`intake-command-payload-put-receipt.v1`](../contracts/agent-platform/intake/v2/intake-command-payload-put-receipt.schema.json).
...
The expand-only migration identity is `V043_1__intake_authority_bindings.sql`, after
`V043__intake_graph_bindings.sql`. V044 remains reserved for the Phase 6 hearing Temporal
projection and V045 remains reserved for outcome execution compensation.

The party authority directly binds the full access-session tenant/case/actor/role/permission tuple
and the existing V019 AgentSession tenant/case/room/access/actor/role/key/profile/memory tuple.
V043_1 does not add `prompt_version` or `agent_session_profile_version` to V019; party rows keep
those pins locally and prove them through the immutable registry. Mutable `ACTIVE` status and
`REGISTERED` status are a same-transaction acceptance/start check, not foreign-key columns.
AgentSession profile identity is the fixed `agent-session-profile.v1` RFC 8785 hash registry ID
...
use the compact V043 key `(binding_id, thread_registration_id)` plus a V043_1 deferrable constraint
trigger that compares every private route and artifact column. The trigger relies on immutable
functional dependencies and the compact FK's concurrent-insert visibility; `object_uri` is never
placed in a primary, unique, or candidate key. New human input requires immutable put-before-DB
provenance plus idempotent orphan cleanup; Java branch payloads are canonical and bounded. Command
authority is one-to-one with payload authority. `CaseCommandRef.payloadRef` compares only
schema/URI/hash/size; artifact ID and object version require separate provenance. The payload table
CHECK rejects mixed EVENT/put-receipt row shapes and applies source-specific byte bounds. Both
server-minted kinds use a 72-character deterministic put key; same-key/same-hash retry returns the
same receipt, while a different hash conflicts. Epoch binding checks both sessions `ACTIVE` and the
registration `REGISTERED`, asserts exactly the two party enum rows, and only then exposes the
bootstrap outbox. The authority row persists the exact wire receipt fields, including
`schema_version`, `put_idempotency_key`, `command_id`, `payload_schema_version`, and
`stored_at_epoch_micros`, and recomputes its RFC 8785 self-hash on reads. Epoch-microsecond integers
are bounded by `2^53-1` for cross-language JCS stability. Command authority binds `case_command`
through request hash as well as identity, persists sequence/type, and compares the four payload-ref
fields transactionally without placing the long URI in a composite index.

Bootstrap and command acceptance acquire `FOR SHARE` locks in fixed table order: access session,
Agent Session, then registration. Revocation/retirement writers acquire `FOR UPDATE` in the same
order. Within each table rows are ordered by access `id ASC`, Agent Session `id ASC`, and
registration `registration_id ASC`; status checks and revocation updates occur only after all locks
are held. Revoke-first rejects acceptance after recheck, while accept-first leaves a committed inert
snapshot replayable without reopening authorization.

The contract gate is satisfied when the manifest, closed schema, wire schemas/fixtures, static gate,
and independent P0/P1 review are committed. It authorizes implementation only; it does not claim
that V043_1, a JDBC read port, worker wiring, or replay compatibility exists.

A separate `P4-R1.5-IMPLEMENTATION-GATE` closes only after the five implementation owners merge
the migration, epoch/party authority, payload/command authority, read-only bridge, and worker
compatibility slices onto one clean SHA. That SHA must pass focused PostgreSQL, transaction/race,
`REPEATABLE_READ`, replay/context, and no-formal-sink checks plus independent P0/P1 review. Until
that second gate passes, D1, E1, and the Phase 4 candidate checkpoint remain blocked. Until P4-E1
separately admits an authenticated synthetic Activity driver, `executionContext` remains null.
Real shadow, `TEMPORAL` allocation, formal Activities/Finalizer registration, canary, and promotion
remain forbidden.
...
must enumerate exact owned files before work starts. Shared files, public contracts, and final
`TemporalWorkerConfiguration` assembly remain primary-controlled. For R1.5 only, D owns the exact
party-aware bridge adapter file and E owns a new worker-registration descriptor; neither may edit
the primary-owned final worker configuration. V043 remains owner C's prior migration, while owner A
alone creates the additive `V043_1__intake_authority_bindings.sql`. An owner submits an interface
request rather than editing across another owner's boundary. No owner stages unrelated changes.
...
### R1.5 Implementation Subwaves

The committed `P4-R1.5` contract gate starts two bounded implementation subwaves without changing
the one-primary/five-owner topology:

- Subwave A runs owners A, B, and D concurrently. A owns only V043_1 and its migration test; B owns
  epoch/party authority and the shared lock protocol; D owns the read-only bridge implementation and
  exact party-aware adapter.
- Subwave B reuses the released slots for C and E. C owns payload/put-receipt/command authority and
  its transaction tests; E owns worker registration, context smoke, replay compatibility, and
  no-formal-sink integration.
- Delegated owners run only T0 checks. R grants the single Maven/PostgreSQL/Testcontainers token,
  integrates in A/B/C/D/E dependency order, runs one deduplicated R1.5 batch, and records
  `P4-R1.5-IMPLEMENTATION-GATE` only after independent P0/P1 review.

Contract-gate completion and implementation-gate completion are separate states. Neither state
changes runtime defaults or authorizes real traffic.
