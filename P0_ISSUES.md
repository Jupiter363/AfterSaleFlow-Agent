...
- Severity: P0
- Status: FIXED / FOCUSED_VERIFIED / DEPLOY_PENDING / LIVE_DATA_CLEANED
...
- Root cause and evidence: The pre-V086 function deleted `agent_execution_manifest` before post-V040 Target E2E finalization receipts, violating `target_e2e_finalization_receipt.fk_target_e2e_finalization_manifest`. V086 corrected the dependency and immutable-trigger graph, but its no-`case_id` event-slot resolver used the child column name `thread_registration_id` against parent `case_intake_graph_thread_binding`, whose primary key is named `registration_id`; the first live purge therefore failed at that resolver query and rolled back. The isolated fixture had no event-slot row and did not execute that lookup branch.
...
- Identifying metadata: observed 2026-08-25 13:08 CST; requested cases `CASE_P9_6A8D1C10_1` and `CASE_P9_6A8D210C_1`; initial failing request `REQ_PURGE_CASE_P9_6A8D1C10_1`, trace `TRACE_622911e067e75edcfbb2a83f50e960e0`, constraint `fk_target_e2e_finalization_manifest`; V086 isolated and independent tests passed 4/4; first deployed retry used request `REQ_PURGE_FAILED_UAT_20260825`, trace `TRACE_397cf7ef07f86814af9e353283844bce`, and failed in `demo_case_purge_row_case_id` while resolving `case_intake_event_slot_authority.thread_registration_id` to the parent registration; V087 independent verification passed 4/4 through all 96 migrations; the two failed UAT cases were subsequently physically deleted in one transaction with audit IDs `PURGE_4B2B43DC371466BBE5227304947D79EA` and `PURGE_4B04615194506A781633DC7194275AC3`, with zero case-owned residual rows and retained global case-ID anti-reuse authority.
...
- Severity: P0
- Status: OPEN / ROOT_MECHANISMS_FIXED / UAT_PENDING
...
- Severity: P0
- Status: OPEN / RUNTIME_CONFIRMED / DIAGNOSIS_IN_PROGRESS
...
- Root cause evidence: The parallel gateway creates a new in-memory `ProgressTracker` at `lastSequenceNo=-1/publicOutputEmitted=false` for every Temporal Activity invocation and does not seed or refresh it from the persisted V4 attempt. Frame ingress commits the V4 event and monotonic attempt progress before invoking that tracker. The first Activity for this run started with persisted `public_output_emitted=false`; later retries started with persisted `public_output_emitted=true`, while each new gateway tracker still began empty. A commit-to-callback failure window or Activity replay can therefore classify a failure after durable public output as `INTAKE_PARALLEL_EXECUTION_UNCLASSIFIED` before the retry budget later freezes it as non-retryable.
...
- Impact: The affected Intake hard gate still cannot reach exact-three READY, FINAL, RESULT_READY, or a formal Intake reply on a fresh deployment, although the previously confirmed `USAGE` payload/frame-id rejection is no longer the first observable failure.
