-- Reviewer-authorized, exact-thread cleanup for Target-E2E test Graph data.
--
...
create table agent_graph_target_e2e_purge_receipt (
    ...
    foreign key (activation_id)
        references agent_graph_target_e2e_activation(activation_id) on delete restrict,
    constraint ck_graph_target_e2e_purge_receipt_id
        check (purge_receipt_id ~ '^GRAPH_PURGE_[A-Z0-9]{16,64}$'),
    constraint ck_graph_target_e2e_purge_java_audit
        check (java_purge_audit_id ~ '^PURGE_[A-Z0-9]{16,64}$'),
    constraint ck_graph_target_e2e_purge_request
        check (purge_request_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$'),
    constraint ck_graph_target_e2e_purge_case
        check (case_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,63}$'),
    constraint ck_graph_target_e2e_purge_thread
        check (thread_id ~ '^grt\.v1\.[0-9a-f]{32}$'),
    constraint ck_graph_target_e2e_purge_room
        check (room_type in ('INTAKE', 'EVIDENCE', 'HEARING', 'REVIEW') and room_epoch >= 0),
    constraint ck_graph_target_e2e_purge_lifecycle
        check (source_thread_lifecycle in ('ACTIVE', 'RETIRED', 'CANCELLED')),
    constraint ck_graph_target_e2e_purge_activation
        check (
            activation_id ~ '^p9act\.v1\.[0-9a-f]{32}$'
            and environment_generation >= 1
        ),
    constraint ck_graph_target_e2e_purge_reviewer
        check (
            reviewer_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$'
            and reviewer_role = 'PLATFORM_REVIEWER'
        ),
    constraint ck_graph_target_e2e_purge_counts
        check (
...
create function reject_agent_graph_target_e2e_purge_receipt_mutation()
returns trigger
language plpgsql
as $function$
begin
    raise exception using errcode = '23514',
        message = 'target-E2E Graph purge receipts are append-only';
end;
$function$;

create trigger trg_reject_agent_graph_target_e2e_purge_receipt_update
before update or delete on agent_graph_target_e2e_purge_receipt
for each row execute function reject_agent_graph_target_e2e_purge_receipt_mutation();

create trigger trg_reject_agent_graph_target_e2e_purge_receipt_truncate
before truncate on agent_graph_target_e2e_purge_receipt
for each statement execute function reject_agent_graph_target_e2e_purge_receipt_mutation();

create function graph_target_e2e_purge_context_allows(target_thread_id varchar)
returns boolean
language sql
stable
security definer
...
as $function$
    select exists (
        select 1
          from agent_graph_target_e2e_purge_receipt receipt
...
as $function$
begin
    if graph_target_e2e_purge_context_allows(old.thread_id) then
        return old;
    end if;
...
as $function$
begin
    if tg_op = 'DELETE' and graph_target_e2e_purge_context_allows(old.thread_id) then
        return old;
    end if;
    ...
    if tg_op = 'DELETE' and exists (
        select 1
         ...
         where permit.request_id = old.request_id
           and graph_target_e2e_purge_context_allows(permit.thread_id)
    ) then
        return old;
    end if;
...
create function purge_target_e2e_test_graph_thread(
...
declare
    ...
    selected_receipt agent_graph_target_e2e_purge_receipt%rowtype;
...
begin
    ...
    then
        raise exception using errcode = '23514',
            message = 'target-E2E Graph purge authority is invalid';
    end if;
    ...
    if found then
        if row(
            ...
            raise exception using errcode = '23514',
                message = 'target-E2E Graph purge replay conflicts with the retained receipt';
        end if;
    ...
    if not found or selected_thread.case_id <> target_case_id then
        raise exception using errcode = '23514',
            message = 'target-E2E Graph purge thread binding is missing or foreign';
    end if;
    ...
    if not exists (
        ...
        select 1
          from agent_graph_target_e2e_room_authority authority
        ...
        raise exception using errcode = '23514',
            message = 'target-E2E Graph purge activation authority is missing or foreign';
    end if;
    ...
    if not found or exists (
        select 1
         ...
         where command.thread_id = target_thread_id
           and (
               command.execution_mode <> 'TARGET_E2E_CANDIDATE'
               or command.activation_id is distinct from target_activation_id
               or command.status not in ('COMPLETED', 'CANCELLED', 'ABORTED')
           )
    ) then
        raise exception using errcode = '23514',
            message = 'target-E2E Graph purge requires only terminal candidate commands';
    end if;
    ...
    if exists (
        ...
        raise exception using errcode = '23514',
            message = 'target-E2E Graph purge requires only terminal attempts';
    end if;
    ...
    if exists (
        ...
        raise exception using errcode = '23514',
            message = 'target-E2E Graph purge cannot remove active execution authority';
    end if;
    ...
    if selected_counts is distinct from target_expected_row_counts then
        raise exception using errcode = '23514',
            message = 'target-E2E Graph purge row-count authority drifted';
    end if;

    insert into agent_graph_target_e2e_purge_receipt (
    ...
    while exists (
        ...
        if deleted_rows = 0 then
            raise exception using errcode = '23514',
                message = 'target-E2E Graph purge cycle lineage is not acyclic';
        end if;
    end loop;
    ...
    while exists (
        ...
        if deleted_rows = 0 then
            raise exception using errcode = '23514',
                message = 'target-E2E Graph purge execution lineage is not acyclic';
        end if;
    end loop;
    ...
    if exists (
        ...
        raise exception using errcode = '23514',
            message = 'target-E2E Graph purge did not remove the exact thread closure';
    end if;
