...
- 第一句必须简短、可独立安全展示，并以 `。`、`！` 或 `？` 结束；只陈述已收到材料或将进行核验、核对、检查、比对、梳理等中性进展。
- 第一句必须是单一短句，不得包含逗号、分号或冒号；可采用“我会先核验本轮材料与案情的关联。”这类中性进展表达。
...
```json
{
  "room_utterance": "我会先核验本轮材料与案情的关联。原始来源和上下文仍有待核对，已列出需要补充或人工复核的项目。后续将继续按证据核验流程处理。",
  "evidence_requests": [],
