"""Target-E2E room adapters over immutable object references.
...
class TargetE2EImmutableObjectStore(Protocol):
...
class TargetE2EImmutableObjectStoreFactory(Protocol):
...
class TargetE2EHearingInvocationDecoder(Protocol):
...
class TargetE2EIntakeExchange(Protocol):
    """Target-only exchange with all-room authority, never SHADOW's ``intake.v2`` DTO."""
...
class TargetE2ESpecializedRoomDependencies:
    ...
    def store_for_execution(self, execution: GatewayExecution) -> TargetE2EImmutableObjectStore:
        ...
        if not callable(getattr(store, "load", None)) or not callable(
            ...
            raise GraphContractError("TARGET_E2E_SCOPED_OBJECT_STORE_REQUIRED")
        return store


class TargetE2EIntakeProvider:
    ...
    def __init__(
        ...
        *,
        ...
        provider: str,
        model: str,
        exchange: TargetE2EIntakeExchange,
    ) -> None:
        if not callable(getattr(exchange, "load", None)) or not callable(
            getattr(exchange, "put", None)
        ):
            raise GraphContractError("TARGET_E2E_INTAKE_EXCHANGE_REQUIRED")
...
def build_target_e2e_intake_provider(
    *,
    saver: FencedPostgresSaver,
    transport: Any,
    provider: str,
    ...
    return TargetE2EIntakeProvider(
        saver=saver,
        transport=transport,
        provider=provider,
        model=model,
...
def _require_target_intake_command(execution: GatewayExecution) -> None:
    ...
    if (
        ...
        raise GraphContractError("TARGET_E2E_INTAKE_OUTER_BINDING_REQUIRED")
...
def _project_target_intake_execution(execution: GatewayExecution) -> GatewayExecution:
    ...
    if not isinstance(record, ThreadRecord) or record.identity != execution.admission.thread:
        raise GraphContractError("TARGET_E2E_INTAKE_THREAD_RECORD_REQUIRED")
...
class TargetE2EObjectEvidenceManifestLoader(TargetEvidenceManifestLoader):
    def __init__(self, store: TargetE2EImmutableObjectStore) -> None:
        self._store = store
...
class TargetE2EObjectEvidenceAssetLoader(TargetEvidenceAssetLoader):
    ...
    async def load(self, item: Mapping[str, Any]) -> TargetEvidenceAsset:
        ...
        if document.get("schema_version") != "target-e2e-evidence-asset.v1":
...
class TargetE2EObjectEvidenceProposalStore(TargetEvidenceProposalStore):
    def __init__(self, store: TargetE2EImmutableObjectStore) -> None:
        self._store = store
...
class TargetE2EObjectHearingInvocationProvider(HearingTargetE2EInvocationProvider):
    def __init__(
        self,
        store: TargetE2EImmutableObjectStore,
        decoder: TargetE2EHearingInvocationDecoder,
    ) -> None:
        self._store = store
        self._decoder = decoder

    async def load(self, execution: GatewayExecution) -> HearingTargetE2ELoadedInvocation:
        command = execution.admission.command
        ...
        if not isinstance(loaded, HearingTargetE2ELoadedInvocation):
            raise GraphContractError("TARGET_E2E_HEARING_INVOCATION_DOCUMENT_INVALID")
        return loaded


class TargetE2EObjectHearingProposalStore(HearingTargetE2EPayloadStore):
    def __init__(self, store: TargetE2EImmutableObjectStore) -> None:
        self._store = store

    async def put(self, **kwargs: Any) -> HearingTargetE2EStoredPayload:
        ...
        if not (
            ...
            raise GraphContractError("TARGET_E2E_HEARING_PROPOSAL_STORE_INVALID")
...
class TargetE2EObjectOutcomeInvocationProvider(OutcomeTargetE2EInvocationProvider):
    """Decode a private REVIEW invocation from the immutable snapshot object."""

    def __init__(self, store: TargetE2EImmutableObjectStore) -> None:
        self._store = store

    async def load(self, execution: GatewayExecution) -> LoadedOutcomeTargetE2EInvocation:
        command = execution.admission.command
        payload = await _load_exact(self._store, command.domain_snapshot_ref)
        document = _object_document(payload, "TARGET_E2E_REVIEW_INVOCATION_DOCUMENT_INVALID")
        if document.get("schema_version") != "target-e2e-review-invocation.v1":
            raise OutcomeReviewContractError("TARGET_E2E_REVIEW_INVOCATION_DOCUMENT_REQUIRED")
        try:
            ...
            raise OutcomeReviewContractError(
                "TARGET_E2E_REVIEW_INVOCATION_DOCUMENT_INVALID"
            ) from error
        event = command.event_ref
        return deterministic_outcome_target_e2e_invocation(
...
class TargetE2EObjectOutcomeProposalStore(OutcomeTargetE2EProposalStore):
    def __init__(self, store: TargetE2EImmutableObjectStore) -> None:
        self._store = store
    ...
    async def put(
        ...
        *,
        ...
        return StoredOutcomeTargetE2EProposal(
...
class TargetE2EOutcomeGraphProvider:
    """Stream the compiled REVIEW graph and bind its proposal source at terminal commit."""

    room_type = RoomType.REVIEW

    def __init__(self, *, saver: FencedPostgresSaver, store: TargetE2EImmutableObjectStore) -> None:
        self._saver = saver
        self._executor = CompiledOutcomeTargetE2EExecutor(
            saver=saver,
            invocation_provider=TargetE2EObjectOutcomeInvocationProvider(store),
            proposal_store=TargetE2EObjectOutcomeProposalStore(store),
        )

    def stream(self, execution: GatewayExecution) -> AsyncIterator[AgentStreamEvent]:
        async def events() -> AsyncIterator[AgentStreamEvent]:
            command = execution.admission.command
            yield _event(execution, 0, "attempt_started", "outcome_target_e2e")
            material = await self._executor.execute(execution)
            source = TargetE2ERoomProposalSource.model_validate(
...
class _ExecutionScopedHearingProvider:
    ...
    def stream(self, execution: GatewayExecution) -> AsyncIterator[AgentStreamEvent]:
        store = self._dependencies.store_for_execution(execution)
        return build_target_e2e_hearing_provider(
            checkpointer=self._saver,
            invocation_provider=TargetE2EObjectHearingInvocationProvider(
                store,
                self._dependencies.hearing_decoder,
            ),
            payload_store=TargetE2EObjectHearingProposalStore(store),
        ).stream(execution)
...
class _ExecutionScopedOutcomeProvider:
    ...
    def stream(self, execution: GatewayExecution) -> AsyncIterator[AgentStreamEvent]:
        return TargetE2EOutcomeGraphProvider(
            saver=self._saver,
            store=self._dependencies.store_for_execution(execution),
        ).stream(execution)


def build_target_e2e_specialized_room_providers(
    *,
    saver: FencedPostgresSaver,
    bulkhead: Any,
    dependencies: TargetE2ESpecializedRoomDependencies,
) -> tuple[TargetE2ERoomProvider, TargetE2ERoomProvider, TargetE2ERoomProvider]:
...
async def _load_exact(
    ...
    *,
    ...
    if not isinstance(payload, bytes) or not payload:
        raise GraphContractError("TARGET_E2E_OBJECT_LOAD_INVALID")
    if len(payload) > reference.size_bytes or (
        ...
        raise GraphContractError("TARGET_E2E_OBJECT_LOAD_BINDING_MISMATCH")
    return payload
...
__all__ = [
    "TargetE2EImmutableObjectStore",
    "TargetE2EHearingInvocationDecoder",
    "TargetE2EObjectEvidenceAssetLoader",
    "TargetE2EObjectEvidenceManifestLoader",
    "TargetE2EObjectEvidenceProposalStore",
    "TargetE2EObjectHearingInvocationProvider",
    "TargetE2EObjectHearingProposalStore",
    "TargetE2EObjectOutcomeInvocationProvider",
    "TargetE2EObjectOutcomeProposalStore",
    "TargetE2EOutcomeGraphProvider",
    "TargetE2ESpecializedRoomDependencies",
    "TargetE2EImmutableObjectStoreFactory",
