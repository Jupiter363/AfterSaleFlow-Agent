...
- Phase 4-8 task partitioning uses one primary plus five logical delegated implementation owners. When the runtime permits only four simultaneously active agents total, execute three child owners in Wave A and reuse the released slots for the remaining two in Wave B; do not collapse ownership to three roles or use delegated owners only for review.
