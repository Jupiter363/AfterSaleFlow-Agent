...
小法庭使用 `hearing_flow.v2` 的 15 个持久化阶段；仅双方回答和双方补证阶段开放提交，并各自共享一个 20 分钟绝对截止时间。庭审期间的材料补充不得重置三小时时钟，也不得把主流程改回补证循环。

前三轮 AI 主审官只输出争点归纳、证据解释区、风险信号和下一轮问题；第三轮结束或三小时到期时，Workflow 必须强制生成确定的非最终裁决执行方案草案并继续后续链路。`FACTS_SUFFICIENT` 仅保留为内部置信度信号，不再提前终止三轮陈述。双方确认同一和解版本时，可将一致方案作为草案提前进入审核。
...
AI 评审团只在最终裁决方案草案生成后介入，不在每轮陈述后介入。高风险、低置信度、重大证据冲突、规则不确定或策略要求时触发最终评审团；评分阈值为 80/100，最多要求 AI 主审官重生成 2 次。跳过评审团不能跳过平台人审。
