...
```text
设计一个活泼但可信的小法庭舞台。书记官先陈列双方证据，AI 主审官主持固定三轮陈述；每轮双方提交或 5 分钟到期后自动封存。前三轮只归纳争点、证据解释和下一轮问题，第三轮后生成确定的裁决执行方案草案。顶部显示三小时倒计时，侧面显示三轮陈述轨迹和双方席位；中央有和解桌，双方确认同一方案后点亮。AI 评审团只在最终方案后从评议席出现，低于 80 分要求法官最多重生成 2 次。所有 AI 结论标注非最终，人类最终落槌。
```
