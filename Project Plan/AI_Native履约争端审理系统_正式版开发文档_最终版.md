...
```text
max_hearing_rounds 固定为 3。
每轮陈述设置 20 分钟 deadline。
双方均提交本轮或本轮到期后自动封存本轮。
前三轮只沉淀争点、证据解释区、风险信号和下一轮问题。
第三轮结束或 PT3H 到期后必须生成确定的非最终裁决执行方案草案。
庭审期间材料补充只更新 EvidenceDossier 版本，不重置庭审时钟，也不新增补证主循环。
```
...
`factsSufficient` 只能作为 AI 主审官内部置信度信号，不再提前终止三轮陈述。
...
小法庭采用固定三轮陈述机制，而不是“事实充分即提前结束”的补证循环：

```text
第 1 轮：双方基于已封存卷宗陈述事实和诉求；
第 2 轮：围绕证据冲突、缺口和责任归属解释；
第 3 轮：补齐最后立场，AI 主审官准备最终裁决方案；
第三轮结束、PT3H 到期或双方确认同一和解版本后，必须进入裁决草案收敛。
```

每轮双方各有一个“提交本轮”动作。双方都提交，或本轮 20 分钟到期后，系统自动封存本轮陈述并推动 AI 主审官处理。`factsSufficient` 只能作为内部置信度和提示词信号，不再作为终止三轮庭审的条件。

AI 主审官在前三轮只输出争点归纳、证据解释区、风险信号和下一轮问题；第三轮后必须生成确定的裁决执行方案草案，不得再要求进入下一轮补证。

评审团触发策略仍由确定性 Java Policy 控制，但触发点统一放在最终方案之后：
...
评审团读取最终方案、双方证据区、双方证据解释区、AI 主审官推理与风险信号，按 100 分制评分。阈值为 80 分；低于阈值时引导 AI 主审官最多重生成 2 次，仍不通过则进入高优先级平台人审。
