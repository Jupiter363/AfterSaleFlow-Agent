...
def validate_scenario_receipt(
    ...
    *,
    ...
    if (
        ...
        raise common.TargetE2EError("Batch 4 scenario receipt did not prove every hard gate")
...
def create_evidence(env_file: Path, case_id: str) -> dict[str, Any]:
    ...
    projection = common.load_json(Path(env["TARGET_E2E_RUN_CONTEXT_PATH"]))[
        "runtime_projection"
    ]
    payload_types = {record["payload_type"] for record in records}
    if not REQUIRED_LEDGER_PAYLOADS.issubset(payload_types):
        raise common.TargetE2EError("P9.0 evidence ledger is incomplete")
    if (
        assertion.get("schema_version") != "target-e2e-assertion-result.v2"
        ...
        raise common.TargetE2EError("target assertion cannot support P9.0 evidence")
    if (
        forensic.get("schema_version") != "target-e2e-forensic-manifest.v2"
        ...
        raise common.TargetE2EError("forensic manifest cannot support P9.0 evidence")
    expected_scenario = (
        "target-e2e-batch-4-scenario.v1",
        "PASS",
        lock["run_id"],
    ...
    if actual_scenario != expected_scenario:
        raise common.TargetE2EError("Batch 4 scenario receipt binding is invalid")
    if scenario.get("engineering_checkpoint") != P9_ENGINEERING_RESULT:
        raise common.TargetE2EError("Batch 4 did not close the engineering checkpoint")
    if any(scenario.get(key) != value for key, value in P9_EXTERNAL_CEILING.items()):
        raise common.TargetE2EError("Batch 4 receipt exceeds the P9.0 authority ceiling")
...
def accept_evidence(
    ...
    *,
    ...
    if (
        ...
        raise common.TargetE2EError("P9.0 evidence is not acceptance eligible")
    if common.TOKEN.fullmatch(acceptance_key_id) is None:
        raise common.TargetE2EError("P9.0 acceptance key ID is invalid")
    ...
    if fingerprint == lock["ledger_public_key_sha256"]:
        raise common.TargetE2EError("P9.0 acceptance must use an independent key")
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (common.TargetE2EError, OSError, json.JSONDecodeError, ValueError) as error:
