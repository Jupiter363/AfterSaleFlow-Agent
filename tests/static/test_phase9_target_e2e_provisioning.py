...
def test_image_builder_requires_exact_digest_pinned_base_inventory(tmp_path: Path) -> None:
    ...
    with pytest.raises(common.TargetE2EError, match="exact non-application inventory"):
        build_image_lock.load_base_images(path)

    path.write_text(json.dumps({**base_images, "postgres": "postgres:16-alpine"}), encoding="utf-8")
    with pytest.raises(common.TargetE2EError, match="immutable manifest"):
        build_image_lock.load_base_images(path)
...
def test_batch4_receipt_requires_every_hard_assertion_and_external_gate_ceiling(
    ...
    env = {"TARGET_E2E_RUN_CONTEXT_PATH": str(run_context_path)}
    lock = {"run_id": "p9-run001", "candidate_sha": candidate}
    receipt = common.seal_self_hash(
        {
            "schema_version": "target-e2e-batch-4-scenario.v1",
            "status": "PASS",
            "run_id": lock["run_id"],
    ...
    with pytest.raises(common.TargetE2EError, match="every hard gate"):
        batch4.validate_scenario_receipt(
            failed,
            env=env,
            lock=lock,
...
def test_p9_evidence_entry_cannot_bypass_the_strict_batch4_receipt_gate(
    ...
    monkeypatch.setattr(
        ...
        lambda _env_file: (
            {"TARGET_E2E_EVIDENCE_DIR": str(evidence_dir)},
            {},
            None,
            [],
        ),
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            common.TargetE2EError("strict scenario gate")
        ),
    )

    with pytest.raises(common.TargetE2EError, match="strict scenario gate"):
...
def test_p9_acceptance_requires_an_independent_signer(tmp_path: Path, monkeypatch) -> None:
    ...
    env = {
        "TARGET_E2E_EVIDENCE_DIR": str(evidence_dir),
        "TARGET_E2E_RUN_CONTEXT_PATH": str(run_context_path),
    }
    lock = {
        "run_id": "p9-run001",
        "candidate_sha": "a" * 40,
    ...
    with pytest.raises(common.TargetE2EError, match="independent key"):
    ...
    with pytest.raises(common.TargetE2EError, match="already exists"):
        p9_gate.accept_evidence(
            tmp_path / "target.env",
            acceptance_private_key=acceptance_private,
            acceptance_public_key=acceptance_public,
...
def test_openssl_config_discovery_fails_closed_when_no_config_exists(
    ...
    with pytest.raises(common.TargetE2EError, match="no bounded readable OpenSSL config"):
