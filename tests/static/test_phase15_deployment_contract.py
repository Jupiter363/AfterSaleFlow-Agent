...
def test_frontend_container_serves_a_production_build_as_non_root() -> None:
    ...
    server = read(ROOT / "apps/web" / "server.mjs")
...
def test_ocr_container_preserves_the_base_image_official_debian_sources() -> None:
    dockerfile = read(ROOT / "apps/ocr-parser" / "Dockerfile")
...
def test_smoke_test_checks_health_and_lists_disputes_through_nginx() -> None:
    smoke = read(ROOT / "tools" / "verify" / "smoke-test.sh")
    dev_up = read(ROOT / "tools" / "dev" / "dev-up.sh")
    dev_reset = read(ROOT / "tools" / "dev" / "dev-reset.sh")
    ...
    assert "./tools/verify/smoke-test.sh" in dev_up
    assert "./tools/dev/dev-up.sh" in dev_reset
...
def test_manual_migration_script_uses_the_runtime_jar_and_entrypoint() -> None:
    init_db = read(ROOT / "tools" / "dev" / "init-db.sh")
    migration_main = read(
        ROOT
        / "apps/domain-service"
        / "src"
