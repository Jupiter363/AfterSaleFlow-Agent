 ...
 * {@link ProductionActivationLifecycleStore}. The production-only HTTP adapter authenticates its caller
 * before invoking this contract.
 */
public interface ProductionActivationLifecycleControl {
  ...
  static ProductionActivationLifecycleControl bind(
      ProductionActivationLifecycleStore store, DeploymentBinding binding, Clock clock) {
    Objects.requireNonNull(store, "store");
    Objects.requireNonNull(binding, "binding");
    Objects.requireNonNull(clock, "clock");
    return new ProductionActivationLifecycleControl() {
      @Override
  ...
  record DeploymentBinding(
    ...
    public DeploymentBinding {
      if (!targetProfileActive) {
        throw new IllegalStateException("production runtime lifecycle requires the production-runtime profile");
      }
      if (environmentId == null || !IDENTIFIER.matcher(environmentId).matches()) {
        throw new IllegalArgumentException("production runtime lifecycle environment binding is invalid");
      }
      if (environmentGeneration < 1
          || environmentGeneration > 9_007_199_254_740_991L) {
        throw new IllegalArgumentException("production runtime lifecycle generation binding is invalid");
      }
      if (activationId == null || !ACTIVATION_ID.matcher(activationId).matches()) {
        throw new IllegalArgumentException("production runtime lifecycle activation binding is invalid");
      }
      if (manifestHash == null || !manifestHash.matches("[0-9a-f]{64}")) {
        throw new IllegalArgumentException("production runtime lifecycle manifest binding is invalid");
      }
      if (runtimeContextHash == null || !runtimeContextHash.matches("[0-9a-f]{64}")) {
        throw new IllegalArgumentException("production runtime lifecycle runtime context binding is invalid");
      }
    }
    ...
    void requireExact(ActivationIdentity identity) {
      ...
      if (environmentGeneration != identity.environmentGeneration()
        ...
        throw new SecurityException("production runtime lifecycle binding mismatch");
      }
    }
