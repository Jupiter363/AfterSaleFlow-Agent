...
/** Private content-addressed MinIO adapter for production-runtime proposal metadata and bytes. */
public final class MinioProductionIntakeProposalStore implements ProductionIntakeProposalStore {

    private final MinioClient minio;
    private final String bucket;
    private final String prefix;

    public MinioProductionIntakeProposalStore(
    ...
    public StoredProposal readExact(IntakeProposalReference reference) {
        ...
        if (!reference.sha256().equals(reference.objectVersion())) {
            throw rejected(
                    ...
                    "production-runtime proposal version must equal its content address");
        }
        byte[] payload;
    ...
    private static String requireBucket(String value) {
        if (value == null || !value.matches("[a-z0-9][a-z0-9.-]{1,61}[a-z0-9]")) {
            throw new IllegalArgumentException("production-runtime proposal bucket is invalid");
        }
        return value;
    }
    ...
    private static String requirePrefix(String value) {
        if (value == null
            ...
            throw new IllegalArgumentException("production-runtime proposal prefix is invalid");
        }
        return value;
    }
