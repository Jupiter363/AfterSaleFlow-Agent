...
import com.example.dispute.workflow.targete2e.finalization.TargetE2eFinalizationReceipt.DomainCommitStatus;
import com.example.dispute.workflow.targete2e.finalization.TargetE2eFinalizationReceipt.FormalWriter;
...
public final class JdbcTargetE2eFinalizationReceiptLedger
        implements TargetE2eFinalizationReceiptLedger {
    ...
    public JdbcTargetE2eFinalizationReceiptLedger(DataSource dataSource) {
    ...
    public StoredReceipt append(AppendCommand command) {
        ...
        try {
            ...
            TargetE2eFinalizationReceiptLedger.requireExact(persisted, command);
            return persisted;
        } catch (SQLException failure) {
    ...
    private static void insert(Connection connection, AppendCommand command) throws SQLException {
        TargetE2eFinalizationReceipt receipt = command.receipt();
        byte[] canonicalBytes = TargetE2eFinalizationReceiptCodec.canonicalBytes(receipt);
    ...
    private static StoredReceipt find(
        ...
        try (PreparedStatement statement = connection.prepareStatement(SELECT)) {
            ...
            try (ResultSet result = statement.executeQuery()) {
                ...
                TargetE2eFinalizationReceipt decoded =
                        TargetE2eFinalizationReceiptCodec.decodeCanonical(canonicalBytes);
                TargetE2eFinalizationReceipt columns = receipt(result);
    ...
    private static TargetE2eFinalizationReceipt receipt(ResultSet result) throws SQLException {
        return new TargetE2eFinalizationReceipt(
    ...
    private static String receiptId(TargetE2eFinalizationReceipt receipt) {
    ...
    private static TargetE2eFinalizationRejectedException conflict(String message) {
        return new TargetE2eFinalizationRejectedException(
                "TARGET_E2E_FINALIZATION_RECEIPT_CONFLICT", message);
    }

    private static TargetE2eFinalizationReceiptPersistenceException persistence(
            String message, SQLException failure) {
        String state = failure.getSQLState();
        boolean retryable = state != null && (state.startsWith("08") || state.startsWith("40"));
        return new TargetE2eFinalizationReceiptPersistenceException(
