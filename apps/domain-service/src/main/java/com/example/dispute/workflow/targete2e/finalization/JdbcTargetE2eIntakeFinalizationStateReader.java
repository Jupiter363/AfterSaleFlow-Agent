...
public final class JdbcTargetE2eIntakeFinalizationStateReader
        implements TargetE2eIntakeFinalizationStateReader {
    ...
    public JdbcTargetE2eIntakeFinalizationStateReader(
    ...
    @Override
    public Optional<TargetE2eIntakeFinalizationState> load(
            ExecuteAgentRunRequest request, ExecuteAgentRunResult result) {
        Objects.requireNonNull(request, "request");
        Objects.requireNonNull(result, "result");
        Optional<TargetE2eIntakeFinalizationState> resolved = transactions.execute(
                ignored -> loadInTransaction(request, result));
        return resolved == null ? Optional.empty() : resolved;
    }

    private Optional<TargetE2eIntakeFinalizationState> loadInTransaction(
            ExecuteAgentRunRequest request, ExecuteAgentRunResult result) {
        ...
        return Optional.of(new TargetE2eIntakeFinalizationState(
                facts.run(),
                facts.attempt(),
                facts.epoch(),
                facts.projection(),
    ...
    private <T> T decode(String value, Class<T> type) {
        ...
        try {
            ...
            throw new TargetE2eFinalizationRejectedException(
                    "TARGET_E2E_PERSISTED_JSON_INVALID",
    ...
    private static <T> Optional<T> unique(List<T> rows, String fact) {
        if (rows.size() > 1) {
            throw new TargetE2eFinalizationRejectedException(
                    "TARGET_E2E_FINALIZATION_FACTS_AMBIGUOUS",
                    fact + " is not unique");
        }
