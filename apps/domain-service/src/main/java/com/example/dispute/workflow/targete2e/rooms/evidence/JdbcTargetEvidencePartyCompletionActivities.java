...
public final class JdbcTargetEvidencePartyCompletionActivities
  ...
  static CompletionAuthority lockCompletionMaterial(Connection c, Request r)
    ...
    try (PreparedStatement s = c.prepareStatement("""
        select material.admission_id, material.material_canonical_json,
          ...
          from target_e2e_evidence_completion_command_material material
         ...
         where material.execution_lane = 'TARGET_E2E_CANDIDATE'
  ...
  private static CompletionIntent lockCompletionIntent(Connection c, Request r, String role)
      throws SQLException {
    if (!"target-e2e-evidence-completion.v1".equals(r.command().payloadRef().schemaVersion())
        || !r.command().payloadRef().uri().startsWith("urn:target-e2e:timeline-event:")) {
      throw new IllegalStateException(
    ...
    String eventId =
        r.command()
            .payloadRef()
            .uri()
            .substring("urn:target-e2e:timeline-event:".length());
    if (eventId.isBlank()) {
      throw new IllegalStateException(
  ...
  static void requireCompletionIntent(
    ...
    if (!request.command().commandId().equals("evidence-complete:" + intent.completionId())
        ...
        || !request.command().payloadRef().uri().equals("urn:target-e2e:timeline-event:" + eventId)
  ...
  private static void completeActivationCommand(
    ...
    try (PreparedStatement select = connection.prepareStatement("""
        select activation_id, command_id, command_hash, command_envelope_hash,
               completion_hash
          from target_e2e_command_completion where admission_id = ? for key share
        """)) {
