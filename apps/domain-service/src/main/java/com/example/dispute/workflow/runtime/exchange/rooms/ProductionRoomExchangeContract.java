...
public final class ProductionRoomExchangeContract {
  ...
  private ProductionRoomExchangeContract() {}
  ...
  public record Authority(String schemaVersion, String activationId, long roomFencingToken,
    ...
    public Authority {
      exact(schemaVersion, "production-runtime-room-exchange-authority.v1", "authority schema");
  ...
  public record LoadRequest(String schemaVersion, Authority authority, ObjectRef objectRef) {
    public LoadRequest { exact(schemaVersion, "production-runtime-room-object-load-request.v1", "load schema"); authority = Objects.requireNonNull(authority); objectRef = Objects.requireNonNull(objectRef); }
  }
  @JsonNaming(PropertyNamingStrategies.SnakeCaseStrategy.class)
  ...
  public record LoadResponse(String schemaVersion, Authority authority, LoadReceipt receipt, String canonicalPayloadBase64) {
    public LoadResponse { exact(schemaVersion, "production-runtime-room-object-load-response.v1", "load response schema"); authority = Objects.requireNonNull(authority); receipt = Objects.requireNonNull(receipt); if (canonicalPayloadBase64 == null || canonicalPayloadBase64.length() > 700000) invalid("payload"); }
  }
  @JsonNaming(PropertyNamingStrategies.SnakeCaseStrategy.class)
  ...
  public record PutRequest(String schemaVersion, Authority authority, Proposal proposal, String checkpointNs, String checkpointId, long cognitiveRevision) {
    public PutRequest { exact(schemaVersion, "production-runtime-room-proposal-put-request.v1", "put schema"); authority = Objects.requireNonNull(authority); proposal = Objects.requireNonNull(proposal); if (checkpointNs == null || checkpointNs.length() > 128 || checkpointId == null || checkpointId.isBlank() || checkpointId.length() > 128 || cognitiveRevision < 1) invalid("checkpoint"); }
  }
  @JsonNaming(PropertyNamingStrategies.SnakeCaseStrategy.class)
  public record ProposalReceipt(String proposalId, String schemaVersion, String sha256, long sizeBytes, String payloadRef) {
    public ProposalReceipt { id(proposalId, "proposalId"); id(schemaVersion, "schemaVersion"); hash(sha256, "sha256"); if (sizeBytes < 1 || sizeBytes > 65536 || payloadRef == null || !payloadRef.matches("urn:production-runtime:proposal:(evidence|hearing|review):[0-9a-f]{64}")) invalid("proposal receipt"); }
  }
  @JsonNaming(PropertyNamingStrategies.SnakeCaseStrategy.class)
  public record PutResponse(String schemaVersion, Authority authority, ProposalReceipt receipt) {
    public PutResponse { exact(schemaVersion, "production-runtime-room-proposal-put-response.v1", "put response schema"); authority = Objects.requireNonNull(authority); receipt = Objects.requireNonNull(receipt); }
  }
  ...
  static void opaqueObjectRef(String value, String schemaVersion) {
    boolean commandObject = value != null && value.matches("urn:production-runtime:object:[A-Za-z0-9._:-]{1,480}");
    boolean verifiedEvidenceParse = "production-runtime-evidence-asset.v1".equals(schemaVersion)
  ...
  static void invalid(String field) { throw new IllegalArgumentException("production runtime room exchange has invalid " + field); }
}
