...
public final class TargetDeterministicEvaluationAgentClient implements EvaluationAgentClient {
  ...
  public EvaluationAgentResult analyze(JsonNode closedCaseSnapshot, String traceId, String requestId) {
    ...
    report.put("schema_version", "target-e2e-deterministic-evaluation.v1");
    ...
    report.put("evaluator_model", "target-e2e-java-deterministic");
    report.put("prompt_version", "target-e2e-v1");
    ...
    return new EvaluationAgentResult(report, "target-e2e-java-deterministic", "target-e2e-v1", 0, 0);
  }
}
