...
import com.example.dispute.workflow.runtime.ingress.TargetIntakeCommandAdmissionReadiness;
...
public final class EpochAwareIntakeBranchIngressRouter {
    ...
    public IntakeIngressSelection select(String caseId) {
        ...
        if (epoch.getLifecycleStatus() != EpochLifecycleStatus.ACTIVE
            ...
            throw invalidState("PRODUCTION_RUNTIME_INTAKE_EPOCH_NOT_READY", "target Intake epoch is not ready", epoch);
        }
        if (authorities.size() != 1 || delegates.size() != 1) {
            throw invalidState(
                    "PRODUCTION_RUNTIME_INTAKE_BRANCH_AUTHORITY_UNAVAILABLE",
