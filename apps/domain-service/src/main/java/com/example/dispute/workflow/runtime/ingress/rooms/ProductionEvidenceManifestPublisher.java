...
public final class ProductionEvidenceManifestPublisher {
  ...
  public ProductionEvidenceManifestPublisher(MinioProductionRoomCommandPayloadPublisher publisher,
      ProductionRoomObjectIndex index, GraphEnvelopeSigningKey signingKey, ObjectMapper mapper) {
  ...
  public Published publish(Input input) {
    ...
    asset.put("schema_version", "production-runtime-evidence-asset.v1"); asset.put("content", "Synthetic browser evidence " + input.commandId());
    ...
    manifest.put("execution_scope", "PRODUCTION"); manifest.put("writer_mode", "PROPOSAL_ONLY"); manifest.put("formal_sink_eligible", false); manifest.put("graph_execution_allowed", true);
  ...
  public void bind(Authority authority, RoomGraphCommand command, Published published) {
    publisher.bind(authority, command, published.manifest(), ProductionRoomObjectIndex.Kind.COMMAND_INPUT);
    publisher.bind(authority, command, published.asset(), ProductionRoomObjectIndex.Kind.MANIFEST_ASSET);
  }
  ...
  public record Published(MinioProductionRoomCommandPayloadPublisher.PublishedObject manifest, MinioProductionRoomCommandPayloadPublisher.PublishedObject asset) {}
