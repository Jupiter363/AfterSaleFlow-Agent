...
public final class ProductionIsolationAttestationVerifier {

  static final String JWS_TYPE = "production-runtime-measurement+jwt";
  static final String SCHEMA_VERSION = "production-runtime-measurement.v1";
  ...
  private final ProductionIsolationAttestationPublicKeySet publicKeys;
  private final Clock clock;
  private final ObjectMapper mapper;

  public ProductionIsolationAttestationVerifier(
      ProductionIsolationAttestationPublicKeySet publicKeys, Clock clock) {
  ...
  VerifiedAttestation verify(
      ...
      ProductionActivationExpectedRuntime measured,
    ...
    ProductionActivationContract.sha256(measuredArtifactDigest, "measuredArtifactDigest");
    if (compactJws == null
        || compactJws.isBlank()
    ...
    String keyId = ProductionActivationContract.keyId(text(header, "kid"));
    if (ProductionActivationContract.same(keyId, challenge.activationKeyId())) {
      throw new IllegalArgumentException(
    ...
    if (ProductionActivationContract.same(
    ...
    String hash =
        ProductionActivationContract.sha256(text(payload, "attestationHash"), "attestationHash");
    ObjectNode hashSource = payload.deepCopy();
    hashSource.remove("attestationHash");
    if (!ProductionActivationContract.same(hash, ContractJson.sha256Hex(hashSource))) {
      throw new IllegalArgumentException("runtime isolation attestation hash is invalid");
    }
    String nonce = ProductionActivationContract.nonce(text(payload, "attestationNonce"));
    if (ProductionActivationContract.same(nonce, challenge.activationNonce())) {
      throw new IllegalArgumentException(
    ...
    if (!SCHEMA_VERSION.equals(text(payload, "schemaVersion"))
        ...
        || !ProductionActivationContract.same(text(payload, "candidateSha"), measured.candidateSha())
        || !ProductionActivationContract.same(
  ...
  private static byte[] decode(String segment, int maximumBytes) {
    ...
    try {
      ...
      if (decoded.length == 0
          ...
          || !ProductionActivationContract.same(segment, ENCODER.encodeToString(decoded))) {
        throw new IllegalArgumentException("runtime isolation attestation encoding is invalid");
      }
      return decoded;
  ...
  private static long integer(ObjectNode parent, String field) {
    ...
    return ProductionActivationContract.generation(value.longValue());
  }
  ...
  record VerifiedAttestation(
    ...
    VerifiedAttestation {
      ProductionActivationContract.keyId(keyId);
      ProductionActivationContract.nonce(nonce);
      ProductionActivationContract.sha256(attestationHash, "attestationHash");
