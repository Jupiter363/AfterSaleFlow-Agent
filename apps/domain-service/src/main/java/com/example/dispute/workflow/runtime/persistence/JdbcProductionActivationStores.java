...
import com.example.dispute.workflow.runtime.DrainAcceptedCommand;
...
public final class JdbcProductionActivationStores
    implements ProductionActivationReplayStore,
        ProductionActivationCaseLedger,
        ProductionActivationLifecycleStore {
  ...
  public JdbcProductionActivationStores(DataSource dataSource, Clock clock) {
    this.dataSource = Objects.requireNonNull(dataSource, "dataSource");
    this.clock = Objects.requireNonNull(clock, "clock");
    this.ledger = new ProductionActivationLedger(dataSource, clock);
  }
  ...
  public RegistrationResult registerOrAttach(Registration registration) {
    ...
    try {
      ProductionActivationLedger.RegistrationDisposition disposition =
          ledger.registerOrAttach(toLedgerRegistration(registration)).disposition();
      return disposition == ProductionActivationLedger.RegistrationDisposition.REGISTERED
          ? RegistrationResult.REGISTERED
          : RegistrationResult.ATTACHED_EXISTING;
    } catch (ProductionPersistenceException failure) {
        ...
        PreparedStatement statement =
            connection.prepareStatement(
                ...
                select activation.activation_id, activation.nonce,
                  ...
                  from production_runtime_activation activation
                  left join production_runtime_environment_generation_watermark watermark
  ...
  private ProductionActivationLedger.ActivationRegistration toLedgerRegistration(
    ...
    ProductionActivationLedger.CaseScope persistedScope;
    if (caseScope instanceof ExplicitCaseIds explicit) {
      persistedScope =
          new ProductionActivationLedger.ExplicitCaseScope(
      ...
      persistedScope =
          new ProductionActivationLedger.SyntheticCaseScope(
              scopeHash,
    ...
    return new ProductionActivationLedger.ActivationRegistration(
        ...
        new ProductionActivationLedger.BuildBindings(
            builds.caseBuildId(), builds.controlBuildId(), builds.agentBuildId()),
        new ProductionActivationLedger.GraphBinding(
            graph.key(),
            graph.version(),
            graph.checkpointSchemaVersion(),
            graph.bindingHash(),
            graph.codeBuildId()),
        new ProductionActivationLedger.ImageDigests(
            images.javaApi(),
  ...
  private static ProductionActivationLedger.DatabaseBinding isolatedDomainDatabaseBinding(
      Registration registration, DatabaseIdentity identity) {
    return new ProductionActivationLedger.DatabaseBinding(
  ...
  private static ProductionActivationLedger.DatabaseBinding databaseBinding(
    ...
    return new ProductionActivationLedger.DatabaseBinding(
        ...
        identity.runtimePrincipalIdentity(),
        hash(
            "production-runtime-database-binding.v1",
            identity.clusterIdentity(),
            identity.databaseIdentity(),
            identity.runtimePrincipalIdentity()));
  }
  ...
  private static void lockReservationKeys(Connection connection, Reservation reservation)
    ...
    List<String> keys =
        List.of(
                "production-runtime-activation:" + reservation.activationId(),
                "production-runtime-case:" + reservation.caseId())
            .stream()
  ...
  private static boolean caseIdClaimed(Connection connection, String caseId) throws SQLException {
    try (PreparedStatement statement =
        connection.prepareStatement("select 1 from production_runtime_case_id_claim where case_id = ?")) {
  ...
  private static int reservationCount(Connection connection, String activationId)
    ...
    try (PreparedStatement statement =
        connection.prepareStatement(
            "select count(*) from production_runtime_case_reservation where activation_id = ?")) {
      statement.setString(1, activationId);
      try (ResultSet result = statement.executeQuery()) {
        result.next();
  ...
  private static void transition(
    ...
    String sql =
        "update production_runtime_activation set lifecycle_status = ?, lifecycle_changed_at = ?, "
            + timestampColumn
  ...
  private static void requireUpdated(PreparedStatement statement, String message)
    ...
    if (statement.executeUpdate() != 1) {
      throw new ProductionPersistenceException("ACTIVATION_STATE_CONFLICT", message);
    }
  }
  ...
  private static String caseScopeHash(CaseScope scope) {
    DigestWriter digest = new DigestWriter("production-runtime-case-scope.v1");
  ...
  private static String bindingSetHash(BindingSnapshot bindings) {
    DigestWriter digest = new DigestWriter("production-runtime-binding-set.v1");
  ...
  private static void addAuthority(DigestWriter digest, MeasuredAuthorityFacts authority) {
    ...
    digest.add(authority.productionActivationDefault());
  }
  ...
  private static ProductionPersistenceException persistenceFailure(
