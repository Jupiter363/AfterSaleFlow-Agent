...
public final class ProductionFinalizationBindingVerifier {

    private static final String LANE = ProductionExecutionLaneVerifier.EXECUTION_LANE;
    ...
    public ProductionFinalizationBindingVerifier(ObjectMapper objectMapper) {
    ...
    public VerifiedEvidence verify(
        ...
        text(commandEnvelope, "schema_version", "production-runtime-graph-command-envelope.v1");
        ...
        text(proposalSource, "schema_version", "production-runtime-room-proposal-source.v2");
        ...
        text(proposal, "schema_version", "production-runtime-intake-proposal.v1");
        ...
        if (payloadRef.length() > 512 || !payloadRef.startsWith("urn:production-runtime:proposal:")) {
            throw rejected("PRODUCTION_RUNTIME_SOURCE_SCHEMA_INVALID", "payload_ref is invalid");
        }
        ...
        if (!formalAuthority.isBoolean() || formalAuthority.booleanValue()) {
            throw rejected("PRODUCTION_RUNTIME_GRAPH_AUTHORITY_INVALID", "proposal grants formal authority");
        }
        ...
        requireEqual(
                payloadRef,
                "urn:production-runtime:proposal:intake:" + artifact.sha256(),
        ...
        text(resultEnvelope, "schema_version", "production-runtime-graph-result-envelope.v1");
        ...
        text(dbBinding, "schema_version", "production-runtime-isolated-domain-db-binding.v1");
    ...
    private static com.example.dispute.workflow.contract.v1.ContractTypes.ArtifactPointer
        ...
        if (result.graphResult() == null
            ...
            throw rejected(
                    "PRODUCTION_RUNTIME_PROPOSAL_REFERENCE_MISSING",
    ...
    private static String activationId(JsonNode document) {
        ...
        if (!value.matches("p9act[.]v1[.][0-9a-f]{32}")) {
            throw rejected("PRODUCTION_RUNTIME_ACTIVATION_ID_INVALID", "activation id is invalid");
        }
        return value;
    }
    ...
    private static String boundedIdentifier(JsonNode document, String field) {
        ...
        if (value.length() > 128 || !value.matches("[A-Za-z0-9][A-Za-z0-9._:/-]{0,127}")) {
            throw rejected("PRODUCTION_RUNTIME_SOURCE_SCHEMA_INVALID", field + " is invalid");
        }
        return value;
    }
    ...
    private static String sha256(JsonNode document, String field) {
        ...
        if (!value.matches("[0-9a-f]{64}")) {
            throw rejected("PRODUCTION_RUNTIME_SOURCE_SCHEMA_INVALID", field + " is not a SHA-256");
        }
        return value;
    }
    ...
    private static String text(JsonNode document, String field) {
        ...
        if (!value.isTextual() || value.textValue().isBlank()) {
            throw rejected("PRODUCTION_RUNTIME_SOURCE_SCHEMA_INVALID", field + " is not text");
        }
    ...
    private static void exactFields(JsonNode document, Set<String> expected, String name) {
        if (!document.isObject()) {
            throw rejected("PRODUCTION_RUNTIME_SOURCE_SCHEMA_INVALID", name + " is not an object");
        }
        ...
        if (!actual.equals(expected)) {
            throw rejected(
                    "PRODUCTION_RUNTIME_SOURCE_SCHEMA_INVALID", name + " fields are not exact");
        }
    }
    ...
    private static void requireEqual(Object actual, Object expected, String field) {
        if (!Objects.equals(actual, expected)) {
            throw rejected(
                    "PRODUCTION_RUNTIME_BINDING_MISMATCH", field + " conflicts with its hash source");
        }
    }
