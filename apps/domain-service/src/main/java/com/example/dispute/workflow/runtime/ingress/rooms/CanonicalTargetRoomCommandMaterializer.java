...
import com.example.dispute.workflow.runtime.rooms.evidence.TargetEvidenceCompletionCommandMaterial;
import com.example.dispute.workflow.runtime.rooms.evidence.TargetEvidenceCompletionCommandMaterialStore;
import com.example.dispute.workflow.runtime.rooms.hearing.TargetHearingCommandMaterial;
import com.example.dispute.workflow.runtime.rooms.hearing.TargetHearingCommandMaterialStore;
import com.example.dispute.workflow.runtime.rooms.review.TargetReviewCommandMaterial;
...
public final class CanonicalTargetRoomCommandMaterializer implements TargetRoomCommandIngress {
    ...
    private static final String OUTPUT_SCHEMA = "production-runtime-room-proposal-source.v2";
    ...
    private final ProductionEvidenceManifestPublisher evidenceManifestPublisher;
    private final ProductionEvidenceTurnInvocationPublisher evidenceTurnInvocationPublisher;
    private final ProductionReviewInvocationPublisher reviewInvocationPublisher;
    ...
    public CanonicalTargetRoomCommandMaterializer(
            ...
            ProductionGraphEnvelopeCodec envelopes,
    ...
    private String materialize(
        ...
        MinioProductionRoomCommandPayloadPublisher.PublishedObject domain;
        JdbcTargetReviewInvocationFactsLoader.Facts loadedReview = null;
        ProductionEvidenceTurnInvocationPublisher.Published evidencePublished = null;
        if (command.roomType() == RoomType.EVIDENCE) {
            RoomGraphCommand provisional = graph(
                    ...
                    new RoomGraphCommand.SnapshotRef(
                            ...
                            ProductionEvidenceTurnInvocationPublisher.SCHEMA_VERSION,
                            "urn:production-runtime:object:target-evidence-turn-invocation:" + commandId,
                            "0".repeat(64),
                            1),
                    eventRef,
                    now);
            ...
            loadedReview = reviewFacts.load(graph(commandId, logicalRunId, attemptId, epoch, command, actor, traceId,
                    new RoomGraphCommand.SnapshotRef("review-invocation:" + commandId, "production-runtime-review-invocation.v1",
                        "urn:production-runtime:object:review-invocation:" + commandId, "0".repeat(64), 1), null, now), epoch.getFencingToken());
            domain = null;
        } else {
        ...
        RoomGraphCommand skeleton = graph(commandId, logicalRunId, attemptId, epoch, command, actor, traceId,
                new RoomGraphCommand.SnapshotRef("review-invocation:" + commandId, "production-runtime-review-invocation.v1",
                    "urn:production-runtime:object:review-invocation:" + commandId, "0".repeat(64), 1), eventRef, now);
        ...
        ProductionGraphCommandEnvelope envelope = envelopes.wrapCommand(grant.activationId(), epoch.getFencingToken(), graph);
        ...
        else if (evidencePublished == null) payloads.bind(exchangeAuthority, graph, domain, ProductionRoomObjectIndex.Kind.COMMAND_INPUT);
    ...
    private void append(AcceptCaseCommand command, CommandAdmission admission, ExecuteAgentRunRequest request,
            ProductionGraphCommandEnvelope envelope, CaseRoomEpochEntity epoch,
    ...
    private JsonNode source(AcceptCaseCommand command, AuthenticatedActor actor, CaseRoomEpochEntity epoch, String objectRole) {
        ...
        value.put("schema_version", "production-runtime-room-command-source.v1");
    ...
    private static Authority authority(String activationId, CaseRoomEpochEntity epoch, RoomGraphCommand graph,
        ...
        return new Authority("production-runtime-room-exchange-authority.v1", activationId, epoch.getFencingToken(),
