...
/**
 * Production-only, read-only recovery port for the Intake AgentRun child.
 *
...
public final class JdbcTargetIntakeAgentRunFinalizationReceiptReadPort
    ...
    private static final String TARGET_RECEIPT_SQL = """
            select receipt.receipt_id, receipt.activation_manifest_hash,
              ...
              from production_runtime_finalization_receipt receipt
              join agent_run run
             ...
             where receipt.activation_id = :activationId
               ...
               and receipt.execution_lane = 'PRODUCTION'
    ...
    private static final String COMPLETION_SQL = """
            select completion_hash
             ...
             where completion.activation_id = :activationId
               ...
               and admission.execution_lane = 'PRODUCTION'
    ...
    private static final String TERMINAL_NO_COMMIT_SQL = """
            select run.id as logical_run_id,
                   ...
                   (select count(*)
                      from production_runtime_finalization_receipt terminal_receipt
              ...
              join agent_run_attempt attempt
                on attempt.agent_run_id = run.id
              left join production_runtime_command_admission admission
               ...
               and admission.room_epoch = :roomEpoch
               and admission.room_fencing_token = :roomFencingToken
              left join production_runtime_intake_command_material material
                on material.admission_id = admission.admission_id
               and material.activation_id = admission.activation_id
               and material.activation_manifest_hash = admission.activation_manifest_hash
    ...
    private final ProductionGraphEnvelopeCodec envelopes;
    ...
    public JdbcTargetIntakeAgentRunFinalizationReceiptReadPort(
        ...
        this.envelopes = new ProductionGraphEnvelopeCodec(objectMapper);
    ...
    public IntakeAgentRunFinalizationReadResult read(IntakeAgentRunFinalizationReadRequest request) {
        ...
        try {
            ...
            if (result == null) {
                throw rejected("PRODUCTION_RUNTIME_FINALIZATION_READ_EMPTY", "receipt read returned no result");
            }
            return result;
            ...
            if (failure instanceof TransientDataAccessException
                ...
                throw new ProductionFinalizationReadPersistenceException(
                        "target Intake finalization read has a retryable database failure", failure);
            }
            throw rejected(
                    "PRODUCTION_RUNTIME_FINALIZATION_READ_PERSISTENCE_INVARIANT",
    ...
    private IntakeAgentRunFinalizationReadResult readInTransaction(
        ...
        if (rows.size() > 1) {
            throw rejected("PRODUCTION_RUNTIME_FINALIZATION_READ_AMBIGUOUS", "target receipt identity is not unique");
        }
        if (rows.isEmpty()) {
        ...
        ProductionFinalizationReceipt receipt = decodeTargetReceipt(row);
    ...
    static String requiredAgentRunProtocol(IntakeAgentRunFinalizationReadRequest request) {
        ...
        throw rejected(
                "PRODUCTION_RUNTIME_FINALIZATION_PROTOCOL_AUTHORITY_INVALID",
        ...
        if (!terminalRows.isEmpty()) {
            ...
            if ("ABORTED".equals(last.runStatus()) || "FAILED".equals(last.runStatus())) {
                if (!"FINALIZATION_REJECTED".equals(last.stopReason())) {
                    throw rejected(
                            "PRODUCTION_RUNTIME_FINALIZATION_TERMINAL_REASON_INVALID",
    ...
    IntakeAgentRunFinalizationReadResult terminalNoCommit(
        ...
        if (rows.size() != terminal.attemptNo()
            ...
            throw rejected(
                    "PRODUCTION_RUNTIME_FINALIZATION_TERMINAL_LINEAGE_INVALID",
        ...
        for (int index = 0; index < rows.size(); index++) {
            ...
            if (row.formalEventCount() != 0) {
                throw rejected(
                        "PRODUCTION_RUNTIME_FINALIZATION_TERMINAL_EVENT_PRESENT",
        ...
        if (rootCommand == null
                ...
                || root.receiptCount() != 0) {
            throw rejected(
                    "PRODUCTION_RUNTIME_FINALIZATION_TERMINAL_AUTHORITY_INVALID",
        ...
        if (!requiredAgentRunProtocol(request).equals(terminal.runProtocol())
            ...
            throw rejected(
                    "PRODUCTION_RUNTIME_FINALIZATION_TERMINAL_EVIDENCE_INVALID",
        ...
        if (!formalOperations.isEmpty()) {
            throw rejected(
                    "PRODUCTION_RUNTIME_FINALIZATION_TERMINAL_OPERATION_PRESENT",
    ...
    private void requireTerminalMaterial(
        ...
        try {
            ...
            if (document == null
                ...
                throw rejected(
                        "PRODUCTION_RUNTIME_FINALIZATION_TERMINAL_MATERIAL_INVALID",
    ...
    private ExecuteAgentRunResult decodeCompletedAudit(TerminalRow terminal) {
        try {
            ...
            if (result.outcome() != ExecuteAgentRunResult.Outcome.COMPLETED) {
                throw rejected(
                        "PRODUCTION_RUNTIME_FINALIZATION_TERMINAL_RESULT_INVALID",
    ...
    private void requireSingleCompletion(
        ...
        if (rows.size() != 1) {
            throw rejected(
                    "PRODUCTION_RUNTIME_FINALIZATION_COMPLETION_MISSING",
    ...
    static void requireCanonicalCompletionHash(
        ...
        if (!Objects.equals(canonicalReceiptHash, persistedCompletionHash)) {
            throw rejected(
                    "PRODUCTION_RUNTIME_FINALIZATION_COMPLETION_HASH_MISMATCH",
    ...
    private FormalProjection readFormalProjection(
        ...
        if (operations.size() != 1) {
            throw rejected("PRODUCTION_RUNTIME_FINALIZATION_OPERATION_MISSING", "formal Intake operation is absent");
        }
        ...
        if (!"COMPLETED".equals(operation.status())
            ...
            throw rejected("PRODUCTION_RUNTIME_FINALIZATION_OPERATION_MISMATCH", "formal Intake operation conflicts with target receipt");
        }
        ...
        if (events.size() != 1) {
            throw rejected("PRODUCTION_RUNTIME_FINALIZATION_EVENT_MISSING", "formal Intake receipt event is absent");
        }
    ...
    static void requireCanonicalFormalOperationHash(
        ...
        if (!Objects.equals(canonicalFormalReceiptHash, persistedOperationHash)) {
            throw rejected(
                    "PRODUCTION_RUNTIME_FINALIZATION_OPERATION_HASH_MISMATCH",
    ...
    private FormalProjection decodeFormalProjection(
        ...
        try {
            ...
            if (!"intake-turn-committed-event.v1".equals(document.path("schema_version").asText())
                ...
                throw rejected("PRODUCTION_RUNTIME_FINALIZATION_EVENT_MISMATCH", "formal Intake event conflicts with its receipt");
            }
            ...
            if (eventType != IntakeDomainEventType.TURN_NEEDS_INPUT
                ...
                throw rejected("PRODUCTION_RUNTIME_FINALIZATION_EVENT_TYPE_INVALID", "formal Intake event is not a turn event");
            }
            ...
            throw rejected("PRODUCTION_RUNTIME_FINALIZATION_EVENT_INVALID", "formal Intake event cannot be decoded", failure);
        }
    }
    ...
    private void requireTargetBinding(
        ...
        if (!receipt.activationId().equals(target.activationId())
            ...
            throw rejected("PRODUCTION_RUNTIME_FINALIZATION_RECEIPT_MISMATCH", "target receipt is outside the exact Intake child authority");
        }
    }
    ...
    private void requireFormalBinding(
        ...
        if (!formal.tenantSurrogate().equals(command.tenantSurrogate())
            ...
            throw rejected("PRODUCTION_RUNTIME_FINALIZATION_FORMAL_RECEIPT_MISMATCH", "formal Intake receipt is outside the exact child authority");
        }
    }

    private ProductionFinalizationReceipt decodeTargetReceipt(TargetRow row) {
        ProductionFinalizationReceipt decoded = ProductionFinalizationReceiptCodec.decodeCanonical(row.canonicalBytes());
        if (!decoded.equals(row.columns())) {
            throw rejected("PRODUCTION_RUNTIME_FINALIZATION_RECEIPT_BYTES_MISMATCH", "receipt columns differ from canonical receipt bytes");
        }
        return decoded;
    }
    ...
    static RoomGraphCommand decodeAttemptCommand(
        ...
        try {
            ...
            if (document == null || !document.isObject()) {
                throw rejected(
                        "PRODUCTION_RUNTIME_FINALIZATION_ATTEMPT_COMMAND_INVALID",
    ...
    private ExecuteAgentRunRequest decodeWinningMaterial(
        ...
        try {
            ...
            if (materialDocument == null
                ...
                throw rejected(
                        "PRODUCTION_RUNTIME_FINALIZATION_WINNING_MATERIAL_INVALID",
    ...
    private TargetRow targetRow(ResultSet rs, int ignored) throws SQLException {
        ProductionFinalizationReceipt columns = new ProductionFinalizationReceipt(
                ...
                rs.getString("receipt_hash"), ProductionFinalizationReceipt.FormalWriter.valueOf(rs.getString("formal_writer")),
                ProductionFinalizationReceipt.DomainCommitStatus.valueOf(rs.getString("domain_commit_status")));
    ...
    private static ProductionFinalizationRejectedException rejected(String code, String message, Throwable cause) {
        return new ProductionFinalizationRejectedException(code, message, cause);
    }
    ...
    static void requireFormalEventProposalHash(
        ...
        if (!Objects.equals(formalPayloadProposalHash, eventProposalHash)) {
            throw rejected(
                    "PRODUCTION_RUNTIME_FINALIZATION_EVENT_MISMATCH",
    ...
    static String canonicalEventHash(JsonNode eventDocument) {
        if (eventDocument == null || !eventDocument.isObject()) {
            throw rejected(
                    "PRODUCTION_RUNTIME_FINALIZATION_EVENT_INVALID",
    ...
    private record TargetRow(String receiptId, String activationManifestHash, byte[] canonicalBytes,
            ProductionFinalizationReceipt columns, String agentRunManifestId, String agentRunManifestHash,
    ...
    record FormalProjection(
        ...
        TurnFinalizationReceipt toActivityReceipt(
            ...
            IntakeGraphExecutionRef graph = new IntakeGraphExecutionRef("intake-graph-execution-ref.v1", formal.threadId(),
                    ...
                    "urn:production-runtime:result:intake:" + formal.resultHash(), formal.resultHash(),
                    "urn:production-runtime:proposal:intake:" + formal.proposalHash(), formal.proposalHash());
            IntakeDomainEventRef committed = new IntakeDomainEventRef("intake-domain-event-ref.v1", event.id(),
                    "urn:production-runtime:intake-finalization:" + formal.receiptHash(),
                    eventHash,
        ...
        private static long committedRevision(long authorityRevision) {
            try {
                ...
                throw rejected(
                        "PRODUCTION_RUNTIME_FINALIZATION_REVISION_OVERFLOW",
