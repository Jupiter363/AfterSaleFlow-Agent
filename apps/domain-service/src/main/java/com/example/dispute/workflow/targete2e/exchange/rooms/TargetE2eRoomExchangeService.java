...
import com.example.dispute.workflow.targete2e.exchange.rooms.TargetE2eRoomExchangeContract.*;
...
public final class TargetE2eRoomExchangeService {
  private static final String PROPOSAL_BUCKET = "target-e2e-intake-activation";
  ...
  public TargetE2eRoomExchangeService(DataSource dataSource, ObjectMapper mapper, io.minio.MinioClient minio,
  ...
  public LoadResponse load(LoadRequest request) {
    ...
    TargetE2eRoomObjectIndex.StoredObject stored = objectIndex.findAdmitted(request.authority(), resolved.command(), request.objectRef())
    ...
    if (!"target-e2e-evidence-asset.v1".equals(request.objectRef().schemaVersion()) && payload.length != request.objectRef().sizeBytes()) throw reject("object payload size differs from immutable reference");
    ...
    return new LoadResponse("target-e2e-room-object-load-response.v1", request.authority(),
  ...
  public PutResponse put(PutRequest request) {
    ...
    TargetE2eRoomObjectIndex.StoredObject stored = objectIndex.recordProposal(request.authority(), resolved.command(),
        new TargetE2eRoomObjectIndex.ProposalIdentity(request.proposal().proposalId(), request.proposal().schemaVersion(),
    ...
    return new PutResponse("target-e2e-room-proposal-put-response.v1", request.authority(),
  ...
  private Resolved resolve(Authority authority) {
    String table = switch (authority.roomType()) { case "EVIDENCE" -> "target_e2e_evidence_command_material"; case "HEARING" -> "target_e2e_hearing_command_material"; case "REVIEW" -> "target_e2e_review_command_material"; default -> throw reject("room is not exchange eligible"); };
    String sql = """
      select m.material_canonical_json, m.material_sha256, m.activation_id, m.activation_manifest_hash,
        ...
        from %s m join target_e2e_activation a on a.activation_id=m.activation_id
        join target_e2e_case_reservation r on r.activation_id=a.activation_id and r.tenant_surrogate=m.tenant_surrogate and r.case_id=m.case_id
        join target_e2e_room_epoch_binding e on e.activation_id=m.activation_id and e.tenant_surrogate=m.tenant_surrogate and e.case_id=m.case_id and e.room_type=:roomType and e.room_epoch=m.room_epoch and e.room_fencing_token=m.room_fencing_token
        join target_e2e_command_admission adm on adm.admission_id=m.admission_id and adm.activation_id=m.activation_id and adm.tenant_surrogate=m.tenant_surrogate and adm.case_id=m.case_id and adm.command_id=m.command_id and adm.command_hash=m.command_hash and adm.command_envelope_hash=m.command_envelope_hash and adm.room_epoch=m.room_epoch and adm.room_fencing_token=m.room_fencing_token
       where m.activation_id=:activationId and m.tenant_surrogate=:tenantSurrogate and m.case_id=:caseId and m.command_id=:commandId and m.room_epoch=:roomEpoch and m.room_fencing_token=:fence
         and a.execution_lane='TARGET_E2E_CANDIDATE' and a.lifecycle_status in ('ACTIVE','DRAIN_ONLY') and (a.lifecycle_status='DRAIN_ONLY' or clock_timestamp() < a.expires_at)
      """.formatted(table);
  ...
  private byte[] get(TargetE2eRoomObjectIndex.StoredObject object, long maxSize) {
  ...
  private void writeIdempotent(String key, byte[] bytes, Proposal proposal) {
    try {
      try { var stat=minio.statObject(StatObjectArgs.builder().bucket(PROPOSAL_BUCKET).object(key).build()); if (stat.size()!=bytes.length) throw reject("proposal replay differs from immutable object"); byte[] existing=get(new TargetE2eRoomObjectIndex.StoredObject("urn:target-e2e:proposal:replay:" + proposal.sha256(), proposal.proposalId(), proposal.schemaVersion(), proposal.sha256(), proposal.sizeBytes(), PROPOSAL_BUCKET, key), proposal.sizeBytes()); if (!MessageDigest.isEqual(existing,bytes)) throw reject("proposal replay drift"); return; }
