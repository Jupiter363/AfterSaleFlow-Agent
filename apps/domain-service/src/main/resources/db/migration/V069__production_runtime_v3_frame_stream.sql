    ...
    add constraint ck_production_runtime_activation_bindings check (
        ...
        and (
            (
                ...
                and graph_version = 'production-runtime-graph.2026-08-18.1'
...
begin
    if new.graph_key = 'all-rooms.production-runtime.v1' then
        raise exception using errcode = '23514',
            message = 'production runtime v1 Intake thread writes are retired';
    end if;
    if new.graph_key <> 'all-rooms.production-runtime.v2' then
        return new;
    end if;
...
begin
    ...
    if new.graph_key <> 'all-rooms.production-runtime.v2'
       or new.graph_version <> 'production-runtime-graph.2026-08-18.1'
