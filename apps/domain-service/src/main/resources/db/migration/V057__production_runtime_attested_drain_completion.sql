...
alter table production_runtime_activation
    drop constraint ck_production_runtime_activation_lifecycle;

alter table production_runtime_activation
    add constraint ck_production_runtime_activation_lifecycle check (
        (
...
create or replace function guard_production_runtime_drain_proof_mutation()
returns trigger
language plpgsql
as $$
begin
    if old.lifecycle_status = 'DRAIN_ONLY' and new.lifecycle_status = 'DRAINED' then
        if old.drain_completion_proof_hash is not null
           ...
           or new.evidence_sealed is not true then
            raise exception using errcode = '23514',
                message = 'production runtime DRAINED transition requires exact attested evidence';
        end if;
       ...
       or new.evidence_sealed is distinct from old.evidence_sealed then
        raise exception using errcode = '55000',
            message = 'production runtime drain evidence is immutable';
    end if;
    return new;
end
$$;

create trigger trg_production_runtime_activation_drain_proof_guard
before update on production_runtime_activation
for each row execute function guard_production_runtime_drain_proof_mutation();
