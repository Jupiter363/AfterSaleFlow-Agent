...
create table production_runtime_room_object_binding (
    object_ref varchar(512) not null references production_runtime_room_object_index(object_ref),
    ...
    constraint ck_production_runtime_room_object_binding_shape check (
...
select object_ref, object_kind, artifact_id, schema_version,
  ...
  from production_runtime_room_object_index;
...
create unique index uq_production_runtime_room_object_binding_proposal_identity
    on production_runtime_room_object_binding (
        activation_id, tenant_surrogate, case_id, room_type, room_epoch,
...
create index ix_production_runtime_room_object_binding_admitted on production_runtime_room_object_binding (
    activation_id, tenant_surrogate, case_id, room_type, room_epoch,
    room_fencing_token, command_id, object_ref);
create index ix_production_runtime_room_object_binding_proposal on production_runtime_room_object_binding (
    activation_id, tenant_surrogate, case_id, room_type, room_epoch,
    room_fencing_token, command_id, logical_run_id, attempt_id);

create trigger trg_production_runtime_room_object_binding_immutable
before update or delete on production_runtime_room_object_binding
for each row execute function reject_production_runtime_append_only_mutation();
create trigger trg_production_runtime_room_object_binding_no_truncate
before truncate on production_runtime_room_object_binding
for each statement execute function reject_production_runtime_append_only_mutation();

revoke all on production_runtime_room_object_binding from public;
