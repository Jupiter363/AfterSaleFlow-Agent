-- V055: PRODUCTION durable relay facts. These rows are append-only evidence; the Temporal
-- workflow only applies exact facts read back from this ledger.
create table production_runtime_outcome_completion_fact (
    ...
    constraint fk_production_runtime_outcome_completion_case
      foreign key (case_id) references fulfillment_dispute_case(id),
    constraint ck_production_runtime_outcome_completion_shape check (
...
create function enforce_production_runtime_outcome_completion_fact()
returns trigger language plpgsql as $$
declare previous production_runtime_outcome_completion_fact%rowtype;
begin
  select * into previous from production_runtime_outcome_completion_fact
...
create trigger trg_production_runtime_outcome_completion_fact
before insert on production_runtime_outcome_completion_fact
for each row execute function enforce_production_runtime_outcome_completion_fact();
create trigger trg_production_runtime_outcome_completion_fact_immutable
before update or delete on production_runtime_outcome_completion_fact
for each row execute function reject_append_only_mutation();
create trigger trg_production_runtime_outcome_completion_fact_no_truncate
before truncate on production_runtime_outcome_completion_fact
for each statement execute function reject_append_only_mutation();

-- V045 deliberately kept the ledger engineering-only. PRODUCTION needs the same
