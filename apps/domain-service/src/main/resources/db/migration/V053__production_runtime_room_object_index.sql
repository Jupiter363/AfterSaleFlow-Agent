...
create table production_runtime_room_object_index (
    ...
    constraint uq_production_runtime_room_object_storage unique (storage_bucket, storage_key),
    constraint uq_production_runtime_room_proposal_identity unique (
    ...
    constraint ck_production_runtime_room_object_index_shape check (
        object_kind in ('COMMAND_INPUT', 'MANIFEST_ASSET', 'PROPOSAL')
        and room_type in ('EVIDENCE', 'HEARING', 'REVIEW')
        and object_ref ~ '^urn:production-runtime:(object|proposal):'
...
create index ix_production_runtime_room_object_admitted_ref on production_runtime_room_object_index (
    activation_id, tenant_surrogate, case_id, room_type, room_epoch,
    room_fencing_token, command_id, object_ref);
create index ix_production_runtime_room_object_proposal_lookup on production_runtime_room_object_index (
    activation_id, tenant_surrogate, case_id, room_type, room_epoch,
    room_fencing_token, command_id, logical_run_id, attempt_id, artifact_id, schema_version);

create trigger trg_production_runtime_room_object_index_immutable
before update or delete on production_runtime_room_object_index
for each row execute function reject_production_runtime_append_only_mutation();
create trigger trg_production_runtime_room_object_index_no_truncate
before truncate on production_runtime_room_object_index
for each statement execute function reject_production_runtime_append_only_mutation();
revoke all on production_runtime_room_object_index from public;
