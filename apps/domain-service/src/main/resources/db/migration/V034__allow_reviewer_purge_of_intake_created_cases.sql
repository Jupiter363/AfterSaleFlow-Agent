alter table demo_case_purge_audit
    alter column source_system drop not null;

alter table demo_case_purge_audit
    drop constraint if exists ck_demo_case_purge_source;

alter table demo_case_purge_audit
    add constraint ck_demo_case_purge_source
        check (
...
comment on function purge_simulated_dispute_case(varchar, varchar, varchar)
    is 'Physically deletes reviewer-approved intake-created or simulated external-import cases and preserves a purge audit snapshot.';
