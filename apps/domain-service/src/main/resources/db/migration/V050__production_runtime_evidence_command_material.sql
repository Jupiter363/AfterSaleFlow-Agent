...
create table production_runtime_evidence_command_material (
    ...
    constraint uq_production_runtime_evidence_material_command unique (activation_id, command_id),
    constraint fk_production_runtime_evidence_material_admission
        foreign key (admission_id) references production_runtime_command_admission(admission_id),
    constraint fk_production_runtime_evidence_material_activation
    ...
    constraint fk_production_runtime_evidence_material_case
    ...
    constraint fk_production_runtime_evidence_material_epoch
    ...
    constraint ck_production_runtime_evidence_material_shape check (
        execution_lane = 'PRODUCTION' and room_type = 'EVIDENCE'
        and material_schema_version = 'production-runtime-evidence-command-material.v1'
...
create or replace function enforce_production_runtime_evidence_command_material()
returns trigger language plpgsql as $$
declare admission_row production_runtime_command_admission%rowtype; material jsonb;
begin
  select * into admission_row from production_runtime_command_admission where admission_id = new.admission_id for share;
  if not found or admission_row.activation_id is distinct from new.activation_id
    ...
    raise exception using errcode = '23514', message = 'production runtime Evidence material must exactly bind its command admission';
  end if;
  ...
  if material #>> '{schema_version}' is distinct from new.material_schema_version
    ...
    raise exception using errcode = '23514', message = 'production runtime Evidence material is not an exact admission binding';
  end if;
  return new;
end $$;

create trigger trg_production_runtime_evidence_material_binding before insert on production_runtime_evidence_command_material
for each row execute function enforce_production_runtime_evidence_command_material();
create trigger trg_production_runtime_evidence_material_immutable before update or delete on production_runtime_evidence_command_material
for each row execute function reject_production_runtime_append_only_mutation();
create trigger trg_production_runtime_evidence_material_no_truncate before truncate on production_runtime_evidence_command_material
for each statement execute function reject_production_runtime_append_only_mutation();
revoke all on production_runtime_evidence_command_material from public;
