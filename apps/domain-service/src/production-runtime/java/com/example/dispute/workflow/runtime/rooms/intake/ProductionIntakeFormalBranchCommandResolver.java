...
public final class ProductionIntakeFormalBranchCommandResolver
  ...
  public ProductionIntakeFormalBranchCommandResolver(
  ...
  public ResolvedBranchCommand resolve(BranchCommitRequest request) {
    ...
    if (bytes.length == 0 || bytes.length > MAX_BYTES) {
      throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_SIZE_INVALID", "branch command object size is invalid");
    }
    if (!reference.hash().equals(sha256(bytes))
        || !reference.hash().equals(request.envelope().commandPayloadHash())) {
      throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_HASH_MISMATCH", "branch command object hash differs from request");
    }
  ...
  private Reference parseReference(BranchCommitRequest request) {
    ...
    try {
      ...
      if (!"minio".equals(uri.getScheme()) || !bucket.equals(uri.getHost())
          ...
          || uri.getPort() != -1) {
        throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_REFERENCE_INVALID", "branch command reference is outside the target object store");
      }
      String path = uri.getRawPath();
      if (path == null || !path.equals(uri.getPath())) {
        throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_REFERENCE_INVALID", "branch command reference must not be encoded");
      }
      String expectedPrefix = "/" + prefix + "/" + IntakeBranchCommand.SCHEMA_VERSION + "/";
      if (!path.startsWith(expectedPrefix)) {
        throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_REFERENCE_INVALID", "branch command reference is outside the allowlisted prefix");
      }
      ...
      if (segments.length != 2 || !segments[0].matches("[A-Za-z0-9][A-Za-z0-9._:-]{0,127}")
        ...
        throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_REFERENCE_INVALID", "branch command reference layout is invalid");
      }
      String hash = segments[1].substring(0, 64);
      if (!hash.equals(request.envelope().commandPayloadHash())) {
        throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_REFERENCE_INVALID", "branch command reference does not bind request hash");
      }
      return new Reference(path.substring(1), hash);
    } catch (IllegalArgumentException failure) {
      throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_REFERENCE_INVALID", "branch command reference is malformed");
    }
  }
  ...
  private IntakeBranchCommand decodeCanonical(byte[] bytes) {
    try {
      ...
      if (document == null || !document.isObject()
        ...
        throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_CANONICAL_INVALID", "branch command is not canonical JSON");
      }
      return mapper.treeToValue(document, IntakeBranchCommand.class);
    } catch (IntakeFinalizationRejectedException failure) {
      throw failure;
    } catch (Exception failure) {
      throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_MALFORMED", "branch command is malformed");
    }
  }
  ...
  private static void requireBinding(BranchCommitRequest request, IntakeBranchCommand command) {
    ...
    try {
      ...
      throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_BINDING_INVALID", "branch command operation is invalid");
    }
    if (!IntakeBranchCommand.SCHEMA_VERSION.equals(command.schemaVersion())
      ...
      throw rejected("PRODUCTION_RUNTIME_BRANCH_COMMAND_BINDING_INVALID", "branch command does not match the exact request");
    }
  }
