...
public final class JdbcProductionFinalizationAuthority
        implements ProductionFinalizationActivationPort, ProductionFinalizationEnvironmentSource {
    ...
    public JdbcProductionFinalizationAuthority(
    ...
    public AuthorizationDecision authorize(AuthorizationRequest request) {
        ...
        if (!runtime.executionLane().equals("PRODUCTION")
                ...
                || !runtime.graphKey().equals("all-rooms.production-runtime.v2")) {
            return AuthorizationDecision.denied(Decision.SCOPE_DENIED);
        }
    ...
    private AuthorizationRow uniqueAuthorization(
        ...
        if (rows.size() != 1) {
            throw new IllegalStateException("production runtime " + label + " is not unique");
        }
    ...
    public EnvironmentEvidence loadEnvironmentEvidence(String authorityActivationId) {
        ...
        if (rows.size() != 1) {
            throw new IllegalStateException("production runtime activation evidence is absent or ambiguous");
        }
        return rows.getFirst();
    }
