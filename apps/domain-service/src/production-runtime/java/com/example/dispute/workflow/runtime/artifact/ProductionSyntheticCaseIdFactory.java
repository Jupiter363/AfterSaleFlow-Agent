 ...
 */
final class ProductionSyntheticCaseIdFactory implements ImportedCaseIdFactory {
  ...
  private static final String NEXT_SLOT_SQL = """
      select candidate.slot_number
       ...
       where not exists (
             select 1
               from production_runtime_case_reservation reservation
  ...
  ProductionSyntheticCaseIdFactory(DataSource dataSource, String activationId, Clock clock) {
    this.dataSource = Objects.requireNonNull(dataSource, "dataSource");
    if (activationId == null || !activationId.matches("p9act\\.v1\\.[0-9a-f]{32}")) {
      throw new IllegalArgumentException("production runtime activationId is invalid");
    }
  ...
  public String nextCaseId() {
    if (!TransactionSynchronizationManager.isActualTransactionActive()) {
      throw new IllegalStateException("production runtime synthetic case allocation requires an import transaction");
    }
    ...
    try {
      ...
      throw new IllegalStateException("production runtime synthetic case allocation failed", failure);
    } finally {
      ...
      try (ResultSet result = statement.executeQuery()) {
        ...
        if (!scope.expiresAt().isAfter(clock.instant())) {
          throw new BusinessException(
              ...
              "production runtime activation has expired",
              Map.of(
      ...
      try (ResultSet result = statement.executeQuery()) {
        if (!result.next()) {
          throw new BusinessException(
              ...
              "production runtime synthetic case capacity is exhausted",
  ...
  private BusinessException activationUnavailable(String reason, Instant expiresAt) {
    ...
    return new BusinessException(
        ...
        "production runtime activation is unavailable",
        details);
  }
  ...
  private void insertReservation(Connection connection, Scope scope, int slot, String caseId)
    ...
    try (PreparedStatement statement = connection.prepareStatement(INSERT_RESERVATION_SQL)) {
      ...
      if (statement.executeUpdate() != 1) {
        throw new IllegalStateException("production runtime synthetic case reservation was not inserted");
      }
    }
  }
