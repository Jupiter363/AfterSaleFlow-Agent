...
class ProductionReviewRoomFinalizationStrategyAuthorizationTest {
  ...
  void reauthorizesTheExactDurableReviewRouteBeforePreparingACommit() {
    ...
    when(graph.graphVersion()).thenReturn(
        com.example.dispute.workflow.runtime.finalization.ProductionExecutionLaneVerifier.GRAPH_VERSION);
    when(graph.checkpointSchemaVersion()).thenReturn(
        com.example.dispute.workflow.runtime.finalization.ProductionExecutionLaneVerifier.CHECKPOINT_SCHEMA_VERSION);
    ...
    TargetReviewFinalizationFactsProvider factsProvider = (ignored, ignoredRequest, ignoredResult) ->
        new FinalizationFacts(
            ...
            "provider-review",
            "model-review",
            "urn:production-runtime:review-manifest",
            new ArtifactPointer(
                "OUTPUT_REVIEW_1", "review-output.v1", "urn:production-runtime:review-output", "9".repeat(64)),
            List.of(),
            List.of(),
            10,
            Instant.parse("2026-07-29T00:30:00Z"));
    ProductionFinalizationActivationPort activation = mock(ProductionFinalizationActivationPort.class);
    when(activation.authorize(org.mockito.ArgumentMatchers.any()))
        .thenReturn(ProductionFinalizationActivationPort.AuthorizationDecision.denied(Decision.REVOKED));
    var strategy = new ProductionReviewRoomFinalizationStrategy(resolver, factsProvider, activation);
