...
class ProductionIntakeContractAlignmentMigrationTest {

    private static final Path MIGRATION = Path.of(
            "src", "main", "resources", "db", "migration",
            "V056__production_runtime_intake_contract_alignment.sql");
    private static final Path V048 = Path.of(
            "src", "main", "resources", "db", "migration",
            "V048__production_runtime_intake_thread_binding.sql");
    ...
    void permitsOnlyTheFrozenLegacyTupleOrTheFullyPinnedTemporalTargetTuple()
        ...
        assertThat(sql)
                ...
                .contains("graph_key = 'all-rooms.production-runtime.v1'")
                .contains("writer_mode = 'temporal'")
                .contains("graph_version = 'production-runtime-graph.2026-07-27.1'")
                .contains("checkpoint_schema_version = 'production-runtime-checkpoint.v1'")
                .contains("output_schema_version in (")
                .contains("'intake-turn-proposal.v2'")
                .contains("'production-runtime-room-proposal-source.v1'");

        int legacyBranch = sql.indexOf("graph_key = 'intake.v2'");
        int targetBranch = sql.indexOf("graph_key = 'all-rooms.production-runtime.v1'");
        assertThat(sql.substring(legacyBranch, targetBranch))
                .contains("output_schema_version = 'intake-turn-proposal.v2'")
                .doesNotContain("production-runtime-room-proposal-source.v1");
