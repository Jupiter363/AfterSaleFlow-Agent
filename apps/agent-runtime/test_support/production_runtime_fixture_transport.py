"""Deterministic local model transport for the isolated production-runtime fixture lane."""
...
from app.config import (
    ...
    GraphProductionSyntheticCaseScope,
)
...
PRODUCTION_RUNTIME_FIXTURE_PROVIDER = "production-runtime-fixture"
PRODUCTION_RUNTIME_FIXTURE_MODEL = "production-runtime.intake-fixture.v1"


@dataclass(frozen=True, slots=True)
class ProductionDeterministicFixtureTransport:
    ...
    def fixture_binding_hash(self) -> str:
        return canonical_sha256(
            {
                "schema_version": "production-runtime-fixture-model-binding.v1",
...
def build_production_runtime_fixture_transport(
    *,
    ...
    if (
        not isinstance(scope, GraphProductionSyntheticCaseScope)
        ...
        or binding.graph_key != "all-rooms.production-runtime.v2"
        ...
        raise GraphContractError("PRODUCTION_RUNTIME_FIXTURE_RUNTIME_BINDING_REQUIRED")
    return ProductionDeterministicFixtureTransport(
...
def _require_fixture_request(request: ModelTransportRequest) -> None:
    ...
    if (
        ...
        or governed.provider != PRODUCTION_RUNTIME_FIXTURE_PROVIDER
        or governed.model != PRODUCTION_RUNTIME_FIXTURE_MODEL
        or governed.temperature != 0
        or governed.tool_allowlist
        or governed.response_format != "STRICT_JSON_SCHEMA"
    ):
        raise GraphContractError("PRODUCTION_RUNTIME_FIXTURE_MODEL_REQUEST_REJECTED")


def _fixture_draft(request: ModelTransportRequest) -> IntakeCognitionDraft:
    """Emit the two valid Intake proposals exercised by the production runtime lane.
    ...
    if matrix is None:
        return IntakeCognitionDraft(
            ...
            matrix_patch={
                ...
                "fact_rows": [
                    {
                        "fact_key": "NEW_PRODUCTION_RUNTIME_DELIVERY",
                ...
                "summary_source_fact_keys": ["NEW_PRODUCTION_RUNTIME_DELIVERY"],
            },
    ...
    if not isinstance(rows, list) or not rows:
        raise GraphContractError("PRODUCTION_RUNTIME_FIXTURE_FROZEN_INITIATOR_MATRIX_REQUIRED")
        ...
        delta_rows.append(
            {
                ...
                "stance": "CONFIRM",
                "position_summary": "The respondent confirms the delivery record for the production runtime case.",
    ...
    return IntakeCognitionDraft(
        ...
        matrix_patch={
            ...
            "respondent_claim": {
                "attitude": "AGREE",
                "position_summary": "The respondent accepts the production runtime intake record.",
            },
        },
        readiness="READY_TO_CONFIRM",
        missing_fields=(),
...
def _fixture_human_prompt(request: ModelTransportRequest) -> str:
    if len(request.messages) != 2 or not isinstance(request.messages[1].content, str):
        raise GraphContractError("PRODUCTION_RUNTIME_FIXTURE_PROMPT_REQUIRED")
...
def _fixture_audience(content: str) -> str:
    ...
    if match is None:
        raise GraphContractError("PRODUCTION_RUNTIME_FIXTURE_AUDIENCE_REQUIRED")
...
def _fixture_frozen_initiator_matrix(content: str) -> dict[str, Any] | None:
    ...
    if match is None:
        raise GraphContractError("PRODUCTION_RUNTIME_FIXTURE_DOSSIER_REQUIRED")
    try:
        dossier = json.loads(match.group(1))
    except json.JSONDecodeError as error:
        raise GraphContractError("PRODUCTION_RUNTIME_FIXTURE_DOSSIER_REQUIRED") from error
...
__all__ = [
    ...
    "ProductionDeterministicFixtureTransport",
    "build_production_runtime_fixture_transport",
]
