...
def _target_admission(
    *,
    ...
    manifest["profile_versions"]["graph_version"] = PRODUCTION_RUNTIME_GRAPH_VERSION
    manifest["profile_versions"]["checkpoint_schema_version"] = (
        PRODUCTION_RUNTIME_CHECKPOINT_SCHEMA_VERSION
    )
    for item in manifest["items"]:
        item["parse_ref"] = (
            f"urn:production-runtime:object:{item['evidence_id']}:{item['parse_hash']}"
        )
    ...
    target_request = replace(
        target_request,
        registry_output_schema_version=PRODUCTION_RUNTIME_OUTPUT_SCHEMA_VERSION,
    )
...
async def test_target_lcel_is_prompt_model_parser_and_deterministic(
    ...
    assert first == second
    assert first["execution_scope"] == "PRODUCTION"
...
def test_target_admission_is_separate_from_public_shadow_verifier(
    ...
    request = admission_request_factory(1)
    command = deepcopy(dict(request.room_graph_command))
    command["graph_key"] = PRODUCTION_RUNTIME_GRAPH_KEY
    command["graph_version"] = PRODUCTION_RUNTIME_GRAPH_VERSION
    command["checkpoint_schema_version"] = PRODUCTION_RUNTIME_CHECKPOINT_SCHEMA_VERSION
    command["invocation_context"]["output_schema_version"] = (
        PRODUCTION_RUNTIME_OUTPUT_SCHEMA_VERSION
    )
    manifest = json.loads(request.signed_manifest_payload)
    ...
    assert admission.runtime_mode == "PRODUCTION"
    assert evidence_execution_scope(admission) == "PRODUCTION"
...
def test_target_admission_rejects_shadow_relabel(
    ...
    with pytest.raises(
        EvidenceGraphContractError,
        match="EVIDENCE_PRODUCTION_RUNTIME_SCOPE_REQUIRED",
    ):
