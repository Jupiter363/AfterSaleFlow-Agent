...
def _graph_command() -> RoomGraphCommand:
    payload = {
        ...
        "domain_snapshot_ref": {
            ...
            "uri": "urn:production-runtime:review-packet:001",
            "sha256": "3" * 64,
            "size_bytes": 1024,
        },
        "event_ref": {
            ...
            "uri": "urn:production-runtime:review-question:001",
            "sha256": "4" * 64,
            "size_bytes": 256,
        },
        "invocation_context": {
...
def _target_execution(command: RoomGraphCommand) -> GatewayExecution:
    ...
    fence = GraphFenceContext(
        ...
        environment_id="production-runtime-review",
...
def test_registry_binding_requires_the_unified_outer_production_runtime_binding() -> None:
    binding = _registry_binding()

    require_exact_outcome_production_runtime_registry_binding(binding)

    with pytest.raises(
        OutcomeReviewContractError,
        match="OUTCOME_PRODUCTION_RUNTIME_REGISTRY_BINDING_MISMATCH",
    ):
        require_exact_outcome_production_runtime_registry_binding(
...
def test_target_context_accepts_the_unified_outer_binding() -> None:
    context = OutcomeProductionExecutionContext.from_gateway_execution(
...
def test_target_context_rejects_a_room_specific_outer_binding() -> None:
    ...
    with pytest.raises(
        OutcomeReviewContractError,
        match="OUTCOME_PRODUCTION_RUNTIME_COMMAND_BINDING_MISMATCH",
    ):
        OutcomeProductionExecutionContext.from_gateway_execution(
...
def test_normalized_review_proposal_uses_only_proposal_as_hash_source() -> None:
    ...
    source = build_outcome_production_runtime_proposal_source(
        ...
        payload_ref="urn:production-runtime:proposal:review:001",
        payload_hash="5" * 64,
    )

    assert source.room_type == "REVIEW"
    assert source.proposal.schema_version == "production-runtime-review-proposal.v1"
...
def test_target_context_cannot_be_built_from_unverified_shadow_execution() -> None:
    ...
    with pytest.raises(
        OutcomeReviewContractError,
        match="OUTCOME_PRODUCTION_RUNTIME_GATEWAY_AUTHORITY_REQUIRED",
    ):
        OutcomeProductionExecutionContext.from_gateway_execution(execution)
...
def test_payload_schema_forbids_any_formal_authority(
    ...
    with pytest.raises(ValueError):
        OutcomeProductionProposalPayload.model_validate(values)
