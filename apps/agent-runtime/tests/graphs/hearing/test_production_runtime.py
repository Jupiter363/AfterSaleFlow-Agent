...
from app.graph_runtime.production_runtime import ProductionGraphResultEnvelope
from app.graph_runtime.production_runtime_room_exchange import build_hearing_agent_context
...
from app.graphs.hearing import production_runtime as hearing_production_runtime
...
OUTER_GRAPH_KEY = "all-rooms.production-runtime.v1"
OUTER_GRAPH_VERSION = "production-runtime-graph.2026-07-27.1"
OUTER_CHECKPOINT_SCHEMA = "production-runtime-checkpoint.v1"
...
class _InvocationProvider:
    ...
    async def load(
        ...
        operation = next(
            operation
            for operation, binding in HEARING_PRODUCTION_RUNTIME_OPERATION_BINDINGS.items()
            if binding.command_stage_code == command.stage_code
        )
        request = _Request(
            stage_code=(
                self.request_stage_override
                or HEARING_PRODUCTION_RUNTIME_OPERATION_BINDINGS[operation].request_stage_code
            ),
            stage_sequence=command.stage_sequence,
        )
        self.calls.append(operation)
        result_schema = HEARING_PRODUCTION_RUNTIME_OPERATION_BINDINGS[
            operation
    ...
    async def put(
        ...
        *,
        ...
        return HearingProductionStoredPayload(
            ...
            payload_ref=f"urn:production-runtime:proposal:hearing:{payload_hash}",
...
def _execution(
    ...
    *,
    ...
    operation_binding = HEARING_PRODUCTION_RUNTIME_OPERATION_BINDINGS[operation]
    payload: dict[str, Any] = {
        ...
        "domain_snapshot_ref": {
            ...
            "uri": "urn:production-runtime:input:hearing:snapshot-1",
            "sha256": "a" * 64,
            "size_bytes": 128,
        },
        "invocation_context": {
            "agent_profile_id": "hearing-agent.v1",
            "prompt_profile_id": "production-runtime.all-rooms.prompt.v1",
            "model_profile_id": "production-runtime.all-rooms.model.v1",
            "output_schema_version": "production-runtime-room-proposal.v1",
            "policy_version": "production-runtime.proposal-only.v1",
            "guardrail_version": "production-runtime.guardrails.v1",
            "tool_capabilities": [],
    ...
    binding = SimpleNamespace(
        ...
        graph_version=OUTER_GRAPH_VERSION,
        checkpoint_schema_version=OUTER_CHECKPOINT_SCHEMA,
        state_schema_version="production-runtime-composite-state.v1",
        state_schema_hash="c" * 64,
        command_schema_version="room-graph-command.v1",
        result_schema_version="room-graph-result.v1",
        prompt_version="production-runtime.all-rooms.prompt.v1",
        model_profile_id="production-runtime.all-rooms.model.v1",
        output_schema_version="production-runtime-room-proposal.v1",
        policy_version="production-runtime.proposal-only.v1",
        guardrail_version="production-runtime.guardrails.v1",
        tool_policy_version="production-runtime.no-tools.v1",
        binding_hash="b" * 64,
        code_build_id="p9-graph-hearing-1",
    )
...
def test_target_registry_explicitly_covers_15_stage_flow_and_model_operations() -> None:
    registry = production_runtime_hearing_family_registrations()

    assert len(HEARING_WORKFLOW_STAGE_CODES) == len(set(HEARING_WORKFLOW_STAGE_CODES)) == 15
    assert len(registry) == 4
    assert set(HEARING_PRODUCTION_RUNTIME_OPERATION_BINDINGS) == set(HearingOperation)
    assert {
        node
...
def test_target_workflow_binds_every_hearing_operation_to_async_execution() -> None:
    ...
    for operation in HearingOperation:
        binding = HEARING_PRODUCTION_RUNTIME_OPERATION_BINDINGS[operation]
        request = _Request(
            stage_code=binding.request_stage_code,
            stage_sequence=1,
        )
        invocation = workflows.production_runtime_invocation(operation, request)
...
def test_composite_provider_exports_exact_hearing_room_and_stream_seam() -> None:
    ...
    provider = build_production_runtime_hearing_provider(
...
def test_target_adapter_runs_each_exact_family_and_returns_proposal_only_source(
    ...
    store = _PayloadStore()
    adapter = HearingProductionRuntimeAdapter(
        checkpointer=_MemoryFencedSaver(execution.fence),
    ...
    expected_payload = _Proposal(
        schema_version=HEARING_PRODUCTION_RUNTIME_OPERATION_BINDINGS[
            operation
        ].result_schema_version
    ...
    assert material.source == HearingProductionProposalSource.model_validate(
        material.source.model_dump(mode="json")
    )
    assert material.source.room_type == "HEARING"
    assert material.source.proposal.schema_version == "production-runtime-hearing-proposal.v1"
...
def test_target_adapter_surface_does_not_accept_activation_jws() -> None:
    parameters = inspect.signature(HearingProductionRuntimeAdapter.execute).parameters
...
def test_target_stream_binds_frozen_execution_identity_before_external_terminal_commit() -> None:
    ...
    envelope = ProductionGraphResultEnvelope.model_validate(result.result_envelope_json)
...
def test_v4_governed_stream_separates_graph_and_model_node_identities() -> None:
    operation = HearingOperation.INTAKE_QUESTIONS
    model_node = HEARING_PRODUCTION_RUNTIME_OPERATION_BINDINGS[operation].model_nodes[-1]
    governed_event = {
    ...
    events = hearing_production_runtime._governed_hearing_events(
    ...
    with pytest.raises(
        ...
        hearing_production_runtime._governed_hearing_events(
...
def test_non_frame_hearing_stream_projects_generation_reset_control_event() -> None:
    ...
    projected = HearingProductionRuntimeAdapter._visible_events(candidate, binding)
...
async def test_v5_frame_bridge_resets_provisional_generation_before_retry() -> None:
    operation = HearingOperation.INTAKE_QUESTIONS
    execution, _ = _execution(operation)
    frame_stream = hearing_production_runtime._HearingV5FrameStream(  # noqa: SLF001
        execution, operation
    )
    bridge = hearing_production_runtime._HearingV5ObserverBridge()  # noqa: SLF001
    observer = AgentStreamObserver(
...
async def test_target_stream_forwards_governed_delta_before_completion_and_replays_bytes(
    ...
    original_bundle_builder = hearing_production_runtime.build_production_runtime_hearing_runtime_bundle
    ...
    invalid_adapter = HearingProductionRuntimeAdapter(
    ...
    cancelled_adapter = HearingProductionRuntimeAdapter(
    ...
    actual_adapter = HearingProductionRuntimeAdapter(
