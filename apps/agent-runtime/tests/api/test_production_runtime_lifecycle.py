...
def _verified() -> VerifiedProductionLifecycleReceipt:
    ...
    return VerifiedProductionLifecycleReceipt(receipt, "java-key-1", "d" * 64)
...
class _Verifier:
    def __init__(self, verified: VerifiedProductionLifecycleReceipt) -> None:
...
class _Reconciler:
    def __init__(self, verified: VerifiedProductionLifecycleReceipt) -> None:
        self.verified = verified
        self.calls: list[VerifiedProductionLifecycleReceipt] = []
        self.failure: Exception | None = None
...
def _post(client: TestClient, **kwargs: Any) -> Any:
    ...
    return client.post(PRODUCTION_RUNTIME_LIFECYCLE_PATH, headers=headers, **kwargs)
...
@pytest.mark.parametrize(
    ...
    [
        ("DISABLED", True, 503, "PRODUCTION_RUNTIME_LIFECYCLE_DISABLED"),
        ("SHADOW", True, 503, "PRODUCTION_RUNTIME_LIFECYCLE_DISABLED"),
        ("PRODUCTION", False, 503, "PRODUCTION_RUNTIME_LIFECYCLE_NOT_READY"),
    ],
)
...
def test_lifecycle_endpoint_rejects_bootstrap_header_body_and_duplicate_authorization() -> None:
    ...
    bootstrap = _post(
        client,
        headers={"X-AfterSaleFlow-Production-Runtime-Activation": "must-not-be-a-bearer"},
    )
    ...
    assert bootstrap.json()["code"] == "PRODUCTION_RUNTIME_ACTIVATION_HEADER_FORBIDDEN"
    assert body.status_code == 400
    assert body.json()["code"] == "PRODUCTION_RUNTIME_LIFECYCLE_BODY_FORBIDDEN"
    assert duplicate_auth.status_code == 401
    assert duplicate_auth.json()["code"] == "PRODUCTION_RUNTIME_LIFECYCLE_AUTHENTICATION_REJECTED"
    assert transport.calls == 0
    assert verifier.calls == []
    assert reconciler.calls == []
...
def test_lifecycle_endpoint_maps_authentication_and_drain_conflicts_without_details() -> None:
    ...
    verifier.failure = ProductionLifecycleAuthenticationError("private signature detail")
    ...
    verifier.failure = ProductionLifecycleBindingError("private deployment binding")
    ...
    reconciler.failure = ProductionDrainIncompleteError("private command identity")
    drain = _post(client)

    assert drain.status_code == 409
    assert drain.json() == {"code": "PRODUCTION_RUNTIME_DRAIN_INCOMPLETE", "retryable": True}
