...
from app.graph_runtime.production_bindings import _build_production_runtime_room_providers
...
from test_support.production_runtime_fixture_transport import (
    PRODUCTION_RUNTIME_FIXTURE_MODEL,
    PRODUCTION_RUNTIME_FIXTURE_PROVIDER,
    ProductionDeterministicFixtureTransport,
)
...
async def test_evidence_asset_loader_reads_only_the_manifest_bound_parse_reference() -> None:
    ...
    loader = ProductionObjectEvidenceAssetLoader(store)
...
def test_default_target_composite_requires_a_specialized_room_factory() -> None:
    source = inspect.getsource(_build_production_runtime_room_providers)

    assert "build_production_runtime_intake_provider" in source
    assert "PRODUCTION_RUNTIME_SPECIALIZED_ROOM_RUNTIME_REQUIRED" in source
...
def test_target_intake_uses_the_governed_executor_and_real_stored_object_uri() -> None:
    provider_source = inspect.getsource(ProductionIntakeProvider)
    ...
    assert "payload_ref=f\"urn:production-runtime:proposal:intake:{stored.sha256}\"" in executor_source


def test_review_target_result_projects_the_exchange_persisted_proposal_as_a_patch() -> None:
    source = inspect.getsource(ProductionOutcomeGraphProvider.stream)
...
async def test_target_evidence_runs_formal_turn_and_replays_exact_guarded_utterance() -> None:
    ...
    production_workflow = _build_production_runtime_evidence_workflow(
        settings=settings,
    ...
    class ObjectStore:
        ...
        async def put(self, **kwargs):
            ...
            return f"urn:production-runtime:proposal:evidence:{kwargs['payload_hash']}"
    ...
    invocation = {
        "schema_version": "production-runtime-evidence-turn-invocation.v2",
    ...
    def snapshot_ref(payload: bytes) -> SnapshotRef:
        ...
        return SnapshotRef(
            ...
            schema_version="production-runtime-evidence-turn-invocation.v2",
            uri=f"urn:production-runtime:object:target-evidence-turn-invocation:{digest[:32]}:{digest}",
            sha256=digest,
    ...
    command = RoomGraphCommand.model_validate_json(
        ...
        "graph_version": "production-runtime-graph.2026-08-18.3",
                ...
                "event_ref": {
                    ...
                    "schema_version": "production-runtime-case-command-payload.v1",
                    "uri": "urn:production-runtime:object:evidence-submit-event:COMMAND_TEST:"
                    + "a" * 64,
                    "sha256": "a" * 64,
                    "size_bytes": 128,
                },
    ...
    fence = GraphFenceContext(
        ...
        room_type="EVIDENCE",
        binding_hash="d" * 64,
        code_build_id="production-runtime-test-build",
    )
    ...
    assert saver.commits[0].result.result_json["artifact_operations"] == [
        {
            ...
            "artifact": {
                ...
                "uri": (
                    "urn:production-runtime:proposal:evidence:"
    ...
    legacy_v1["schema_version"] = "production-runtime-evidence-turn-invocation.v1"
        ...
        update={
            ...
            "event_ref": command.event_ref.model_copy(
                update={
                    ...
                    "uri": "urn:production-runtime:object:evidence-opening-event:COMMAND_TEST:"
                    + "8" * 64,
                    "sha256": "8" * 64,
                }
            ),
...
def test_target_fixture_transport_emits_a_valid_intake_cognition_draft_without_network() -> None:
    ...
    transport = ProductionDeterministicFixtureTransport(
    ...
    def request_for(content: str) -> ModelTransportRequest:
        ...
        governed_request=GovernedProviderRequest(
            provider=PRODUCTION_RUNTIME_FIXTURE_PROVIDER,
            model=PRODUCTION_RUNTIME_FIXTURE_MODEL,
            temperature=0,
    ...
    respondent = transport.generate(
        request_for(
            ...
            "<authorized_dossier_json>{\"case_fact_matrix\":{\"schema_version\":\"case_fact_matrix.v2\",\"fact_rows\":[{\"fact_id\":\"FACT_PRODUCTION_RUNTIME_DELIVERY\",\"category\":\"FULFILLMENT\",\"fact_target\":\"Whether the signed parcel was received by the user.\",\"materiality\":\"CORE\"}]}}</authorized_dossier_json>"
        )
    )

    assert initiator.model == PRODUCTION_RUNTIME_FIXTURE_MODEL
