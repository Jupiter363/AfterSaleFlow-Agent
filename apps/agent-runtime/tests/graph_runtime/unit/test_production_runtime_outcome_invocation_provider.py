...
from app.graph_runtime.production_runtime_room_adapters import ProductionObjectOutcomeInvocationProvider
from app.graphs.outcome import production_runtime as outcome_production_runtime
...
ACTION_HASH = "a" * 64
EVENT_HASH = "e" * 64
SNAPSHOT_URI = "urn:production-runtime:object:review-invocation:command-review-001"
...
def _java_invocation_document(*, event_hash: str = EVENT_HASH) -> dict[str, Any]:
    ...
    return {
        "schema_version": "production-runtime-review-invocation.v1",
        "private_command": {
...
def _execution(payload: bytes, document: dict[str, Any]) -> SimpleNamespace:
    ...
    values: dict[str, Any] = {
        ...
        "event_ref": {
            ...
            "schema_version": "production-runtime-review-human-decision-event.v1",
            "uri": "urn:production-runtime:event:review-decision:command-review-001",
            "sha256": EVENT_HASH,
            "size_bytes": 512,
        },
        "invocation_context": {
            "agent_profile_id": "production-runtime.all-rooms.agent.v1",
...
async def test_java_review_invocation_epoch_zero_keeps_action_and_event_hashes_distinct() -> None:
    ...
    loaded = await ProductionObjectOutcomeInvocationProvider(store).load(execution)
    outcome_production_runtime._require_loaded_invocation(_authority(execution), loaded)
...
async def test_java_review_invocation_rejects_private_event_hash_drift() -> None:
    ...
    with pytest.raises(
        ...
        match="OUTCOME_PRODUCTION_RUNTIME_LOADED_INPUT_MISMATCH",
    ):
        outcome_production_runtime._require_loaded_invocation(_authority(execution), loaded)
