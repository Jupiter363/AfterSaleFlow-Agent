...
from app.graph_runtime.production_runtime_composite import ProductionCompositeExecutor
...
def _binding() -> GraphProductionBindingSettings:
    command = _command()
...
def _authority(**overrides: Any) -> ProductionRuntimeAuthority:
    return ProductionRuntimeAuthority.from_context(
        GraphProductionRuntimeContextSettings.model_validate(_context_values(**overrides)),
        (_binding(),),
    )


def _command_envelope(command: RoomGraphCommand) -> ProductionGraphCommandEnvelope:
    values = {
...
def _command_token(
    ...
    **overrides: Any,
    ...
    return jwt.encode(
        ...
        key,
        algorithm="ES256",
        headers={"alg": "ES256", "kid": kid, "typ": "production-runtime-graph-command+jwt"},
    )
...
def test_runtime_projection_and_command_credential_are_distinct() -> None:
    ...
    verified = ProductionInvocationVerifier(
...
def test_parallel_prepare_execute_and_terminate_claims_are_strictly_discriminated() -> None:
    ...
    envelope = _command_envelope(command)
    verifier = ProductionInvocationVerifier(
        key_resolver=_Resolver(key.public_key()),
        authority=_authority(),
        now=lambda: NOW,
    )
...
async def test_signed_java_agent_session_round_trips_and_missing_or_tampered_claims_fail_closed() -> None:
    ...
    identity = await ProductionThreadIdentityResolver().resolve(
        command=command,
    ...
    with pytest.raises(GraphThreadBindingError, match="AGENT_SESSION_REQUIRED"):
        await ProductionThreadIdentityResolver().resolve(
            command=command,
...
async def test_non_intake_fallback_session_is_replay_stable_and_isolated_by_exact_thread() -> None:
    ...
    async def identity(command: RoomGraphCommand):
        ...
        return await ProductionThreadIdentityResolver().resolve(
            command=command,
            verified_invocation=verified,
        )
...
def test_command_envelope_hash_and_signed_room_fence_are_tamper_evident() -> None:
    ...
    with pytest.raises(ValueError, match="self-hash"):
        ProductionGraphCommandEnvelope.model_validate(
...
def test_result_envelope_binds_candidate_lane_command_result_and_proposal() -> None:
    ...
    proposal = {"schema_version": "production-runtime-intake-proposal.v1", "case_id": "case-001"}
    values = {
...
def test_candidate_terminal_materializer_atomically_binds_proposal_and_result_envelope() -> None:
    ...
    assert envelope.execution_provider == "production-runtime-composite"
...
def test_shadow_and_candidate_gateways_reject_each_others_credentials() -> None:
    key = ec.generate_private_key(ec.SECP256R1())
    command = _command()
    candidate = ProductionInvocationVerifier(
        key_resolver=_Resolver(key.public_key()),
        authority=_authority(),
        now=lambda: NOW,
    ).verify_envelope(
    ...
    shadow = GraphCommandGateway(
        ...
        pool=object(),
        input_authorizer=ProductionInputAuthorizer(),
    )
    ...
    with pytest.raises(Exception, match="PRODUCTION_RUNTIME_CREDENTIAL"):
...
class _StaleGenerationConnection:
    async def execute(self, sql: str, params: tuple[Any, ...]) -> _Cursor:
        if "from agent_graph_production_runtime_environment_generation" in sql:
            return _Cursor(
                {
...
async def test_composite_executor_requires_exact_four_room_providers_and_no_fallback() -> None:
    ...
    execution = SimpleNamespace(
        ...
        fence=SimpleNamespace(execution_lane=GraphGatewayMode.PRODUCTION),
    )
    ...
    with pytest.raises(GraphContractError, match="exactly four"):
        ProductionCompositeExecutor(providers[:-1])
    with pytest.raises(GraphContractError, match="duplicate"):
        ProductionCompositeExecutor((*providers, providers[0]))
