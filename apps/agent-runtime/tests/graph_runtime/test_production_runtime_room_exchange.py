...
_SECRET = "production-runtime-service-secret"
...
async def test_exchange_posts_canonical_json_only_to_the_fixed_java_path() -> None:
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert str(request.url) == (
            "https://java.internal" + PRODUCTION_RUNTIME_ROOM_OBJECT_LOAD_PATH
        )
...
async def test_exchange_reuses_one_lifecycle_owned_http_client(
    ...
    with pytest.raises(GraphContractError, match="PRODUCTION_RUNTIME_ROOM_EXCHANGE_CLOSED"):
