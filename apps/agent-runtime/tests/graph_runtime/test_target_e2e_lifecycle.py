...
from app.graph_runtime.target_e2e_lifecycle import (
    TARGET_E2E_LIFECYCLE_JWT_TYPE,
    FilesystemTargetE2ECheckpointBarrierControl,
    PostgresTargetE2ELifecycleRepository,
    TargetE2ECheckpointCommitBinding,
    TargetE2ECheckpointDurabilityRepository,
    TargetE2ECheckpointNotDurableError,
...
def _binding(**changes: Any) -> TargetE2ELifecycleBinding:
    values: dict[str, Any] = {
    ...
    return TargetE2ELifecycleBinding(**values)


def _row(
    binding: TargetE2ELifecycleBinding,
    *,
    state: TargetE2ELifecycleState = TargetE2ELifecycleState.ACTIVE,
...
async def test_java_receipts_advance_exactly_and_replay_without_another_update() -> None:
    ...
    transitions = (
        (
            TargetE2ELifecycleState.ACTIVE,
            TargetE2ELifecycleState.DRAIN_ONLY,
            NOW,
        ),
        (
            TargetE2ELifecycleState.DRAIN_ONLY,
...
async def test_replay_with_another_valid_hash_and_timestamp_is_rejected() -> None:
    ...
    changed = build_target_e2e_lifecycle_receipt(
        binding,
        from_state=TargetE2ELifecycleState.DRAIN_ONLY,
        to_state=TargetE2ELifecycleState.DRAINED,
        transitioned_at=persisted_at + timedelta(microseconds=1),
    )

    with pytest.raises(TargetE2ELifecycleTransitionError, match="different receipt hash"):
        await PostgresTargetE2ELifecycleRepository().reconcile(
            connection,
            receipt=changed,
            expected_binding=binding,
        )
...
async def test_cross_binding_regression_and_premature_transition_fail_closed() -> None:
    ...
    with pytest.raises(TargetE2ELifecycleBindingError):
    ...
    with pytest.raises(TargetE2ELifecycleTransitionError):
        await repository.reconcile(
            regressed,
            receipt=receipt,
            expected_binding=binding,
    ...
    premature = build_target_e2e_lifecycle_receipt(
        binding,
        from_state=TargetE2ELifecycleState.ACTIVE,
        to_state=TargetE2ELifecycleState.DRAIN_ONLY,
...
async def test_drained_requires_zero_unresolved_commands_and_active_leases() -> None:
    ...
    with pytest.raises(TargetE2EDrainIncompleteError):
        await PostgresTargetE2ELifecycleRepository().reconcile(
            connection,
            receipt=receipt,
            expected_binding=binding,
...
def test_receipt_model_rejects_skip_and_unknown_payload_members() -> None:
    ...
    with pytest.raises(ValidationError, match="skips or regresses"):
        TargetE2ELifecycleReceipt.model_validate(receipt)
    ...
    return jwt.encode(
        {
            ...
            "sub": "target-e2e-lifecycle-reconcile",
            "iat": now,
            "nbf": now,
            "exp": now + 30,
        ...
        private_key,
        algorithm="ES256",
        headers={"alg": "ES256", "kid": "java-target-key-1", "typ": TARGET_E2E_LIFECYCLE_JWT_TYPE},
    )
...
def test_verifier_requires_java_mtls_signature_time_and_exact_binding() -> None:
    ...
    same_second_receipt = build_target_e2e_lifecycle_receipt(
        binding,
        from_state=TargetE2ELifecycleState.ACTIVE,
        to_state=TargetE2ELifecycleState.DRAIN_ONLY,
    ...
    with pytest.raises(TargetE2ELifecycleAuthenticationError):
        verifier.verify(
    ...
    future_receipt = build_target_e2e_lifecycle_receipt(
        binding,
        from_state=TargetE2ELifecycleState.ACTIVE,
        to_state=TargetE2ELifecycleState.DRAIN_ONLY,
        transitioned_at=NOW + timedelta(seconds=1),
    ...
    changed_binding_verifier = TargetE2ELifecycleReceiptVerifier(
        key_resolver=_KeyResolver(private_key.public_key()),
...
class _DurabilityRepository(TargetE2ECheckpointDurabilityRepository):
...
def _checkpoint_binding(binding: TargetE2ELifecycleBinding) -> TargetE2ECheckpointCommitBinding:
    return TargetE2ECheckpointCommitBinding(
...
async def test_checkpoint_barrier_is_default_off_and_when_enabled_waits_after_durability() -> None:
    ...
    disabled = TargetE2ECheckpointRecoveryBarrier(
        expected_binding=binding,
    ...
    async def release(_binding: TargetE2ECheckpointCommitBinding) -> None:
        events.append("released")

    unarmed = TargetE2ECheckpointRecoveryBarrier(
        expected_binding=binding,
        enabled=True,
    ...
    enabled = TargetE2ECheckpointRecoveryBarrier(
        expected_binding=binding,
        enabled=True,
        isolated_synthetic_environment=True,
...
async def test_checkpoint_barrier_rejects_non_durable_commit_and_is_bounded() -> None:
    ...
    nondurable = TargetE2ECheckpointRecoveryBarrier(
        expected_binding=binding,
        enabled=True,
        isolated_synthetic_environment=True,
        arming_policy=lambda _binding: True,
    ...
    with pytest.raises(TargetE2ECheckpointNotDurableError):
        await nondurable.wait_after_durable_commit(object(), checkpoint)

    bounded = TargetE2ECheckpointRecoveryBarrier(
        expected_binding=binding,
        enabled=True,
        isolated_synthetic_environment=True,
...
async def test_postgres_durability_probe_accepts_only_explicit_true() -> None:
    ...
    class Connection:
        ...
        async def execute(self, sql: str, params: tuple[Any, ...]) -> _Cursor:
            ...
            assert "TARGET_E2E_CANDIDATE" in sql
            self.params = params
            return _Cursor({"durable": self.durable})

    accepted = Connection(True)
    await TargetE2ECheckpointDurabilityRepository().require_durable(accepted, checkpoint)
    assert accepted.params is not None
    assert accepted.params[-1] == checkpoint.manifest_hash

    with pytest.raises(TargetE2ECheckpointNotDurableError):
        await TargetE2ECheckpointDurabilityRepository().require_durable(
            Connection(1),
            checkpoint,
        )
...
async def test_filesystem_checkpoint_barrier_requires_exact_arm_and_release(
    ...
    control = FilesystemTargetE2ECheckpointBarrierControl(
