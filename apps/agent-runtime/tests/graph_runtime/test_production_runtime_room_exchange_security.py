...
async def test_room_exchange_forces_identity_encoding_and_rejects_compressed_responses() -> None:
    ...
    exchange = JavaProductionRoomExchange(
...
def test_room_exchange_accepts_only_exact_room_scoped_content_addressed_refs() -> None:
    ...
    assert _PROPOSAL_REF.fullmatch(f"urn:production-runtime:proposal:evidence:{digest}")
    assert _PROPOSAL_REF.fullmatch(f"urn:production-runtime:proposal:hearing:{digest}")
    assert _PROPOSAL_REF.fullmatch(f"urn:production-runtime:proposal:review:{digest}")
    assert _PROPOSAL_REF.fullmatch(f"urn:production-runtime:proposal:intake:{digest}") is None
    assert _PROPOSAL_REF.fullmatch("urn:production-runtime:proposal:review:not-a-hash") is None
    assert _PROPOSAL_REF.fullmatch(f"urn:production-runtime:proposal:review:{digest}:suffix") is None
