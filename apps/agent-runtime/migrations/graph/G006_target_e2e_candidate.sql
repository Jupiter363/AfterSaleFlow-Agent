-- Isolated target-E2E candidate activation and immutable execution-lane bindings.
...
alter table agent_graph_command
    ...
    check (
        ...
        or (
            execution_mode = 'TARGET_E2E_CANDIDATE'
...
create table agent_graph_target_e2e_activation (
    ...
    constraint ck_target_e2e_activation_id
        check (activation_id ~ '^p9act\.v1\.[0-9a-f]{32}$'),
    constraint ck_target_e2e_activation_hashes
        check (
            context_hash ~ '^[0-9a-f]{64}$'
            and candidate_sha ~ '^[0-9a-f]{40}$'
        ),
    constraint ck_target_e2e_activation_generation
        check (environment_generation >= 1),
    constraint ck_target_e2e_activation_cases
        check (jsonb_typeof(case_scope) = 'object'),
    constraint ck_target_e2e_activation_rooms
        check (
            jsonb_typeof(allowed_room_types) = 'array'
            and jsonb_array_length(allowed_room_types) > 0
        ),
    constraint ck_target_e2e_activation_context
        check (jsonb_typeof(context_json) = 'object'),
    constraint ck_target_e2e_activation_expiry
        check (expires_at > issued_at and expires_at <= issued_at + interval '7200 seconds')
);

create table agent_graph_target_e2e_environment_generation (
    environment_id varchar(128) primary key,
    environment_generation bigint not null,
    activation_id varchar(64) not null,
    context_hash varchar(64) not null,
    updated_at timestamptz not null default clock_timestamp(),
    constraint ck_target_e2e_generation_positive check (environment_generation >= 1),
    constraint ck_target_e2e_generation_context_hash
        check (context_hash ~ '^[0-9a-f]{64}$')
);

create table agent_graph_target_e2e_activation_lifecycle (
    activation_id varchar(64) primary key,
    ...
    constraint ck_target_e2e_activation_lifecycle_state
        check (lifecycle_state in ('ACTIVE', 'DRAIN_ONLY', 'DRAINED', 'REVOKED_TERMINAL')),
    constraint ck_target_e2e_activation_lifecycle_times
        check (
...
create table agent_graph_target_e2e_synthetic_case_reservation (
    activation_id varchar(64) not null,
    ...
    foreign key (activation_id)
        references agent_graph_target_e2e_activation(activation_id) on delete restrict,
    constraint ck_target_e2e_synthetic_slot check (slot_number between 1 and 16),
    constraint ck_target_e2e_synthetic_fixture_hash
        check (fixture_set_hash ~ '^[0-9a-f]{64}$')
);

create table agent_graph_target_e2e_room_authority (
    tenant_surrogate varchar(128) not null,
    ...
    foreign key (activation_id)
        references agent_graph_target_e2e_activation(activation_id) on delete restrict,
    constraint ck_target_e2e_room_type
        check (room_type in ('INTAKE', 'EVIDENCE', 'HEARING', 'REVIEW')),
    constraint ck_target_e2e_room_epoch check (room_epoch >= 0),
    constraint ck_target_e2e_room_fence check (room_fencing_token >= 1)
);

create function reject_agent_graph_target_e2e_immutable_mutation()
returns trigger
language plpgsql
as $function$
begin
    raise exception using errcode = '23514',
        message = 'target-E2E grant rows are immutable';
end;
$function$;

create trigger trg_reject_agent_graph_target_e2e_activation_mutation
before update or delete on agent_graph_target_e2e_activation
for each row execute function reject_agent_graph_target_e2e_immutable_mutation();

create trigger trg_reject_agent_graph_target_e2e_synthetic_case_mutation
before update or delete on agent_graph_target_e2e_synthetic_case_reservation
for each row execute function reject_agent_graph_target_e2e_immutable_mutation();
