...
PRODUCTION_RUNTIME_HEARING_PROPOSAL_SCHEMA = "production-runtime-hearing-proposal.v1"
PRODUCTION_RUNTIME_HEARING_SOURCE_SCHEMA = "production-runtime-room-proposal-source.v2"
_TARGET_RUNTIME_BINDING_METADATA_KEY = "hearing_production_runtime_binding_sha256"
_ACTIVATION_ID = re.compile(r"^p9act\.v1\.[0-9a-f]{32}$")
_IDENTIFIER = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._:/-]{0,127}$")
_SHA256 = re.compile(r"^[0-9a-f]{64}$")


class HearingProductionStoredPayload(BaseModel):
...
class HearingProductionProposal(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True, frozen=True)

    schema_version: Literal["production-runtime-hearing-proposal.v1"] = (
        PRODUCTION_RUNTIME_HEARING_PROPOSAL_SCHEMA
    )
    proposal_id: str = Field(pattern=_IDENTIFIER.pattern)
...
class HearingProductionProposalSource(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True, frozen=True)

    schema_version: Literal["production-runtime-room-proposal-source.v2"] = (
        PRODUCTION_RUNTIME_HEARING_SOURCE_SCHEMA
    )
    room_type: Literal["HEARING"] = "HEARING"
    proposal: HearingProductionProposal
...
class HearingProductionLoadedInvocation:
...
class HearingProductionInvocationProvider(Protocol):
    async def load(
        self,
...
class HearingProductionPayloadStore(Protocol):
    async def put(
        self,
        *,
        execution: GatewayExecution,
...
@dataclass(frozen=True, slots=True)
class HearingProductionFamilyRegistration:
    identity: HearingGraphIdentity
    operation_bindings: Mapping[HearingOperation, HearingProductionOperationBinding]
    execution_lane: Literal["PRODUCTION"] = PRODUCTION_RUNTIME_EXECUTION_LANE
    proposal_schema_version: Literal["production-runtime-hearing-proposal.v1"] = (
        PRODUCTION_RUNTIME_HEARING_PROPOSAL_SCHEMA
    )
    graph_output_authority: Literal["PROPOSAL_ONLY"] = "PROPOSAL_ONLY"
    external_effects_allowed: Literal[False] = False

def _family_registration(identity: HearingGraphIdentity) -> HearingProductionFamilyRegistration:
    return HearingProductionFamilyRegistration(
        identity=identity,
        operation_bindings=MappingProxyType(
            {
                operation: HEARING_PRODUCTION_RUNTIME_OPERATION_BINDINGS[operation]
...
PRODUCTION_RUNTIME_HEARING_FAMILY_REGISTRY: Mapping[
    tuple[str, str, str], HearingProductionFamilyRegistration
] = MappingProxyType(
    {
        (
            identity.graph_key,
...
if (
    len(PRODUCTION_RUNTIME_HEARING_FAMILY_REGISTRY) != 4
    or {
        operation
        for registration in PRODUCTION_RUNTIME_HEARING_FAMILY_REGISTRY.values()
        for operation in registration.operation_bindings
    }
    != set(HearingOperation)
):
    raise RuntimeError("production-runtime Hearing registry must bind four explicit families")


def production_runtime_hearing_family_registrations() -> Mapping[
    tuple[str, str, str], HearingProductionFamilyRegistration
]:
    """Return the closed internal families used by the explicit HEARING provider."""

    return PRODUCTION_RUNTIME_HEARING_FAMILY_REGISTRY


@dataclass(frozen=True, slots=True)
class HearingProductionExecutionContext:
    execution: GatewayExecution
    activation_id: str
    ...
    def from_gateway_execution(
        ...
        if (
            ...
            != PRODUCTION_RUNTIME_EXECUTION_LANE
        ...
        if any(not reference.uri.startswith("urn:production-runtime:") for reference in references):
    ...
    def require_operation(
        ...
        registration = PRODUCTION_RUNTIME_HEARING_FAMILY_REGISTRY.get(key)
...
@dataclass(frozen=True, slots=True)
class HearingProductionRuntimeBundle:
    graph: Any
    identity: HearingGraphIdentity
    context: HearingProductionExecutionContext
    operation: HearingOperation
    request: BaseModel
    invocation: HearingGraphInvocation
...
def build_production_runtime_hearing_runtime_bundle(
    *,
    execution: GatewayExecution,
    operation: HearingOperation,
    request: BaseModel,
    ...
    context = HearingProductionExecutionContext.from_gateway_execution(execution)
    ...
    runtime_binding_sha256 = canonical_sha256(
        {
            "schema_version": "hearing-production-runtime-binding.v1",
    ...
    return HearingProductionRuntimeBundle(
        graph=graph,
...
@dataclass(frozen=True, slots=True)
class HearingProductionProposalMaterial:
    source: HearingProductionProposalSource
    proposal_hash: str
    payload: HearingProductionStoredPayload
    checkpoint_ns: str
    checkpoint_id: str
    cognitive_revision: int
    runtime_binding_sha256: str
...
@dataclass(frozen=True, slots=True)
class _HearingProductionExecutionPlan:
    context: HearingProductionExecutionContext
    loaded: HearingProductionLoadedInvocation
    registration: HearingProductionFamilyRegistration
    bundle: HearingProductionRuntimeBundle
...
class _HearingV5FrameStream:
    ...
    def __init__(self, execution: GatewayExecution, operation: HearingOperation) -> None:
        ...
        self.model_node = HEARING_PRODUCTION_RUNTIME_OPERATION_BINDINGS[operation].model_nodes[-1]
...
class HearingProductionRuntimeAdapter:
    ...
    def __init__(
        ...
        *,
        checkpointer: FencedPostgresSaver,
        invocation_provider: HearingProductionInvocationProvider,
        payload_store: HearingProductionPayloadStore,
    ...
    async def stream(
        ...
        yield self._event(
            ...
            payload=AgentStreamPayload(node="hearing_production_runtime"),
        )
        sequence += 1
    ...
    async def _commit_terminal(
        self,
        execution: GatewayExecution,
        material: HearingProductionProposalMaterial,
    ) -> Any:
        command = execution.admission.command
        invocation = command.invocation_context
        materializer = TerminalResultMaterializer(
            ...
            target_proposal_source=ProductionRoomProposalSource.model_validate(
    ...
    async def _prepare_execution(
        self,
        execution: GatewayExecution,
    ) -> _HearingProductionExecutionPlan:
        context = HearingProductionExecutionContext.from_gateway_execution(execution)
        ...
        registration = context.require_operation(operation, request)
        bundle = build_production_runtime_hearing_runtime_bundle(
            execution=execution,
            operation=operation,
            request=request,
        ...
        return _HearingProductionExecutionPlan(
            context=context,
            loaded=loaded,
    ...
    async def _proposal_material(
        self,
        execution: GatewayExecution,
        plan: _HearingProductionExecutionPlan,
        ...
        proposal_id_hash = canonical_sha256(
            {
                "schema_version": "production-runtime-hearing-proposal-id.v1",
                "activation_id": context.activation_id,
                "command_id": command.command_id,
                "operation": operation.value,
        ...
        if not isinstance(stored, HearingProductionStoredPayload) or (
            stored.proposal_id,
        ...
        source = HearingProductionProposalSource(proposal=proposal)
        return HearingProductionProposalMaterial(
            source=source,
    ...
    def _visible_events(
        candidate: Any,
        operation_binding: HearingProductionOperationBinding,
    ) -> tuple[tuple[str, AgentStreamPayload], ...]:
        if not isinstance(candidate, tuple) or len(candidate) != 2:
            raise HearingGraphContractError("HEARING_TARGET_STREAM_EVENT_INVALID")
        mode, value = candidate
...
def build_production_runtime_hearing_provider(
    *,
    checkpointer: FencedPostgresSaver,
    invocation_provider: HearingProductionInvocationProvider,
    payload_store: HearingProductionPayloadStore,
    ...
    return HearingProductionRuntimeAdapter(
        checkpointer=checkpointer,
