"""Production-Runtime REVIEW executor backed by the private Outcome LangGraph.
...
PRODUCTION_RUNTIME_REVIEW_ROOM_TYPE = "REVIEW"
_RUNTIME_BINDING_METADATA_KEY = "outcome_production_runtime_binding_sha256"


class OutcomeProductionProposalPayload(BaseModel):
...
class OutcomeProductionProposal(BaseModel):
    """Exact REVIEW member of production-runtime-room-proposal-source.v2."""

    model_config = ConfigDict(extra="forbid", frozen=True, strict=True)

    schema_version: Literal["production-runtime-review-proposal.v1"] = "production-runtime-review-proposal.v1"
...
class OutcomeProductionProposalSource(BaseModel):
    ...
    proposal: OutcomeProductionProposal
...
class StoredOutcomeProductionProposal:
    artifact_id: str
    schema_version: str
    uri: str
    sha256: str
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or not self.uri.startswith("urn:production-runtime:proposal:review:")
            or len(self.uri) > 512
            ...
            raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_STORED_PROPOSAL_INVALID")


@dataclass(frozen=True, slots=True)
class LoadedOutcomeProductionInvocation:
...
class OutcomeProductionInvocationProvider(Protocol):
    async def load(
        self,
...
class OutcomeProductionProposalStore(Protocol):
    async def put(
        self,
        execution: GatewayExecution,
        *,
...
@dataclass(frozen=True, slots=True)
class OutcomeProductionExecutionContext:
    ...
    def from_gateway_execution(
        ...
        if type(execution) is not GatewayExecution:
            raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_GATEWAY_EXECUTION_REQUIRED")
        ...
        if (
            ...
            or lane != PRODUCTION_RUNTIME_EXECUTION_LANE
            or fence_lane != PRODUCTION_RUNTIME_EXECUTION_LANE
            ...
            raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_GATEWAY_AUTHORITY_REQUIRED")
        ...
        if (
            ...
            raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_GATEWAY_AUTHORITY_INVALID")
        return cls(
            execution=execution,
...
@dataclass(frozen=True, slots=True)
class OutcomeProductionExecutionResult:
    payload: OutcomeProductionProposalPayload
    payload_bytes: bytes
    source: OutcomeProductionProposalSource
    checkpoint_ns: str
    checkpoint_id: str
    cognitive_revision: int
...
@dataclass(frozen=True, slots=True)
class OutcomeProductionExecutorRegistration:
    """Process-local exact binding; the shared registry must import it explicitly."""

    binding: VersionBinding
    executor: CompiledOutcomeProductionExecutor
    provider: str
    model: str

    def __post_init__(self) -> None:
        require_exact_outcome_production_runtime_registry_binding(self.binding)
        if not self.provider or len(self.provider) > 64 or not self.model or len(self.model) > 128:
            raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_PROVIDER_BINDING_INVALID")


class CompiledOutcomeProductionExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        invocation_provider: OutcomeProductionInvocationProvider,
        proposal_store: OutcomeProductionProposalStore,
    ) -> None:
        if not isinstance(saver, FencedPostgresSaver):
            raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_FENCED_SAVER_REQUIRED")
        if not callable(getattr(invocation_provider, "load", None)) or not callable(
            getattr(proposal_store, "put", None)
        ):
            raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_PORTS_INCOMPLETE")
        self._saver = saver
        self._invocation_provider = invocation_provider
        self._proposal_store = proposal_store

    async def execute(self, execution: GatewayExecution) -> OutcomeProductionExecutionResult:
        authority = OutcomeProductionExecutionContext.from_gateway_execution(execution)
        ...
        if graph.checkpointer is not self._saver:
            raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_CHECKPOINTER_BINDING_INVALID")
        runtime_binding_hash = canonical_sha256(
            {
                "schema_version": "outcome-production-runtime-binding.v1",
        ...
        proposal_identity_hash = canonical_sha256(
            {
                "schema_version": "outcome-production-runtime-proposal-identity.v1",
        ...
        source = build_outcome_production_runtime_proposal_source(
            command=execution.admission.command,
            proposal_id=proposal_id,
            payload_ref=stored.uri,
            payload_hash=payload_hash,
        )
        return OutcomeProductionExecutionResult(
            payload=payload,
...
class DeterministicOutcomeProductionModel:
    """Fixture-only model used to make isolated production-runtime hashes repeatable."""
    ...
    def __call__(self, request: ReviewCopilotRequest) -> ReviewCopilotAnswer:
        ...
        return ReviewCopilotAnswer(
            answer="Synthetic production-runtime review remains advisory and requires human review.",
            statements=[
                ReviewStatement(
                    kind="SUGGESTION",
...
def build_outcome_production_runtime_registration(
    *,
    binding: VersionBinding,
    saver: FencedPostgresSaver,
    invocation_provider: OutcomeProductionInvocationProvider,
    ...
    provider: str,
    model: str,
) -> OutcomeProductionExecutorRegistration:
    return OutcomeProductionExecutorRegistration(
        binding=binding,
        executor=CompiledOutcomeProductionExecutor(
            saver=saver,
...
def build_outcome_production_runtime_proposal_source(
    *,
    ...
    if command.room_type != PRODUCTION_RUNTIME_REVIEW_ROOM_TYPE:
        raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_REVIEW_COMMAND_REQUIRED")
    return OutcomeProductionProposalSource(
        proposal=OutcomeProductionProposal(
            proposal_id=proposal_id,
...
def require_exact_outcome_production_runtime_registry_binding(binding: VersionBinding) -> None:
    expected = {
    ...
    if type(binding) is not VersionBinding or any(
        ...
        raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_REGISTRY_BINDING_MISMATCH")
...
def _require_gateway_command_binding(
    ...
    require_exact_outcome_production_runtime_registry_binding(registry)
    if (
        type(command) is not RoomGraphCommand
        or type(fence) is not GraphFenceContext
        or command.room_type != PRODUCTION_RUNTIME_REVIEW_ROOM_TYPE
        or command.graph_key != PRODUCTION_RUNTIME_GRAPH_KEY
        or command.graph_version != PRODUCTION_RUNTIME_GRAPH_VERSION
        or command.checkpoint_schema_version != PRODUCTION_RUNTIME_CHECKPOINT_SCHEMA_VERSION
        ...
        or invocation.output_schema_version != PRODUCTION_RUNTIME_OUTPUT_SCHEMA_VERSION
        ...
        raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_COMMAND_BINDING_MISMATCH")


def _require_loaded_invocation(
    authority: OutcomeProductionExecutionContext,
    loaded: LoadedOutcomeProductionInvocation,
) -> None:
    if not isinstance(loaded, LoadedOutcomeProductionInvocation):
        raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_LOADED_INPUT_MISMATCH")
...
def _require_recovery_snapshot(
    ...
    *,
    ...
    if (
        ...
        raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_RECOVERY_BINDING_MISMATCH")
    validate_outcome_review_recovery_state(
        values,
        command=loaded.command,
        request=loaded.request,
...
def _projection(state: Mapping[str, Any]) -> OutcomeReviewProjection:
    ...
    if not isinstance(value, Mapping):
        raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_PROJECTION_MISSING")
...
def _reconstruct_projection(
    ...
    if (
        ...
        raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_DETERMINISTIC_REPLAY_MISMATCH")
    _require_proposal_only_projection(projection)
    return projection


def _proposal_payload(projection: OutcomeReviewProjection) -> OutcomeProductionProposalPayload:
    _require_proposal_only_projection(projection)
    return OutcomeProductionProposalPayload(
...
def _require_proposal_only_projection(projection: OutcomeReviewProjection) -> None:
    if (
        ...
        raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_FORMAL_AUTHORITY_FORBIDDEN")
...
def _terminal_checkpoint(
    ...
    if (
        ...
        raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_CHECKPOINT_NOT_TERMINAL")
    ...
    if (
        ...
        raise OutcomeReviewContractError("OUTCOME_PRODUCTION_RUNTIME_CHECKPOINT_BINDING_INVALID")
    return checkpoint_ns, checkpoint_id, cast(int, values["cognitive_revision"])


def deterministic_outcome_production_runtime_invocation(
    *,
    command: OutcomeReviewPrivateCommand,
    request: ReviewCopilotRequest,
    snapshot_uri: str,
    ...
    model = DeterministicOutcomeProductionModel()
    copilot = ReviewCopilot(model)
    return LoadedOutcomeProductionInvocation(
        command=command,
        request=request,
