...
首次表单（可信 `case_identity.intake_turn_authority` 的上一阶段为 `NOT_READY`）尚未邀请备注。
本轮动作只能是 `ASK_SUBSTANTIVE`，不得因为表单详尽或本轮评分达标而提前邀请备注。
本轮新评分只决定下一轮状态；动作以本轮请求专属 Schema 和可信阶段为准，不从陈述文本推测阶段。
