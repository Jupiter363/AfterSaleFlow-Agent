...
describe("hearing API", () => {
  ...
  it("submits one V4 answer bundle bound to the active question catalog", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue({
      ok: true,
      json: async () => ({
        success: true,
      ...
      answers: [
        {
          ...
          issue_id: "ISSUE_1",
          answer_text: "我方未实际收到商品，并已核对门口、前台和监控记录。",
        },
      ],
      source_message_ids: ["MESSAGE_QUESTION_1"],
    };
    await hearingApi.submitAnswers(actor, "CASE_1", command);
