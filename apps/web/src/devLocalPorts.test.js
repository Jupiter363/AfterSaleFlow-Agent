...
describe("local service port contract", () => {
  ...
  it("keeps dev and Docker ports fixed and waits for the Python health endpoint", () => {
    ...
    const devLocal = readProjectFile("tools/dev/dev-local.ps1");
    const smokeTest = readProjectFile("tools/verify/smoke-test.sh");
    const generateOpenapi = readProjectFile("tools/generate/generate-openapi.sh");
    const javaLocalConfig = readProjectFile(
      "apps/domain-service/src/main/resources/application-local.yml",
    );
